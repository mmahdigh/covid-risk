export const obj = {
  "US - California - Alameda": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 351.2175041538798,
        lastWeekAverageDeathPerMillionEachDay: 3.8463829853795555,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Amador": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 578.5872408935398,
        lastWeekAverageDeathPerMillionEachDay: 14.37483828306931,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Butte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 355.8621444800307,
        lastWeekAverageDeathPerMillionEachDay: 7.169383863150801,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Calaveras": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 479.25062629343205,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Colusa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 371.2813848795657,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Contra Costa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 258.33834694666615,
        lastWeekAverageDeathPerMillionEachDay: 2.3530338408373237,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Del Norte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 71.9114051488566,
        lastWeekAverageDeathPerMillionEachDay: 5.136528939204043,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - El Dorado": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 41.484523679884674,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Fresno": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 557.5011935730222,
        lastWeekAverageDeathPerMillionEachDay: 2.1447853048461996,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Glenn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 422.63938294650075,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Humboldt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 110.6537423095649,
        lastWeekAverageDeathPerMillionEachDay: 5.269225824264995,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Imperial": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2174.2129514664907,
        lastWeekAverageDeathPerMillionEachDay: 16.554920950252466,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Inyo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1053.2734630522757,
        lastWeekAverageDeathPerMillionEachDay: 15.838698692515422,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Kern": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 534.3245182747871,
        lastWeekAverageDeathPerMillionEachDay: 2.2217235687101335,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Kings": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 673.4667189747616,
        lastWeekAverageDeathPerMillionEachDay: 4.670365596218874,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 326.1578604044358,
        lastWeekAverageDeathPerMillionEachDay: -2.218760955132216,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Lassen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 228.96019363490663,
        lastWeekAverageDeathPerMillionEachDay: 4.672657012957278,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Los Angeles": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 884.0427739240156,
        lastWeekAverageDeathPerMillionEachDay: 8.210747373105143,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Madera": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 705.5368754250701,
        lastWeekAverageDeathPerMillionEachDay: 3.6321074667957274,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Marin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 525.449529799943,
        lastWeekAverageDeathPerMillionEachDay: 2.2077711336132055,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Mariposa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 232.51758414230076,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Mendocino": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 268.0319695505431,
        lastWeekAverageDeathPerMillionEachDay: 1.531611254574532,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Merced": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 777.8738115816768,
        lastWeekAverageDeathPerMillionEachDay: 4.6302012594147435,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Mono": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 207.6986984214899,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Monterey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 407.7767871336057,
        lastWeekAverageDeathPerMillionEachDay: 7.569706298686787,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Napa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 174.2362643744918,
        lastWeekAverageDeathPerMillionEachDay: 1.0371206212767368,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Nevada": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 390.95784672447496,
        lastWeekAverageDeathPerMillionEachDay: 25.77744044337198,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Orange": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 558.9332970577751,
        lastWeekAverageDeathPerMillionEachDay: 3.6437502665335844,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Placer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 258.58021886430566,
        lastWeekAverageDeathPerMillionEachDay: 3.945051882811875,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Plumas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 53.17169139150316,
        lastWeekAverageDeathPerMillionEachDay: 7.5959559130718795,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Riverside": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 705.9168297210413,
        lastWeekAverageDeathPerMillionEachDay: 9.887924138458228,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Sacramento": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 477.43061148488016,
        lastWeekAverageDeathPerMillionEachDay: 6.535101873033833,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - San Benito": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 366.1953891224048,
        lastWeekAverageDeathPerMillionEachDay: 9.098022089997633,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - San Bernardino": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 608.6918629319498,
        lastWeekAverageDeathPerMillionEachDay: 6.814937425441143,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - San Diego": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 383.42524555691017,
        lastWeekAverageDeathPerMillionEachDay: 5.049573546396808,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - San Francisco": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 199.6485731366039,
        lastWeekAverageDeathPerMillionEachDay: 1.458471719342074,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - San Joaquin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 736.0775072558087,
        lastWeekAverageDeathPerMillionEachDay: 5.060884312683176,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - San Luis Obispo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 208.39882590220802,
        lastWeekAverageDeathPerMillionEachDay: 6.55976933832616,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - San Mateo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 258.29242616163106,
        lastWeekAverageDeathPerMillionEachDay: 3.5408052648419845,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Santa Barbara": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 329.2280609810997,
        lastWeekAverageDeathPerMillionEachDay: 2.2396466733408142,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Santa Clara": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 302.4091060932064,
        lastWeekAverageDeathPerMillionEachDay: 4.223797855259193,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Santa Cruz": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 256.2103560225905,
        lastWeekAverageDeathPerMillionEachDay: 8.888930719151098,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Shasta": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 366.5037760995113,
        lastWeekAverageDeathPerMillionEachDay: 6.346385733324871,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Siskiyou": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 183.74331059509862,
        lastWeekAverageDeathPerMillionEachDay: 3.2811305463410463,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Solano": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 207.75484035269176,
        lastWeekAverageDeathPerMillionEachDay: 0.6382637184414494,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Sonoma": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 351.9873122734335,
        lastWeekAverageDeathPerMillionEachDay: 3.4678552933343196,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Stanislaus": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 949.7693676678896,
        lastWeekAverageDeathPerMillionEachDay: 12.971447250312615,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Sutter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 443.4315413886626,
        lastWeekAverageDeathPerMillionEachDay: 14.731944896633308,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Tehama": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 491.67230041177555,
        lastWeekAverageDeathPerMillionEachDay: 4.389931253676567,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Trinity": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 162.80016280016278,
        lastWeekAverageDeathPerMillionEachDay: 11.628583057154485,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Tulare": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 757.1938780982207,
        lastWeekAverageDeathPerMillionEachDay: 7.967236272988158,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Tuolumne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 348.7646389368185,
        lastWeekAverageDeathPerMillionEachDay: 5.244581036643887,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Ventura": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 248.22518989227024,
        lastWeekAverageDeathPerMillionEachDay: 2.0263280807532262,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Yolo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 471.65532879818596,
        lastWeekAverageDeathPerMillionEachDay: 9.718172983479105,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - California - Yuba": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 190.67473432653682,
        lastWeekAverageDeathPerMillionEachDay: 9.07974925364461,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 902.5532400115186,
        lastWeekAverageDeathPerMillionEachDay: 11.595973105073044,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Alamosa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1293.6610608020699,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Arapahoe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 784.3555338948202,
        lastWeekAverageDeathPerMillionEachDay: 2.828466557734442,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Bent": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1793.078716155639,
        lastWeekAverageDeathPerMillionEachDay: 179.30787161556393,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Boulder": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 490.50264258298694,
        lastWeekAverageDeathPerMillionEachDay: 5.255385456246288,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Broomfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 879.8694387284468,
        lastWeekAverageDeathPerMillionEachDay: 10.136744685811598,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Chaffee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1031.636863823934,
        lastWeekAverageDeathPerMillionEachDay: -7.017937849142408,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Cheyenne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1638.4489350081922,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Clear Creek": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 206.1855670103093,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Conejos": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 365.6307129798903,
        lastWeekAverageDeathPerMillionEachDay: -17.410986332375728,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Costilla": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 514.5356315924878,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Crowley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1979.871308364956,
        lastWeekAverageDeathPerMillionEachDay: 47.13979305630847,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Delta": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 641.8073294397022,
        lastWeekAverageDeathPerMillionEachDay: 13.753014202279335,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Denver": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 888.3253966180382,
        lastWeekAverageDeathPerMillionEachDay: 7.464919299311245,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 484.11807924728186,
        lastWeekAverageDeathPerMillionEachDay: 9.763725968012407,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Eagle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 217.67917717271027,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - El Paso": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 671.8461749881664,
        lastWeekAverageDeathPerMillionEachDay: 10.509990340723972,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Elbert": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 261.8878371805904,
        lastWeekAverageDeathPerMillionEachDay: 10.68929947675879,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Fremont": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 292.64825769769436,
        lastWeekAverageDeathPerMillionEachDay: 5.972413422401925,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Garfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 399.59374635786946,
        lastWeekAverageDeathPerMillionEachDay: 16.649739431577895,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Gilpin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 160.17940092904053,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Grand": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 254.2265158256006,
        lastWeekAverageDeathPerMillionEachDay: -9.079518422342877,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Gunnison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 286.3360439812164,
        lastWeekAverageDeathPerMillionEachDay: -16.36205965606951,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Huerfano": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2174.8586341887776,
        lastWeekAverageDeathPerMillionEachDay: 82.8517574929058,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1010.497854622127,
        lastWeekAverageDeathPerMillionEachDay: 9.068256274804439,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Kiowa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1422.4751066856334,
        lastWeekAverageDeathPerMillionEachDay: 203.21072952651903,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Kit Carson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1127.236860645343,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - La Plata": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 391.31285462727453,
        lastWeekAverageDeathPerMillionEachDay: 30.491910750177233,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Larimer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 313.81427238518455,
        lastWeekAverageDeathPerMillionEachDay: 6.804646212433848,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Las Animas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 206.8109747690611,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 350.81564637782844,
        lastWeekAverageDeathPerMillionEachDay: 25.05826045555917,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Logan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2320.4962291936276,
        lastWeekAverageDeathPerMillionEachDay: 19.124968921925504,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Mesa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 771.6749886518384,
        lastWeekAverageDeathPerMillionEachDay: 21.306752387745835,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Mineral": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1300.3901170351105,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Moffat": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1355.1155612436949,
        lastWeekAverageDeathPerMillionEachDay: 21.50977081339198,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Montezuma": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 572.8908070121835,
        lastWeekAverageDeathPerMillionEachDay: 21.824411695702224,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Montrose": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 654.8482155386126,
        lastWeekAverageDeathPerMillionEachDay: 20.046373945059567,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Morgan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2373.7443236548784,
        lastWeekAverageDeathPerMillionEachDay: 29.487507126147555,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Otero": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1477.1856877120035,
        lastWeekAverageDeathPerMillionEachDay: 70.34217560533351,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Ouray": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 605.815831987076,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Park": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 212.2578933404086,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Phillips": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 937.8663540445488,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Pitkin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 168.8523667473406,
        lastWeekAverageDeathPerMillionEachDay: 8.040588892730504,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Prowers": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1560.9595793624712,
        lastWeekAverageDeathPerMillionEachDay: 23.47307638139054,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Pueblo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1585.2847575167436,
        lastWeekAverageDeathPerMillionEachDay: 43.258171553426386,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Rio Blanco": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 316.25553447185325,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Rio Grande": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 443.7738528445904,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Routt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 702.0828457758015,
        lastWeekAverageDeathPerMillionEachDay: 5.572086077585726,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Saguache": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 586.166471277843,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Sedgwick": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 889.6797153024913,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Summit": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 161.23311083164037,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Teller": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 275.7208129825114,
        lastWeekAverageDeathPerMillionEachDay: 5.626955366990028,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2037.4898125509376,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Weld": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 807.4158993133883,
        lastWeekAverageDeathPerMillionEachDay: 9.245220221145667,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Colorado - Yuma": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 598.8621618924044,
        lastWeekAverageDeathPerMillionEachDay: 28.517245804400208,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Connecticut - Fairfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1725.7974922932754,
        lastWeekAverageDeathPerMillionEachDay: 6.057555255504653,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Connecticut - Hartford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1943.435159018526,
        lastWeekAverageDeathPerMillionEachDay: 7.529589685423356,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Connecticut - Litchfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1136.7858350939648,
        lastWeekAverageDeathPerMillionEachDay: 15.051519767794655,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Connecticut - Middlesex": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1403.6297372503632,
        lastWeekAverageDeathPerMillionEachDay: 10.553607046995213,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Connecticut - New Haven": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1667.140485541505,
        lastWeekAverageDeathPerMillionEachDay: 10.863572086235372,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Connecticut - New London": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 795.6079425050717,
        lastWeekAverageDeathPerMillionEachDay: 8.618637156453044,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Connecticut - Tolland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 663.477551237054,
        lastWeekAverageDeathPerMillionEachDay: 10.426075805153705,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Connecticut - Windham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 436.71113699028956,
        lastWeekAverageDeathPerMillionEachDay: 11.009524461940075,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Delaware - Kent": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 862.8986757824167,
        lastWeekAverageDeathPerMillionEachDay: 11.06280353567201,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Delaware - New Castle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 755.25321564269,
        lastWeekAverageDeathPerMillionEachDay: 4.857755957078913,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Delaware - Sussex": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1212.5093393104921,
        lastWeekAverageDeathPerMillionEachDay: 7.928884009575653,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - District of Columbia - District of Columbia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1044.2806153462493,
        lastWeekAverageDeathPerMillionEachDay: 4.453222240282513,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Alachua": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 427.44096668562275,
        lastWeekAverageDeathPerMillionEachDay: 4.778843105801994,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Baker": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1232.4546388223214,
        lastWeekAverageDeathPerMillionEachDay: 9.781386022399374,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Bay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1087.547580206634,
        lastWeekAverageDeathPerMillionEachDay: 4.906229685142709,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Bradford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 780.1141803482144,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Brevard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 832.306102581312,
        lastWeekAverageDeathPerMillionEachDay: 5.695850145979894,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Broward": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 909.4735807142441,
        lastWeekAverageDeathPerMillionEachDay: 2.560454900659471,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Calhoun": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1843.3179723502308,
        lastWeekAverageDeathPerMillionEachDay: 30.384362181597208,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Charlotte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1148.6951458366418,
        lastWeekAverageDeathPerMillionEachDay: 6.049744020206145,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Citrus": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1470.0281309928705,
        lastWeekAverageDeathPerMillionEachDay: 3.8182548856957674,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 779.9244704723332,
        lastWeekAverageDeathPerMillionEachDay: 3.2578298683054854,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Collier": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 823.5862635164275,
        lastWeekAverageDeathPerMillionEachDay: 3.3403679006975433,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Columbia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1562.3692213263396,
        lastWeekAverageDeathPerMillionEachDay: 15.942543074758566,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - DeSoto": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1342.0699455277493,
        lastWeekAverageDeathPerMillionEachDay: 18.7964978365231,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Dixie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 653.7501485795792,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Duval": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 726.6994168654824,
        lastWeekAverageDeathPerMillionEachDay: 1.9390583783356468,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Escambia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1077.5455836338733,
        lastWeekAverageDeathPerMillionEachDay: 4.9366936359735965,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Flagler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 399.71845917223516,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 329.89690721649475,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Gadsden": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1292.1594393342095,
        lastWeekAverageDeathPerMillionEachDay: 3.128715349477505,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Gilchrist": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1076.3104079216446,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Glades": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 796.466584606473,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Gulf": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1906.2981156976323,
        lastWeekAverageDeathPerMillionEachDay: 20.948330941732223,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Hamilton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 831.7161075686166,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Hardee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 742.4731781564392,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Hendry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1094.6646994431492,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Hernando": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1309.8184818481848,
        lastWeekAverageDeathPerMillionEachDay: 7.366808109382367,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Highlands": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1798.137844682313,
        lastWeekAverageDeathPerMillionEachDay: 12.10414405545312,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Hillsborough": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 697.0260223048327,
        lastWeekAverageDeathPerMillionEachDay: 3.882071970508676,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Holmes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1019.523882346944,
        lastWeekAverageDeathPerMillionEachDay: 21.846940336005943,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Indian River": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 950.4574076274205,
        lastWeekAverageDeathPerMillionEachDay: 4.466435186219082,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2369.97457663636,
        lastWeekAverageDeathPerMillionEachDay: 21.54522342396691,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 982.73199494595,
        lastWeekAverageDeathPerMillionEachDay: 20.055754998896937,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Lafayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2374.7328425552123,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 776.317151433599,
        lastWeekAverageDeathPerMillionEachDay: 5.447839659183151,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Lee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 829.2487317944865,
        lastWeekAverageDeathPerMillionEachDay: 4.8201357090669905,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Leon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 582.4607775681069,
        lastWeekAverageDeathPerMillionEachDay: 5.83920579015646,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Levy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 433.7035876924559,
        lastWeekAverageDeathPerMillionEachDay: 6.884183931626284,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Liberty": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1675.8439071103662,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1730.3844697993836,
        lastWeekAverageDeathPerMillionEachDay: 15.449861337494497,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Manatee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1001.8524350717789,
        lastWeekAverageDeathPerMillionEachDay: 2.8340945829470408,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1181.6871319195031,
        lastWeekAverageDeathPerMillionEachDay: 3.907695542061849,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Martin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1186.3354037267081,
        lastWeekAverageDeathPerMillionEachDay: 4.436557231588288,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Miami-Dade": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1496.5365447893587,
        lastWeekAverageDeathPerMillionEachDay: 4.469313692189428,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 444.5761707172496,
        lastWeekAverageDeathPerMillionEachDay: 9.622860838035706,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Nassau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 722.1438645980255,
        lastWeekAverageDeathPerMillionEachDay: 3.2238565383840423,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Okaloosa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1001.2432499122131,
        lastWeekAverageDeathPerMillionEachDay: 17.62513506954471,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Okeechobee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1185.7332574464049,
        lastWeekAverageDeathPerMillionEachDay: 10.163427920969184,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Orange": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 513.1141941021292,
        lastWeekAverageDeathPerMillionEachDay: 3.6907314660692614,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Osceola": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 721.222298809584,
        lastWeekAverageDeathPerMillionEachDay: 1.1405729554184247,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Palm Beach": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1215.283577303126,
        lastWeekAverageDeathPerMillionEachDay: 4.867624475179411,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Pasco": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 611.971903449247,
        lastWeekAverageDeathPerMillionEachDay: 5.415680561497761,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Pinellas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1022.5682977160932,
        lastWeekAverageDeathPerMillionEachDay: 6.739954391021677,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1030.6618449536893,
        lastWeekAverageDeathPerMillionEachDay: 4.730519081829899,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Putnam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 858.8183196682813,
        lastWeekAverageDeathPerMillionEachDay: 3.8340103556619702,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Santa Rosa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 634.7897326829903,
        lastWeekAverageDeathPerMillionEachDay: 3.8753951934248496,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Sarasota": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1076.6769185368262,
        lastWeekAverageDeathPerMillionEachDay: 12.845029006710375,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Seminole": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 640.0664651799603,
        lastWeekAverageDeathPerMillionEachDay: 1.5138752724218552,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - St. Johns": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 408.05223068552783,
        lastWeekAverageDeathPerMillionEachDay: 1.6192548836727292,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - St. Lucie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1160.5345160022782,
        lastWeekAverageDeathPerMillionEachDay: 1.3054381507337212,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Sumter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 868.4488747923274,
        lastWeekAverageDeathPerMillionEachDay: 14.024640214037586,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Suwannee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2093.792917126326,
        lastWeekAverageDeathPerMillionEachDay: 9.648815286296433,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Taylor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1159.0708887755575,
        lastWeekAverageDeathPerMillionEachDay: 13.246524443149227,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4265.931613834744,
        lastWeekAverageDeathPerMillionEachDay: 37.502695506239505,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Volusia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 730.1855828109976,
        lastWeekAverageDeathPerMillionEachDay: 3.6147801129257306,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Wakulla": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 622.4250866949228,
        lastWeekAverageDeathPerMillionEachDay: 12.702552789692302,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Walton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 567.0235314765563,
        lastWeekAverageDeathPerMillionEachDay: 3.8573029352146686,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Florida - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1216.9748361009697,
        lastWeekAverageDeathPerMillionEachDay: 11.216357936414466,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Appling": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2284.3467855977374,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Atkinson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1102.2657685241886,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Bacon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2239.340738086707,
        lastWeekAverageDeathPerMillionEachDay: 12.796232789066897,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Baker": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1974.983541803818,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Baldwin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1626.1973713521943,
        lastWeekAverageDeathPerMillionEachDay: -3.182382331413296,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Banks": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 935.8427784132267,
        lastWeekAverageDeathPerMillionEachDay: 37.13661819100106,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Barrow": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 756.8476693897165,
        lastWeekAverageDeathPerMillionEachDay: 5.148623601290589,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Bartow": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1048.8407061575303,
        lastWeekAverageDeathPerMillionEachDay: 2.6519360459103165,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Ben Hill": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2455.089820359281,
        lastWeekAverageDeathPerMillionEachDay: 8.554319931565438,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Berrien": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1546.6309223075732,
        lastWeekAverageDeathPerMillionEachDay: 7.364909153845587,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Bibb": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1658.4072760986949,
        lastWeekAverageDeathPerMillionEachDay: 5.5964250037076315,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Bleckley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2563.5050104870656,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Brantley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 994.295881521796,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Brooks": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1940.868215048198,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Bryan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 479.4710677063618,
        lastWeekAverageDeathPerMillionEachDay: 3.605045621852344,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Bulloch": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 703.4468897598232,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Burke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 759.5049814591431,
        lastWeekAverageDeathPerMillionEachDay: 6.382394802177672,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Butts": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1884.8251523901188,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Calhoun": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1777.3469058006137,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Camden": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 384.1510262320272,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Candler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3332.4076645376285,
        lastWeekAverageDeathPerMillionEachDay: 13.223839938641381,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 916.7277818521234,
        lastWeekAverageDeathPerMillionEachDay: 8.333888925928393,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Catoosa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 517.9047055341816,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Charlton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1418.7574671445639,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Chatham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 791.210309919497,
        lastWeekAverageDeathPerMillionEachDay: 4.442228814270414,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Chattahoochee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 275.05271843770055,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Chattooga": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1694.2998910807214,
        lastWeekAverageDeathPerMillionEachDay: 17.28877439878287,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Cherokee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 483.0488497640789,
        lastWeekAverageDeathPerMillionEachDay: 3.3123349698108266,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Clarke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 475.3333177486343,
        lastWeekAverageDeathPerMillionEachDay: 3.3395783448381797,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1058.5744530698657,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Clayton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 749.343041716851,
        lastWeekAverageDeathPerMillionEachDay: 3.4216577247344793,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Clinch": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2115.442731943185,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Cobb": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 723.5499729655421,
        lastWeekAverageDeathPerMillionEachDay: 3.3828310424363006,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Coffee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1894.946040256049,
        lastWeekAverageDeathPerMillionEachDay: 3.3012997217004334,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Colquitt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1140.3508771929824,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Columbia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 599.8187781563869,
        lastWeekAverageDeathPerMillionEachDay: 3.6463147608290996,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Cook": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1273.8853503184712,
        lastWeekAverageDeathPerMillionEachDay: 16.543965588551576,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Coweta": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 518.4870950582118,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Crawford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 644.9532408900357,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Crisp": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1653.853030573932,
        lastWeekAverageDeathPerMillionEachDay: 6.385532936578888,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Dade": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 496.4010920824025,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Dawson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 459.62923241918185,
        lastWeekAverageDeathPerMillionEachDay: 5.471776576418831,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - DeKalb": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 661.1378683176675,
        lastWeekAverageDeathPerMillionEachDay: 1.8814395797315524,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Decatur": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1817.9063778215425,
        lastWeekAverageDeathPerMillionEachDay: 5.4104356482783995,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Dodge": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2863.382674108226,
        lastWeekAverageDeathPerMillionEachDay: 20.799389884563386,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Dooly": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1792.3823749066466,
        lastWeekAverageDeathPerMillionEachDay: 10.668942707777658,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Dougherty": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2353.4494519987265,
        lastWeekAverageDeathPerMillionEachDay: 1.624188717735491,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 717.4924663291036,
        lastWeekAverageDeathPerMillionEachDay: 2.9285406788943007,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Early": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3925.417075564278,
        lastWeekAverageDeathPerMillionEachDay: 14.01934669844385,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Echols": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 748.876684972541,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Effingham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 528.8042802040562,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Elbert": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1771.3868917370012,
        lastWeekAverageDeathPerMillionEachDay: 22.32840619836556,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Emanuel": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2207.8954340722426,
        lastWeekAverageDeathPerMillionEachDay: 6.308272668777836,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Evans": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1220.1989862962268,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Fannin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1565.6025660607913,
        lastWeekAverageDeathPerMillionEachDay: 16.365183617360188,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Fayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 603.0361559503938,
        lastWeekAverageDeathPerMillionEachDay: 3.74556618602729,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Floyd": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1126.9264350545188,
        lastWeekAverageDeathPerMillionEachDay: 13.053201950438444,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Forsyth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 302.96578943058813,
        lastWeekAverageDeathPerMillionEachDay: 3.5092562482307503,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1070.7096663668676,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Fulton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 719.0275364048812,
        lastWeekAverageDeathPerMillionEachDay: 2.2826270996980353,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Gilmer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1083.8726130893558,
        lastWeekAverageDeathPerMillionEachDay: 4.554086609619142,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Glascock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1009.7610232245037,
        lastWeekAverageDeathPerMillionEachDay: 48.08385824878589,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Glynn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1524.1757726398723,
        lastWeekAverageDeathPerMillionEachDay: 3.349836862944774,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Gordon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1035.143108534755,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Grady": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1299.0703527787925,
        lastWeekAverageDeathPerMillionEachDay: 5.799421217762466,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1528.0506439642,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Gwinnett": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 614.1522029372497,
        lastWeekAverageDeathPerMillionEachDay: 1.6784283806980738,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Habersham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1875.2206141899044,
        lastWeekAverageDeathPerMillionEachDay: 3.1516312843527805,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Hall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1051.648152767791,
        lastWeekAverageDeathPerMillionEachDay: 4.192617220336708,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Hancock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 5557.526309566039,
        lastWeekAverageDeathPerMillionEachDay: 16.892177232723522,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Haralson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1275.5102040816328,
        lastWeekAverageDeathPerMillionEachDay: 33.56605800214823,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Harris": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 823.0219094108297,
        lastWeekAverageDeathPerMillionEachDay: 4.0542951202503925,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Hart": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1182.9803472619728,
        lastWeekAverageDeathPerMillionEachDay: 5.451522337612778,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Heard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 754.8435796359978,
        lastWeekAverageDeathPerMillionEachDay: 11.981644121206314,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Henry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 639.4924987529896,
        lastWeekAverageDeathPerMillionEachDay: 2.4361619000113888,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Houston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 728.479757764644,
        lastWeekAverageDeathPerMillionEachDay: 1.8098875969307924,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Irwin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1699.235344095157,
        lastWeekAverageDeathPerMillionEachDay: 15.171744143706759,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 918.0974827685435,
        lastWeekAverageDeathPerMillionEachDay: 7.830255716576064,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Jasper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 492.2990365004571,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Jeff Davis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2249.421104862719,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2603.8276266111184,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Jenkins": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3918.8566159520524,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3318.46935600954,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Jones": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 835.2183748042457,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Lamar": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1572.5743041358703,
        lastWeekAverageDeathPerMillionEachDay: 7.4884490673136686,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Lanier": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 959.4166746618056,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Laurens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2523.8716190636437,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Lee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1100.2934115764203,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Liberty": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 504.59835598600137,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1388.7135462694105,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Long": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 255.63679124699632,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Lowndes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1124.3036982777712,
        lastWeekAverageDeathPerMillionEachDay: 1.216778894240012,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Lumpkin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 595.0609937518595,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Macon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1235.8075229782962,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 736.2784471218207,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1196.3153487259244,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - McDuffie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1126.1261261261263,
        lastWeekAverageDeathPerMillionEachDay: 6.703131703131704,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - McIntosh": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 625.9563221588537,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Meriwether": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 897.6236594699296,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Miller": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 524.6589716684156,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Mitchell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2378.447605543613,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2211.9080426426867,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1090.274749236808,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Morgan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 415.0238638721726,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Murray": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 972.6656025538707,
        lastWeekAverageDeathPerMillionEachDay: 24.940143655227455,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Muscogee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1036.9363893159793,
        lastWeekAverageDeathPerMillionEachDay: 0.7297230044447427,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Newton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1029.1380297823596,
        lastWeekAverageDeathPerMillionEachDay: 3.8352970054000974,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Oconee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 918.5700099304865,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Oglethorpe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 917.4913166000392,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Out of GA": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 0,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Paulding": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 581.0265197104354,
        lastWeekAverageDeathPerMillionEachDay: 5.928842037861586,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Peach": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1089.0873448050534,
        lastWeekAverageDeathPerMillionEachDay: 5.186130213357397,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Pickens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 552.2997146451474,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Pierce": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1643.9763678397123,
        lastWeekAverageDeathPerMillionEachDay: 7.339180213570144,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Pike": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 896.5299019090812,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 985.6147185131297,
        lastWeekAverageDeathPerMillionEachDay: 13.409724061403125,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Pulaski": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2693.723623956182,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Putnam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1401.5100140150998,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Quitman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 434.97172683775557,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Rabun": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 875.2990605123417,
        lastWeekAverageDeathPerMillionEachDay: 8.336181528688968,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Randolph": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4573.620537031573,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Richmond": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1209.7690081869268,
        lastWeekAverageDeathPerMillionEachDay: 7.759451364464253,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Rockdale": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 803.1156486534061,
        lastWeekAverageDeathPerMillionEachDay: 12.573239117861542,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Schley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 380.4451207913259,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Screven": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1074.0369468709725,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Seminole": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1483.3127317676144,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Spalding": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1379.2483096712292,
        lastWeekAverageDeathPerMillionEachDay: 4.2833798436994694,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Stephens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1928.6403085824493,
        lastWeekAverageDeathPerMillionEachDay: 5.5104008816641405,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Stewart": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2567.5879776468814,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Sumter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2472.564693131012,
        lastWeekAverageDeathPerMillionEachDay: 4.838678460138967,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Talbot": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1452.7845036319611,
        lastWeekAverageDeathPerMillionEachDay: 23.060071486221602,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Tattnall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1384.1651506762635,
        lastWeekAverageDeathPerMillionEachDay: 5.649653676229647,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Taylor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1620.9476309226934,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Telfair": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2206.809583858764,
        lastWeekAverageDeathPerMillionEachDay: 18.014772113132768,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Terrell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3985.4647755245574,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Thomas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1867.2245843738049,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Tift": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1968.3102056884165,
        lastWeekAverageDeathPerMillionEachDay: 3.5148396530150294,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Toombs": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2609.019754006709,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Towns": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2907.701254465398,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Treutlen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2318.5045645558616,
        lastWeekAverageDeathPerMillionEachDay: 20.700933612105906,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Troup": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1816.3096021280855,
        lastWeekAverageDeathPerMillionEachDay: 2.043092915779624,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Turner": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3130.8703819661864,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Twiggs": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2339.9014778325122,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1672.7183713434783,
        lastWeekAverageDeathPerMillionEachDay: 5.82828700816543,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Upson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3077.5075987841956,
        lastWeekAverageDeathPerMillionEachDay: 5.427702996092055,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Walker": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 802.7407863992775,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Walton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 930.3013965092555,
        lastWeekAverageDeathPerMillionEachDay: 4.530688619363257,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Ware": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2266.7487546874127,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1522.6494099733534,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1963.2865416707568,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1637.3174725164567,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Webster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 767.1653241273494,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Wheeler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2418.841502227881,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - White": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1071.4981492304696,
        lastWeekAverageDeathPerMillionEachDay: 23.19260063269415,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Whitfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1060.9014795274688,
        lastWeekAverageDeathPerMillionEachDay: 15.019197264867639,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Wilcox": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3821.656050955414,
        lastWeekAverageDeathPerMillionEachDay: 66.1758623542063,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Wilkes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1022.8086325048582,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Wilkinson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2010.2747375474648,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Georgia - Worth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1975.6013236528868,
        lastWeekAverageDeathPerMillionEachDay: 7.055719013046024,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Grand Princess": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 0,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-08-04 02:27:56",
      },
    ],
  },
  "US - Guam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 724.5979699078723,
        lastWeekAverageDeathPerMillionEachDay: 3.479462040373937,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Hawaii - Hawaii": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 218.34819589803143,
        lastWeekAverageDeathPerMillionEachDay: 0.7089227139546475,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Hawaii - Honolulu": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 223.69000259603536,
        lastWeekAverageDeathPerMillionEachDay: 1.0261009293396117,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Hawaii - Kauai": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 13.832597900211637,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Hawaii - Maui": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 101.5428540709725,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Hawaii - Out of HI": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 0,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Ada": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 645.7815514123099,
        lastWeekAverageDeathPerMillionEachDay: 7.7125954693247865,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 698.6492780624127,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Bannock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 580.8126822157434,
        lastWeekAverageDeathPerMillionEachDay: 11.388483965014576,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Bear Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 326.53061224489795,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Benewah": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 430.2000430200042,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Bingham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 811.7750101471876,
        lastWeekAverageDeathPerMillionEachDay: 15.25892876216518,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Blaine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 521.26319447461,
        lastWeekAverageDeathPerMillionEachDay: 18.6165426598075,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Boise": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 255.39522410930917,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Bonner": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 262.3581626183344,
        lastWeekAverageDeathPerMillionEachDay: 6.246622919484152,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Bonneville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 856.6965110614638,
        lastWeekAverageDeathPerMillionEachDay: 15.598115747617689,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Boundary": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 489.9959167006941,
        lastWeekAverageDeathPerMillionEachDay: 11.666569445254622,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Canyon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 857.0844336934248,
        lastWeekAverageDeathPerMillionEachDay: 10.56594298244251,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Caribou": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1537.3864430468207,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Cassia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 790.6783187682064,
        lastWeekAverageDeathPerMillionEachDay: -5.944949765174484,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Clearwater": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 571.0370031978073,
        lastWeekAverageDeathPerMillionEachDay: 32.63068589701756,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Custer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 463.4994206257241,
        lastWeekAverageDeathPerMillionEachDay: 33.107101473266006,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Elmore": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 327.14187052451746,
        lastWeekAverageDeathPerMillionEachDay: 10.385456207127538,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 576.5350245027386,
        lastWeekAverageDeathPerMillionEachDay: 10.295268294691759,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Fremont": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 916.1004656844034,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Gem": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 938.6042402826855,
        lastWeekAverageDeathPerMillionEachDay: 39.43715295305402,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Gooding": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1119.9683773634629,
        lastWeekAverageDeathPerMillionEachDay: 28.23449690832259,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Idaho": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 899.9820003599929,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 468.6819992634997,
        lastWeekAverageDeathPerMillionEachDay: 14.347408140719377,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Jerome": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 614.4519088972637,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Kootenai": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 615.581452892931,
        lastWeekAverageDeathPerMillionEachDay: 1.7243177952182942,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Latah": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 74.79804527775006,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Lemhi": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1121.2158963498193,
        lastWeekAverageDeathPerMillionEachDay: 17.797077719838402,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Lewis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1302.7618551328817,
        lastWeekAverageDeathPerMillionEachDay: 37.22176728951091,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1490.8684308609766,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 250.58260455559176,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Minidoka": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1045.6770759066499,
        lastWeekAverageDeathPerMillionEachDay: 6.790110882510713,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Nez Perce": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1014.650564244704,
        lastWeekAverageDeathPerMillionEachDay: 3.5353678196679583,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Oneida": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 220.70183182520412,
        lastWeekAverageDeathPerMillionEachDay: 31.528833117886304,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Owyhee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1522.4562293834051,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Payette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 918.542023297566,
        lastWeekAverageDeathPerMillionEachDay: 11.929117185682674,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Power": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 520.7655253222235,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Shoshone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1785.437043937277,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Teton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 164.71750947125676,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Twin Falls": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1266.1433274246644,
        lastWeekAverageDeathPerMillionEachDay: 8.221709918341977,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Valley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 175.56179775280899,
        lastWeekAverageDeathPerMillionEachDay: 12.540128410914928,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Idaho - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1373.3568765940752,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1054.481546572935,
        lastWeekAverageDeathPerMillionEachDay: 13.099149646868758,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Alexander": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 867.9048776254125,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Bond": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 913.1864117861926,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Boone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1176.602420439265,
        lastWeekAverageDeathPerMillionEachDay: 26.68032699408764,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Brown": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1368.1970203709334,
        lastWeekAverageDeathPerMillionEachDay: 21.717413021760848,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Bureau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1961.505455437048,
        lastWeekAverageDeathPerMillionEachDay: 8.756720783201107,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Calhoun": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 211.0149820637265,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2027.2631946871722,
        lastWeekAverageDeathPerMillionEachDay: 9.986518200429419,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Cass": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2305.0959084547626,
        lastWeekAverageDeathPerMillionEachDay: 23.521386820966963,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Champaign": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 376.74842266404056,
        lastWeekAverageDeathPerMillionEachDay: 3.406405268210132,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Christian": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1764.4873699851412,
        lastWeekAverageDeathPerMillionEachDay: 22.111370551192245,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1489.5408328476133,
        lastWeekAverageDeathPerMillionEachDay: 9.251806415202568,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2503.0339805825242,
        lastWeekAverageDeathPerMillionEachDay: 65.0138696255201,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Clinton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2049.9440924338423,
        lastWeekAverageDeathPerMillionEachDay: 19.016178965063474,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Coles": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1363.070662373323,
        lastWeekAverageDeathPerMillionEachDay: 5.644184937363657,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Cook": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1499.7379730198613,
        lastWeekAverageDeathPerMillionEachDay: 10.651390501583686,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Crawford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1392.8322708523062,
        lastWeekAverageDeathPerMillionEachDay: 38.26462282561281,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Cumberland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2136.3551922719676,
        lastWeekAverageDeathPerMillionEachDay: 26.538573817043073,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - De Witt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1470.776314106663,
        lastWeekAverageDeathPerMillionEachDay: 27.405769828074465,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - DeKalb": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 648.2549548604821,
        lastWeekAverageDeathPerMillionEachDay: 10.895041258159363,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1232.9822758797843,
        lastWeekAverageDeathPerMillionEachDay: 22.017540640710433,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - DuPage": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1026.0899903675395,
        lastWeekAverageDeathPerMillionEachDay: 6.655886194871655,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Edgar": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1981.2365246780496,
        lastWeekAverageDeathPerMillionEachDay: 83.24523212932982,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Edwards": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 938.2329945269743,
        lastWeekAverageDeathPerMillionEachDay: 44.677761644141626,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Effingham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1440.837450011762,
        lastWeekAverageDeathPerMillionEachDay: 25.20415364452062,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Fayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2155.9805024371954,
        lastWeekAverageDeathPerMillionEachDay: 40.17354973485457,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Ford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2854.718000154309,
        lastWeekAverageDeathPerMillionEachDay: 99.19869498605708,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1117.7831500688865,
        lastWeekAverageDeathPerMillionEachDay: 18.567826413104427,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Fulton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 757.13453698311,
        lastWeekAverageDeathPerMillionEachDay: 20.800399367667858,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Gallatin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 828.5004142502073,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3084.277893438199,
        lastWeekAverageDeathPerMillionEachDay: 22.03055638170142,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Grundy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 744.3099463313354,
        lastWeekAverageDeathPerMillionEachDay: 13.990788464874726,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Hamilton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 985.7072449482504,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Hancock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1524.734583239214,
        lastWeekAverageDeathPerMillionEachDay: 56.47165123108201,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Hardin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1308.557969118032,
        lastWeekAverageDeathPerMillionEachDay: 37.38737054622948,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Henderson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 902.7986758952754,
        lastWeekAverageDeathPerMillionEachDay: 21.495206568935128,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Henry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 940.4452803958048,
        lastWeekAverageDeathPerMillionEachDay: 23.36510013405726,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Iroquois": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1659.65921664085,
        lastWeekAverageDeathPerMillionEachDay: 15.80627825372238,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 881.0572687224667,
        lastWeekAverageDeathPerMillionEachDay: 15.103838892385143,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Jasper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1456.81581685744,
        lastWeekAverageDeathPerMillionEachDay: 14.86546751895347,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1910.6251990234584,
        lastWeekAverageDeathPerMillionEachDay: 22.7455380836126,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Jersey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1561.5670784917097,
        lastWeekAverageDeathPerMillionEachDay: 39.36723727290024,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Jo Daviess": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1130.2095596891922,
        lastWeekAverageDeathPerMillionEachDay: 6.72743785529281,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1046.9517596843039,
        lastWeekAverageDeathPerMillionEachDay: 11.504964392135207,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Kane": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1066.86100566676,
        lastWeekAverageDeathPerMillionEachDay: 8.318081281607032,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Kankakee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1356.2469279641732,
        lastWeekAverageDeathPerMillionEachDay: 20.805322001367948,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Kendall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 527.1726490425614,
        lastWeekAverageDeathPerMillionEachDay: 3.322516695646395,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Knox": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2173.081953359223,
        lastWeekAverageDeathPerMillionEachDay: 28.744470282529402,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - LaSalle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1840.4512786535256,
        lastWeekAverageDeathPerMillionEachDay: 30.23598529216506,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1082.501238272305,
        lastWeekAverageDeathPerMillionEachDay: 7.383487036340088,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Lawrence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1530.8075009567547,
        lastWeekAverageDeathPerMillionEachDay: 27.335848231370615,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Lee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1818.3951196621301,
        lastWeekAverageDeathPerMillionEachDay: 33.51880404907152,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Livingston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1542.8635547576303,
        lastWeekAverageDeathPerMillionEachDay: 24.04462682739164,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Logan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1467.607799287162,
        lastWeekAverageDeathPerMillionEachDay: 14.975589788644507,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Macon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1538.3284138872598,
        lastWeekAverageDeathPerMillionEachDay: 2.747015024798678,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Macoupin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1268.753060588523,
        lastWeekAverageDeathPerMillionEachDay: 34.9781545525658,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1349.9844086307737,
        lastWeekAverageDeathPerMillionEachDay: 14.124585361931636,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2580.2983469963715,
        lastWeekAverageDeathPerMillionEachDay: 34.557567147272835,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Marshall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 699.4229760447629,
        lastWeekAverageDeathPerMillionEachDay: 12.489696000799336,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Mason": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2619.9565835766157,
        lastWeekAverageDeathPerMillionEachDay: 53.468501705645224,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Massac": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1524.8329944815569,
        lastWeekAverageDeathPerMillionEachDay: 72.61109497531223,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - McDonough": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1549.7607977899063,
        lastWeekAverageDeathPerMillionEachDay: 4.81292173226679,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - McHenry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 630.3326466823057,
        lastWeekAverageDeathPerMillionEachDay: 5.105787270622506,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - McLean": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 524.7293271220929,
        lastWeekAverageDeathPerMillionEachDay: 11.660651713824285,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Menard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 491.96457855034436,
        lastWeekAverageDeathPerMillionEachDay: 23.42688469287354,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Mercer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1489.9267992485584,
        lastWeekAverageDeathPerMillionEachDay: 27.762611166122205,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1616.7681958599185,
        lastWeekAverageDeathPerMillionEachDay: 8.24881732581591,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 774.2662067994651,
        lastWeekAverageDeathPerMillionEachDay: 15.083107924664903,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Morgan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2139.164537405669,
        lastWeekAverageDeathPerMillionEachDay: 16.977496328616418,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Moultrie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1379.215226536101,
        lastWeekAverageDeathPerMillionEachDay: 29.554611997202162,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Ogle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1204.5100013822248,
        lastWeekAverageDeathPerMillionEachDay: 22.566932110205613,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Peoria": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 987.8389766657923,
        lastWeekAverageDeathPerMillionEachDay: 9.56744771589145,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Perry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2199.2732836106325,
        lastWeekAverageDeathPerMillionEachDay: 47.81028877414418,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Piatt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 367.10719530102796,
        lastWeekAverageDeathPerMillionEachDay: 8.740647507167331,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Pike": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2249.212775528565,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Pope": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 239.40627244433804,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Pulaski": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 374.88284910965325,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Randolph": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1069.7879302749984,
        lastWeekAverageDeathPerMillionEachDay: 17.979629080252074,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Richland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1804.9377941081673,
        lastWeekAverageDeathPerMillionEachDay: 55.253197778821445,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Rock Island": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1550.6170751133006,
        lastWeekAverageDeathPerMillionEachDay: 18.124095683142478,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Saline": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1489.9323145034266,
        lastWeekAverageDeathPerMillionEachDay: 18.244069157184814,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Sangamon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 862.9900550669845,
        lastWeekAverageDeathPerMillionEachDay: 18.345877020981813,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Schuyler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1477.5413711583924,
        lastWeekAverageDeathPerMillionEachDay: 126.64640324214791,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Scott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 201.97939810139368,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Shelby": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1525.3767218267542,
        lastWeekAverageDeathPerMillionEachDay: 13.2067248643009,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - St. Clair": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1251.5114407399708,
        lastWeekAverageDeathPerMillionEachDay: 8.801838704105288,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Stark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2620.7412953949824,
        lastWeekAverageDeathPerMillionEachDay: 187.19580681392733,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Stephenson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1168.5918468245764,
        lastWeekAverageDeathPerMillionEachDay: 22.472920131241853,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Tazewell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1229.1070764701865,
        lastWeekAverageDeathPerMillionEachDay: 17.34189878617547,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1801.4772113132765,
        lastWeekAverageDeathPerMillionEachDay: 8.5784629110156,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Vermilion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1069.1940125135297,
        lastWeekAverageDeathPerMillionEachDay: 11.314222354640528,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Wabash": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 868.0555555555555,
        lastWeekAverageDeathPerMillionEachDay: 12.400793650793648,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1781.0496319164092,
        lastWeekAverageDeathPerMillionEachDay: 16.962377446822945,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1080.146899978397,
        lastWeekAverageDeathPerMillionEachDay: 51.43556666563795,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2405.18038852914,
        lastWeekAverageDeathPerMillionEachDay: 8.810184573366813,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - White": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1403.5606116569402,
        lastWeekAverageDeathPerMillionEachDay: 63.318523834147676,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Whiteside": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2791.11916628908,
        lastWeekAverageDeathPerMillionEachDay: 10.356657388827754,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Will": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1039.4604071268186,
        lastWeekAverageDeathPerMillionEachDay: 9.513565206492967,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Williamson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1576.647596738592,
        lastWeekAverageDeathPerMillionEachDay: 10.725493855364572,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Winnebago": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1199.6942372209564,
        lastWeekAverageDeathPerMillionEachDay: 9.100082709640631,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Illinois - Woodford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1118.0737928703295,
        lastWeekAverageDeathPerMillionEachDay: 26.001716113263477,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 838.5275456298739,
        lastWeekAverageDeathPerMillionEachDay: 7.985976625046417,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Allen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1123.1245007237035,
        lastWeekAverageDeathPerMillionEachDay: 12.428943166962513,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Bartholomew": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 919.0847348380858,
        lastWeekAverageDeathPerMillionEachDay: 11.936165387507607,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Benton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 571.5592135345221,
        lastWeekAverageDeathPerMillionEachDay: 32.66052648768698,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Blackford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1786.0180302772578,
        lastWeekAverageDeathPerMillionEachDay: 12.14978251889291,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Boone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 854.915024394558,
        lastWeekAverageDeathPerMillionEachDay: 4.211404061056935,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Brown": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 530.082162735224,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 691.1191193167793,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Cass": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 955.1858632492239,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 870.653074335176,
        lastWeekAverageDeathPerMillionEachDay: 13.283195308858442,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1143.9466158245946,
        lastWeekAverageDeathPerMillionEachDay: 16.342094511779923,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Clinton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 925.9545047686656,
        lastWeekAverageDeathPerMillionEachDay: 8.818614331130147,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Crawford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 661.8133686300464,
        lastWeekAverageDeathPerMillionEachDay: 13.506395278164211,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Daviess": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1888.9988306197715,
        lastWeekAverageDeathPerMillionEachDay: 12.85033218108688,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - DeKalb": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1150.0862564692354,
        lastWeekAverageDeathPerMillionEachDay: 32.85960732769244,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Dearborn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 788.5478587892756,
        lastWeekAverageDeathPerMillionEachDay: 5.776907390397623,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Decatur": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2296.7732218833544,
        lastWeekAverageDeathPerMillionEachDay: 59.16746004851733,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Delaware": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1068.9096245673984,
        lastWeekAverageDeathPerMillionEachDay: 5.006602456990156,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Dubois": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 865.780606514414,
        lastWeekAverageDeathPerMillionEachDay: 10.02834679360325,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Elkhart": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1424.825894999055,
        lastWeekAverageDeathPerMillionEachDay: 11.077363615153002,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Fayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1818.0244134706952,
        lastWeekAverageDeathPerMillionEachDay: 6.183756508403724,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Floyd": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1095.2344565854157,
        lastWeekAverageDeathPerMillionEachDay: 7.277305359371532,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Fountain": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1162.3638810718219,
        lastWeekAverageDeathPerMillionEachDay: 34.95831221268637,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1230.3365849371648,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Fulton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1151.496946029839,
        lastWeekAverageDeathPerMillionEachDay: 7.152154944284713,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Gibson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1455.7770581419531,
        lastWeekAverageDeathPerMillionEachDay: 16.976991931684584,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1261.9927321382415,
        lastWeekAverageDeathPerMillionEachDay: 45.6141951375268,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1722.9496898690559,
        lastWeekAverageDeathPerMillionEachDay: 4.4751939996598855,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Hamilton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 695.2436459168489,
        lastWeekAverageDeathPerMillionEachDay: 10.143372341643994,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Hancock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 985.057824173575,
        lastWeekAverageDeathPerMillionEachDay: 10.965393219000834,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Harrison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 715.784277428113,
        lastWeekAverageDeathPerMillionEachDay: 3.526030923291197,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Hendricks": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1127.3493784899395,
        lastWeekAverageDeathPerMillionEachDay: 7.549214588102274,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Henry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1000.5836738097223,
        lastWeekAverageDeathPerMillionEachDay: 5.9558552012483466,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Howard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1235.704593913549,
        lastWeekAverageDeathPerMillionEachDay: 24.229501841442136,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Huntington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 684.5564074479739,
        lastWeekAverageDeathPerMillionEachDay: 27.382256297918957,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 926.9516854694672,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Jasper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 774.6856563971157,
        lastWeekAverageDeathPerMillionEachDay: 8.513029191177095,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Jay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 978.6651008025054,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 835.7063266064132,
        lastWeekAverageDeathPerMillionEachDay: 17.686906383204512,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Jennings": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 973.4991887506759,
        lastWeekAverageDeathPerMillionEachDay: 20.60315743387674,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1308.743290319725,
        lastWeekAverageDeathPerMillionEachDay: 10.838453750059834,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Knox": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 929.1140624146036,
        lastWeekAverageDeathPerMillionEachDay: 7.807681196761375,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Kosciusko": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 805.4772452678212,
        lastWeekAverageDeathPerMillionEachDay: 12.585581957309707,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - LaGrange": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1110.7184328772655,
        lastWeekAverageDeathPerMillionEachDay: 14.42491471269176,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - LaPorte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1110.221316249272,
        lastWeekAverageDeathPerMillionEachDay: 7.800149762875447,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1143.1678726572782,
        lastWeekAverageDeathPerMillionEachDay: 8.239047730863266,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Lawrence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1256.3367864227464,
        lastWeekAverageDeathPerMillionEachDay: 6.297427500865896,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1211.7095910287185,
        lastWeekAverageDeathPerMillionEachDay: 18.743460461772713,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1076.1137985158337,
        lastWeekAverageDeathPerMillionEachDay: 11.255800810239935,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Marshall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1405.162350296165,
        lastWeekAverageDeathPerMillionEachDay: 12.353075606999251,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Martin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1072.6474890297418,
        lastWeekAverageDeathPerMillionEachDay: 41.79146061154838,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Miami": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 675.7517738484063,
        lastWeekAverageDeathPerMillionEachDay: 8.044663974385788,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 552.4452439180493,
        lastWeekAverageDeathPerMillionEachDay: 13.474274241903641,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1017.26746309145,
        lastWeekAverageDeathPerMillionEachDay: 18.631272217792127,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Morgan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 978.8761367021806,
        lastWeekAverageDeathPerMillionEachDay: 28.373221353686393,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Newton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1287.1853546910754,
        lastWeekAverageDeathPerMillionEachDay: 10.215756783262503,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Noble": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1110.0871313672924,
        lastWeekAverageDeathPerMillionEachDay: 5.984297204136347,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Ohio": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1191.4893617021276,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Orange": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1476.127455970681,
        lastWeekAverageDeathPerMillionEachDay: 7.271563822515669,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Owen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1057.743160728881,
        lastWeekAverageDeathPerMillionEachDay: 48.079234578585506,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Parke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 413.29633347109865,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Perry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1095.5188064061767,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Pike": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1856.4855920574696,
        lastWeekAverageDeathPerMillionEachDay: 11.530966410294843,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Porter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 733.6154329211395,
        lastWeekAverageDeathPerMillionEachDay: 9.222594013865754,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Posey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1022.5351004837377,
        lastWeekAverageDeathPerMillionEachDay: 22.4732989117305,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Pulaski": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2347.607868533959,
        lastWeekAverageDeathPerMillionEachDay: 57.82285390477732,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Putnam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 904.8328720459868,
        lastWeekAverageDeathPerMillionEachDay: 3.801818790109188,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Randolph": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1256.8416784917902,
        lastWeekAverageDeathPerMillionEachDay: 17.375691407720602,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Ripley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1306.312667702302,
        lastWeekAverageDeathPerMillionEachDay: 25.21839126838421,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Rush": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 603.0999336590072,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Scott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1382.3147488794873,
        lastWeekAverageDeathPerMillionEachDay: 17.952139595837497,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Shelby": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1520.2664937736145,
        lastWeekAverageDeathPerMillionEachDay: 3.1938371717933074,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Spencer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 493.1696010257929,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - St. Joseph": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1111.004833974675,
        lastWeekAverageDeathPerMillionEachDay: 8.93428674435642,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Starke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1478.5823005001087,
        lastWeekAverageDeathPerMillionEachDay: 31.062653371851024,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Steuben": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 520.321443024802,
        lastWeekAverageDeathPerMillionEachDay: 8.259070524203207,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Sullivan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1064.39595529537,
        lastWeekAverageDeathPerMillionEachDay: 6.911662047372531,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Switzerland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 465.0730164635848,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Tippecanoe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 332.0867308360411,
        lastWeekAverageDeathPerMillionEachDay: 10.947914203385972,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Tipton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1914.4441510430418,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 283.5270768358378,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Vanderburgh": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 997.5144804933561,
        lastWeekAverageDeathPerMillionEachDay: 18.895301919369018,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Vermillion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1548.5869144405729,
        lastWeekAverageDeathPerMillionEachDay: 46.08889626311229,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Vigo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1279.9192810030079,
        lastWeekAverageDeathPerMillionEachDay: 16.015674006294155,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Wabash": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1193.7024132146084,
        lastWeekAverageDeathPerMillionEachDay: 36.87111701048983,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 846.9449485783424,
        lastWeekAverageDeathPerMillionEachDay: 17.28459078731311,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Warrick": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1349.2491825137306,
        lastWeekAverageDeathPerMillionEachDay: 15.873519794279185,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 428.02111570837496,
        lastWeekAverageDeathPerMillionEachDay: 5.095489472718749,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1942.808572642827,
        lastWeekAverageDeathPerMillionEachDay: 23.851444530213275,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Wells": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1554.990104608425,
        lastWeekAverageDeathPerMillionEachDay: 10.097338341613149,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - White": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1037.258318811717,
        lastWeekAverageDeathPerMillionEachDay: 5.927190393209811,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Indiana - Whitley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 500.5299729124957,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Adair": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2376.9574944071587,
        lastWeekAverageDeathPerMillionEachDay: 19.974432726110578,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 555.247084952804,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Allamakee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1388.1785636004968,
        lastWeekAverageDeathPerMillionEachDay: 10.437432809026292,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Appanoose": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2736.1982938998876,
        lastWeekAverageDeathPerMillionEachDay: 11.496631486974318,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Audubon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1273.6535662299855,
        lastWeekAverageDeathPerMillionEachDay: 25.992929923060927,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Benton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1325.7945018522128,
        lastWeekAverageDeathPerMillionEachDay: 33.423390802996956,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Black Hawk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1577.407260645594,
        lastWeekAverageDeathPerMillionEachDay: 25.038210486438,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Boone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 571.7770831745064,
        lastWeekAverageDeathPerMillionEachDay: 5.445496030233394,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Bremer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1596.0418162955868,
        lastWeekAverageDeathPerMillionEachDay: 45.60119475130248,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Buchanan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 755.6080283353012,
        lastWeekAverageDeathPerMillionEachDay: 6.74650025299376,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Buena Vista": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1121.3047910295618,
        lastWeekAverageDeathPerMillionEachDay: 29.1247997670016,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Butler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1108.1099799155065,
        lastWeekAverageDeathPerMillionEachDay: 39.57535642555381,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Calhoun": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 930.9060819197355,
        lastWeekAverageDeathPerMillionEachDay: 14.776287014598974,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1338.953632531614,
        lastWeekAverageDeathPerMillionEachDay: 35.42205377067762,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Cass": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2960.4238080398877,
        lastWeekAverageDeathPerMillionEachDay: 33.388238436540085,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Cedar": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 912.6536747731786,
        lastWeekAverageDeathPerMillionEachDay: 15.338717223078632,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Cerro Gordo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1319.199057714959,
        lastWeekAverageDeathPerMillionEachDay: 23.557126030624268,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Cherokee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1602.136181575434,
        lastWeekAverageDeathPerMillionEachDay: 50.8614660817598,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Chickasaw": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 838.0122349786308,
        lastWeekAverageDeathPerMillionEachDay: 11.971603356837582,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Clarke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 638.637573177222,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 686.8131868131868,
        lastWeekAverageDeathPerMillionEachDay: 8.919651776794634,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Clayton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2051.3989401105478,
        lastWeekAverageDeathPerMillionEachDay: 97.68566381478799,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Clinton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1119.9896616338926,
        lastWeekAverageDeathPerMillionEachDay: 12.307578699273545,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Crawford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1010.7015457788347,
        lastWeekAverageDeathPerMillionEachDay: 8.493290300662474,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Dallas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 663.4350957165634,
        lastWeekAverageDeathPerMillionEachDay: 3.0573045885555916,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Davis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2000.0000000000002,
        lastWeekAverageDeathPerMillionEachDay: 95.23809523809524,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Decatur": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 381.1944091486658,
        lastWeekAverageDeathPerMillionEachDay: 18.152114721365038,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Delaware": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1645.9937687378756,
        lastWeekAverageDeathPerMillionEachDay: 16.795854783039548,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Des Moines": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 795.5449482895783,
        lastWeekAverageDeathPerMillionEachDay: 18.33052876243268,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Dickinson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 753.2738440143703,
        lastWeekAverageDeathPerMillionEachDay: 8.277734549608464,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Dubuque": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1366.751960210048,
        lastWeekAverageDeathPerMillionEachDay: 14.680472182707284,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Emmet": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2497.8279756733277,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Fayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 763.3587786259542,
        lastWeekAverageDeathPerMillionEachDay: 7.270083605961468,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Floyd": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1981.8437539956528,
        lastWeekAverageDeathPerMillionEachDay: 73.0633642026047,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1688.1827209533267,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Fremont": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 718.3908045977012,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 787.5787578757876,
        lastWeekAverageDeathPerMillionEachDay: 16.07303587501607,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Grundy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1144.5389143230868,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Guthrie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2058.190663298718,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Hamilton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1421.5122182359714,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Hancock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2163.687676387582,
        lastWeekAverageDeathPerMillionEachDay: 26.8781077812122,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Hardin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1424.6705449364833,
        lastWeekAverageDeathPerMillionEachDay: 33.92072726039246,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Harrison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3630.151612214392,
        lastWeekAverageDeathPerMillionEachDay: 20.33698382192937,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Henry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1052.4205673048011,
        lastWeekAverageDeathPerMillionEachDay: 14.31864717421498,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Howard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1637.9122079056563,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Humboldt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1883.239171374764,
        lastWeekAverageDeathPerMillionEachDay: 89.67805577975066,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Ida": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3061.224489795918,
        lastWeekAverageDeathPerMillionEachDay: 83.29862557267803,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Iowa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1173.9990113692536,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1491.8462883893205,
        lastWeekAverageDeathPerMillionEachDay: 44.093978967172035,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Jasper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1344.6282102998523,
        lastWeekAverageDeathPerMillionEachDay: 7.683589773142012,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1093.1948619841487,
        lastWeekAverageDeathPerMillionEachDay: 62.46827782766564,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 297.7371973005161,
        lastWeekAverageDeathPerMillionEachDay: 2.8355923552430102,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Jones": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1837.4353271118416,
        lastWeekAverageDeathPerMillionEachDay: 41.44590963410169,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Keokuk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1854.3821979308998,
        lastWeekAverageDeathPerMillionEachDay: 41.8281698781406,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Kossuth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1687.7067440761493,
        lastWeekAverageDeathPerMillionEachDay: 28.932115612733988,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Lee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 683.3645304097215,
        lastWeekAverageDeathPerMillionEachDay: 12.733500566640773,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Linn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1018.940830855822,
        lastWeekAverageDeathPerMillionEachDay: 10.082284040626563,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Louisa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2084.277299501586,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Lucas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 813.953488372093,
        lastWeekAverageDeathPerMillionEachDay: 16.611295681063122,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Lyon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1786.473840918758,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 489.6560166483046,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Mahaska": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1448.2914686580675,
        lastWeekAverageDeathPerMillionEachDay: 6.4655869136520865,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1202.8989865576038,
        lastWeekAverageDeathPerMillionEachDay: 34.36854247307439,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Marshall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1346.2368868907008,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Mills": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 860.4143225891853,
        lastWeekAverageDeathPerMillionEachDay: 18.910204892070006,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Mitchell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2267.1452862270926,
        lastWeekAverageDeathPerMillionEachDay: 67.47456209009204,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Monona": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1625.0725478816016,
        lastWeekAverageDeathPerMillionEachDay: 33.16474587513473,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2076.0347735824575,
        lastWeekAverageDeathPerMillionEachDay: 18.536024764129085,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1310.8803065443183,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Muscatine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1687.605475342209,
        lastWeekAverageDeathPerMillionEachDay: 30.13581205968231,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - O'Brien": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3490.147604159093,
        lastWeekAverageDeathPerMillionEachDay: 51.936720299986504,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Osceola": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1007.0493454179258,
        lastWeekAverageDeathPerMillionEachDay: 23.97736536709347,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Page": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 860.5282319454556,
        lastWeekAverageDeathPerMillionEachDay: 28.369062591608426,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Palo Alto": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 675.219446320054,
        lastWeekAverageDeathPerMillionEachDay: 64.30661393524323,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Plymouth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1827.064384160146,
        lastWeekAverageDeathPerMillionEachDay: 34.044677965717,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Pocahontas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1208.6417887898479,
        lastWeekAverageDeathPerMillionEachDay: 86.33155634213198,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 807.8978131675102,
        lastWeekAverageDeathPerMillionEachDay: 8.160583971388991,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Pottawattamie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 933.4163036714375,
        lastWeekAverageDeathPerMillionEachDay: 21.457846061412358,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Poweshiek": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1188.9321227842631,
        lastWeekAverageDeathPerMillionEachDay: 7.7203384596380715,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Ringgold": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1430.322844299142,
        lastWeekAverageDeathPerMillionEachDay: 58.38052425710783,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Sac": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1131.570826046703,
        lastWeekAverageDeathPerMillionEachDay: 29.391450027187087,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Scott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 803.7330218626946,
        lastWeekAverageDeathPerMillionEachDay: 18.998827854924947,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Shelby": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2008.032128514056,
        lastWeekAverageDeathPerMillionEachDay: 49.88899698171568,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Sioux": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1176.3018218333093,
        lastWeekAverageDeathPerMillionEachDay: 12.295837858884765,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Story": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 267.71831914083015,
        lastWeekAverageDeathPerMillionEachDay: 5.883919101996266,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Tama": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3263.320280052213,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Taylor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1306.975984316288,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1307.0827546769056,
        lastWeekAverageDeathPerMillionEachDay: 70.02229042911993,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Van Buren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1561.612720045429,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Wapello": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2573.7081414967547,
        lastWeekAverageDeathPerMillionEachDay: 12.255753054746451,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 544.0484980375394,
        lastWeekAverageDeathPerMillionEachDay: 11.10303057219468,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1229.2283177782838,
        lastWeekAverageDeathPerMillionEachDay: 26.01541413287373,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3260.3632976245926,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Webster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1559.7147950089125,
        lastWeekAverageDeathPerMillionEachDay: 31.830914183855352,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Winnebago": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2704.268881591655,
        lastWeekAverageDeathPerMillionEachDay: 27.59458042440464,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Winneshiek": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 850.3826722024909,
        lastWeekAverageDeathPerMillionEachDay: 14.292145751302368,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Woodbury": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1542.087346154965,
        lastWeekAverageDeathPerMillionEachDay: 19.397325108867484,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Worth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 270.96599376778204,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Iowa - Wright": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1114.4722177997132,
        lastWeekAverageDeathPerMillionEachDay: 45.4886619510087,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Allen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 646.7782359123614,
        lastWeekAverageDeathPerMillionEachDay: 80.84727948904518,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Anderson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 254.51768897938405,
        lastWeekAverageDeathPerMillionEachDay: 36.35966985419772,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Atchison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 622.1613886642194,
        lastWeekAverageDeathPerMillionEachDay: 8.888019838060277,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Barber": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 451.77320984865594,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Barton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 737.0340199387098,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Bourbon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 550.4334663547544,
        lastWeekAverageDeathPerMillionEachDay: 49.14584521024592,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Brown": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1882.0577164366375,
        lastWeekAverageDeathPerMillionEachDay: 74.68483001732689,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Butler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 269.0140634574285,
        lastWeekAverageDeathPerMillionEachDay: 4.2700644993242625,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Chase": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1132.9305135951658,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Chautauqua": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 923.0769230769231,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Cherokee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 601.8355985756558,
        lastWeekAverageDeathPerMillionEachDay: 7.164709506853045,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Cheyenne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3763.6432066240127,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2507.52256770311,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2124.468882779305,
        lastWeekAverageDeathPerMillionEachDay: 53.55803906166315,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Cloud": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2162.5312997951287,
        lastWeekAverageDeathPerMillionEachDay: 16.259633833046077,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Coffey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1222.6433549333658,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Comanche": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2352.9411764705883,
        lastWeekAverageDeathPerMillionEachDay: 168.06722689075627,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Cowley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1059.9289561132118,
        lastWeekAverageDeathPerMillionEachDay: 110.49452438245838,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Crawford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 669.7923643670463,
        lastWeekAverageDeathPerMillionEachDay: 11.040533478577684,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Decatur": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 353.73187124159887,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Dickinson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 758.1501137225171,
        lastWeekAverageDeathPerMillionEachDay: 7.736225650229765,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Doniphan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 657.8947368421051,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 253.56006510768123,
        lastWeekAverageDeathPerMillionEachDay: 2.3369591254164166,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Edwards": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2859.1851322373122,
        lastWeekAverageDeathPerMillionEachDay: 408.4550188910446,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Ellis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1085.7002766784576,
        lastWeekAverageDeathPerMillionEachDay: 15.009681244402639,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Ellsworth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 655.5227794165846,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Finney": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 932.3497957057067,
        lastWeekAverageDeathPerMillionEachDay: 7.8348722328210645,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Ford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1279.0386388649274,
        lastWeekAverageDeathPerMillionEachDay: 127.47893410613895,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 665.5183213279048,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Geary": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 284.18061256709825,
        lastWeekAverageDeathPerMillionEachDay: 13.532410122242773,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Gove": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 7587.253414264038,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Graham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1208.7026591458502,
        lastWeekAverageDeathPerMillionEachDay: 57.55726948313572,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1958.0419580419577,
        lastWeekAverageDeathPerMillionEachDay: 19.980019980019975,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Gray": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1169.004676018704,
        lastWeekAverageDeathPerMillionEachDay: 71.571714858288,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Greeley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 811.6883116883117,
        lastWeekAverageDeathPerMillionEachDay: 115.95547309833023,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Greenwood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 334.3363423604146,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Harper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2759.3818984547465,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Harvey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 668.0414766621162,
        lastWeekAverageDeathPerMillionEachDay: 16.597303768002888,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Haskell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 252.01612903225805,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Hodgeman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2229.654403567447,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 759.2437931819907,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 472.61460904269285,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Jewell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 694.6856547412295,
        lastWeekAverageDeathPerMillionEachDay: 49.62040391008782,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 639.1091648254236,
        lastWeekAverageDeathPerMillionEachDay: 10.908727859728938,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Kearny": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2084.4189682126107,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Kingman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 978.7472035794183,
        lastWeekAverageDeathPerMillionEachDay: 39.948865452221156,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Kiowa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 404.04040404040416,
        lastWeekAverageDeathPerMillionEachDay: 57.72005772005773,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Labette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 356.8151697420736,
        lastWeekAverageDeathPerMillionEachDay: 7.281942239634155,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Lane": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1954.3973941368079,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Leavenworth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 440.32388267814764,
        lastWeekAverageDeathPerMillionEachDay: 1.7473169947545542,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 337.6097231600271,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Linn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 309.1827269916521,
        lastWeekAverageDeathPerMillionEachDay: 14.72298699960248,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Logan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1789.549033643522,
        lastWeekAverageDeathPerMillionEachDay: 255.6498619490746,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Lyon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1656.8760355475222,
        lastWeekAverageDeathPerMillionEachDay: 17.214296473221008,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 420.7337596768764,
        lastWeekAverageDeathPerMillionEachDay: 48.08385824878587,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Marshall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2163.3872463170906,
        lastWeekAverageDeathPerMillionEachDay: 58.86768017189362,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - McPherson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 770.7939177352673,
        lastWeekAverageDeathPerMillionEachDay: 40.04124247975414,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Meade": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 991.8175055789734,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Miami": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 204.4571662236762,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Mitchell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 334.50409767519653,
        lastWeekAverageDeathPerMillionEachDay: 47.78629966788522,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 628.3577869238744,
        lastWeekAverageDeathPerMillionEachDay: 62.835778692387436,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Morris": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1601.423487544484,
        lastWeekAverageDeathPerMillionEachDay: 152.51652262328417,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Morton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3092.38500193274,
        lastWeekAverageDeathPerMillionEachDay: 110.44232149759786,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Nemaha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4202.91271625452,
        lastWeekAverageDeathPerMillionEachDay: 125.6684865325272,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Neosho": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 562.2540138689325,
        lastWeekAverageDeathPerMillionEachDay: 53.54800132085071,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Ness": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 5090.909090909091,
        lastWeekAverageDeathPerMillionEachDay: 51.94805194805195,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Norton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4476.776720761051,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Osage": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 376.19913474199006,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Osborne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 292.3121894182988,
        lastWeekAverageDeathPerMillionEachDay: 41.758884202614105,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Ottawa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1402.524544179523,
        lastWeekAverageDeathPerMillionEachDay: 200.36064916850327,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Pawnee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 779.544745868413,
        lastWeekAverageDeathPerMillionEachDay: 89.09082809924719,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Phillips": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2292.7015666794036,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Pottawatomie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 123.03654185293034,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Pratt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1855.085115670013,
        lastWeekAverageDeathPerMillionEachDay: 31.177901103697696,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Rawlins": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1185.7707509881423,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Reno": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1403.2710732604278,
        lastWeekAverageDeathPerMillionEachDay: 25.346439746904277,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Republic": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1941.3287316652281,
        lastWeekAverageDeathPerMillionEachDay: 30.814741772463936,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Rice": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 314.56432840515885,
        lastWeekAverageDeathPerMillionEachDay: 29.958507467157983,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Riley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 255.95430542084273,
        lastWeekAverageDeathPerMillionEachDay: 5.773405385432543,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Rooks": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1829.2682926829266,
        lastWeekAverageDeathPerMillionEachDay: 29.036004645760737,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Rush": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1646.9038208168643,
        lastWeekAverageDeathPerMillionEachDay: 235.27197440240917,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Russell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2479.579929988331,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Saline": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 848.3328415461789,
        lastWeekAverageDeathPerMillionEachDay: 7.9037221262066355,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Scott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1866.0584698320545,
        lastWeekAverageDeathPerMillionEachDay: 29.619975711619908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Sedgwick": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 457.32711678506786,
        lastWeekAverageDeathPerMillionEachDay: 10.242798620488808,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Seward": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 746.6865783087549,
        lastWeekAverageDeathPerMillionEachDay: 13.333688898370621,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Shawnee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1074.2049469964666,
        lastWeekAverageDeathPerMillionEachDay: 15.34578495709238,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Sheridan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2776.6759222530736,
        lastWeekAverageDeathPerMillionEachDay: 396.6679888932962,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Sherman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1183.0319418624301,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Smith": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 279.09572983533354,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Stafford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1684.3118383060635,
        lastWeekAverageDeathPerMillionEachDay: 68.74742197167606,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Stanton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2492.5224327018946,
        lastWeekAverageDeathPerMillionEachDay: 213.64477994587665,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Stevens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 911.5770282588879,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Sumner": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 525.4860746190228,
        lastWeekAverageDeathPerMillionEachDay: 37.534719615644484,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Thomas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 771.5057220007715,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Trego": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2140.563681769532,
        lastWeekAverageDeathPerMillionEachDay: 152.89740584068088,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Wabaunsee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1009.9552734093204,
        lastWeekAverageDeathPerMillionEachDay: 103.05666055197146,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Wallace": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 658.7615283267457,
        lastWeekAverageDeathPerMillionEachDay: 94.10878976096366,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1479.8372179060302,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Wichita": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2359.603586597452,
        lastWeekAverageDeathPerMillionEachDay: 202.2517359940673,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Wilson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 586.5102639296189,
        lastWeekAverageDeathPerMillionEachDay: 16.757436112274824,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Woodson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 637.3486297004462,
        lastWeekAverageDeathPerMillionEachDay: 91.04980424292089,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kansas - Wyandotte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1148.5289761770912,
        lastWeekAverageDeathPerMillionEachDay: 5.181333727114697,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Adair": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1926.8826163941255,
        lastWeekAverageDeathPerMillionEachDay: 22.31910366479682,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Allen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 703.7297677691765,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Anderson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 43.96184112190618,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Ballard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 380.3245436105477,
        lastWeekAverageDeathPerMillionEachDay: 18.110692552883222,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Barren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 768.3789464168682,
        lastWeekAverageDeathPerMillionEachDay: -3.2284829681381018,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Bath": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 160,
        lastWeekAverageDeathPerMillionEachDay: 11.428571428571427,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Bell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 883.5279655808235,
        lastWeekAverageDeathPerMillionEachDay: 10.975502678022652,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Boone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 262.01331027616203,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Bourbon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 50.535678188801285,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Boyd": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 642.15077700244,
        lastWeekAverageDeathPerMillionEachDay: 6.115721685737524,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Bracken": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 361.31518728170545,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Breathitt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 237.52969121140137,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Breckinridge": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 195.3411144210578,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Bullitt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 330.5744649591067,
        lastWeekAverageDeathPerMillionEachDay: 3.4981424863397534,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Butler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1242.332479229754,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Caldwell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1019.8478073272142,
        lastWeekAverageDeathPerMillionEachDay: 11.207118761837519,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Calloway": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 512.8073639137458,
        lastWeekAverageDeathPerMillionEachDay: 10.988729226723123,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Campbell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 235.08292015729188,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Carlisle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 210.08403361344537,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 658.4516978647354,
        lastWeekAverageDeathPerMillionEachDay: 13.437789752341537,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Carter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 186.58805090122024,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Casey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1299.5853703818307,
        lastWeekAverageDeathPerMillionEachDay: 8.840716805318575,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Christian": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 695.4201615077845,
        lastWeekAverageDeathPerMillionEachDay: 10.137320138597442,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 386.06844441993206,
        lastWeekAverageDeathPerMillionEachDay: 3.939473922652368,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 401.98984975629367,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Clinton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1272.264631043257,
        lastWeekAverageDeathPerMillionEachDay: 41.942790034393084,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Crittenden": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 340.6768112650466,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Cumberland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 302.3888720895071,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Daviess": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 660.026992148634,
        lastWeekAverageDeathPerMillionEachDay: 14.073070195066824,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Edmonson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1069.9588477366256,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Elliott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 133.03179459890913,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Fayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 414.66554438778036,
        lastWeekAverageDeathPerMillionEachDay: 3.0945189879685104,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Fleming": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 68.5824017557095,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Floyd": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 252.88712804518252,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 333.39216724520014,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Fulton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 670.1289998324679,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Gallatin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1014.7705491036194,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Garrard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 226.4236386278727,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 239.33942319199014,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Graves": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1180.7009069929697,
        lastWeekAverageDeathPerMillionEachDay: 15.33377801289571,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Grayson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1059.5224580921029,
        lastWeekAverageDeathPerMillionEachDay: 16.217180481001574,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Green": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 913.9932364500502,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Greenup": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 512.8497350276368,
        lastWeekAverageDeathPerMillionEachDay: 4.070235992282832,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Hancock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1261.1786287548728,
        lastWeekAverageDeathPerMillionEachDay: 32.75788646116552,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Hardin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 369.5091836550767,
        lastWeekAverageDeathPerMillionEachDay: 6.43744222395604,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Harlan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 615.1480199923107,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Harrison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 52.949274594938046,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Hart": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 262.67402153926975,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Henderson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 884.7600088476,
        lastWeekAverageDeathPerMillionEachDay: 3.1598571744557145,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Henry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 186.03497457522013,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Hickman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 684.931506849315,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Hopkins": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1499.3510271673456,
        lastWeekAverageDeathPerMillionEachDay: 31.96910505687304,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1350.4388926401082,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 659.9222439443005,
        lastWeekAverageDeathPerMillionEachDay: 4.657836278545317,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Jessamine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 776.1249191536542,
        lastWeekAverageDeathPerMillionEachDay: 10.559522709573526,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 585.9022895258698,
        lastWeekAverageDeathPerMillionEachDay: 12.876973396172962,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Kenton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 389.22621827806324,
        lastWeekAverageDeathPerMillionEachDay: 1.7108844759475308,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Knott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 810.4822369309738,
        lastWeekAverageDeathPerMillionEachDay: 28.945794176106208,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Knox": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 321.0788248515011,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Larue": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 208.36227253785248,
        lastWeekAverageDeathPerMillionEachDay: 19.844025955985952,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Laurel": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 197.32622958906816,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Lawrence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 261.1477443363583,
        lastWeekAverageDeathPerMillionEachDay: 9.326705154869938,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Lee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2431.446710792922,
        lastWeekAverageDeathPerMillionEachDay: 19.29719611740414,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Leslie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 101.24531740407006,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Letcher": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 139.19175984781702,
        lastWeekAverageDeathPerMillionEachDay: 6.628179040372239,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Lewis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1657.2504708097927,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 855.431993156544,
        lastWeekAverageDeathPerMillionEachDay: 5.8192652595683265,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Livingston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 435.0663476180117,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Logan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1365.212899417017,
        lastWeekAverageDeathPerMillionEachDay: 5.2710922757413785,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Lyon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 609.0133982947626,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 279.6089775990192,
        lastWeekAverageDeathPerMillionEachDay: 9.217878382385248,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Magoffin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 82.23007976317736,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 882.0629896746743,
        lastWeekAverageDeathPerMillionEachDay: 22.236882092638847,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Marshall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 610.9324758842444,
        lastWeekAverageDeathPerMillionEachDay: 4.593477262287552,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Martin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 893.2559178204555,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Mason": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 644.4053895723492,
        lastWeekAverageDeathPerMillionEachDay: 50.213406979663574,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - McCracken": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 550.307254883977,
        lastWeekAverageDeathPerMillionEachDay: 10.918794739761449,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - McCreary": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 58.03493703209332,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - McLean": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2172.260236776366,
        lastWeekAverageDeathPerMillionEachDay: 15.516144548402615,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Meade": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 139.9972000559989,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Menifee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 154.10695022345504,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Mercer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 273.5603884557516,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Metcalfe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 695.0650382285772,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1784.0375586854461,
        lastWeekAverageDeathPerMillionEachDay: 13.41381623071764,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 426.18176652342225,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Muhlenberg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 816.4065051270327,
        lastWeekAverageDeathPerMillionEachDay: 13.995540087891987,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Nelson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 367.7027231631086,
        lastWeekAverageDeathPerMillionEachDay: 15.449694250550781,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Nicholas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 412.7115146512588,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Ohio": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 958.5729765774777,
        lastWeekAverageDeathPerMillionEachDay: 23.81547767894354,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Oldham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 718.5736313417866,
        lastWeekAverageDeathPerMillionEachDay: 19.247507982369285,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Owen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 275.20410971470506,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Owsley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1812.0045300113252,
        lastWeekAverageDeathPerMillionEachDay: 129.42889500080892,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Pendleton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 68.54009595613434,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Perry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 427.0517897352279,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Pike": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 570.1845324486834,
        lastWeekAverageDeathPerMillionEachDay: 7.404993927904978,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Pulaski": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 338.5709229135567,
        lastWeekAverageDeathPerMillionEachDay: 4.397024972903334,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Robertson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2371.9165085388995,
        lastWeekAverageDeathPerMillionEachDay: 135.53808620222281,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Rockcastle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 419.28721174004187,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Rowan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 122.64922322158627,
        lastWeekAverageDeathPerMillionEachDay: 5.840439201027917,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Russell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1060.09038665402,
        lastWeekAverageDeathPerMillionEachDay: 55.794230876527365,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Scott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 385.93782892428595,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Shelby": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 571.1488250652742,
        lastWeekAverageDeathPerMillionEachDay: 2.9140246176799702,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Simpson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 484.6004738315744,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Spencer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 258.38457960828896,
        lastWeekAverageDeathPerMillionEachDay: 22.147249680710484,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Taylor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 426.86949435368075,
        lastWeekAverageDeathPerMillionEachDay: 11.087519333861838,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Todd": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1220.1073694485115,
        lastWeekAverageDeathPerMillionEachDay: 23.240140370447836,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Trigg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 136.50945327963962,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 625.8257422988665,
        lastWeekAverageDeathPerMillionEachDay: 19.8674838825037,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 624.5485191427885,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 744.1091360066143,
        lastWeekAverageDeathPerMillionEachDay: 59.05628063544558,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 245.90567058476367,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Webster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 772.6781023025807,
        lastWeekAverageDeathPerMillionEachDay: 11.03825860432258,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Whitley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 248.1800132362674,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Wolfe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 139.7233477714126,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Kentucky - Woodford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 37.40555098376599,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Acadia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2127.4881134660327,
        lastWeekAverageDeathPerMillionEachDay: 11.512381566374637,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Allen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2770.5154719631637,
        lastWeekAverageDeathPerMillionEachDay: 5.574477810791073,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Ascension": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 892.5468389624339,
        lastWeekAverageDeathPerMillionEachDay: 2.25675559788226,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Assumption": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1187.7027088757936,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Avoyelles": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2067.5567955360707,
        lastWeekAverageDeathPerMillionEachDay: 32.0275579342937,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Beauregard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1333.4400085340162,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Bienville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3171.9658636054683,
        lastWeekAverageDeathPerMillionEachDay: 53.944997680365105,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Bossier": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1393.2729319342877,
        lastWeekAverageDeathPerMillionEachDay: 14.618682901651123,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Caddo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2002.464571780653,
        lastWeekAverageDeathPerMillionEachDay: 5.947325725514265,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Calcasieu": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1341.9453783991034,
        lastWeekAverageDeathPerMillionEachDay: 9.831101673253505,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Caldwell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1814.8820326678767,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Cameron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 573.6411874372579,
        lastWeekAverageDeathPerMillionEachDay: 20.487185265616354,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Catahoula": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2106.5936380872126,
        lastWeekAverageDeathPerMillionEachDay: 15.047097414908663,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Claiborne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2042.11869814933,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Concordia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1713.4846046004468,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - De Soto": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1638.5682554709974,
        lastWeekAverageDeathPerMillionEachDay: 10.403607971244428,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - East Baton Rouge": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1263.466944205209,
        lastWeekAverageDeathPerMillionEachDay: 4.544845123040321,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - East Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2040.518874799592,
        lastWeekAverageDeathPerMillionEachDay: 41.643242342848815,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - East Feliciana": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4703.423046772929,
        lastWeekAverageDeathPerMillionEachDay: 7.465750867893538,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Evangeline": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1646.9531366971103,
        lastWeekAverageDeathPerMillionEachDay: 12.833401065172287,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3647.26455158631,
        lastWeekAverageDeathPerMillionEachDay: 7.137504014846008,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1607.932466836393,
        lastWeekAverageDeathPerMillionEachDay: 6.380684392207908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Iberia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1646.8566518688242,
        lastWeekAverageDeathPerMillionEachDay: 4.091569321413227,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Iberville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2091.599766232967,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2350.10162601626,
        lastWeekAverageDeathPerMillionEachDay: 27.221254355400692,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1491.3536172839788,
        lastWeekAverageDeathPerMillionEachDay: 0.6606217573793925,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Jefferson Davis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1912.7773527161435,
        lastWeekAverageDeathPerMillionEachDay: 18.216927168725174,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - LaSalle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1611.6035455278002,
        lastWeekAverageDeathPerMillionEachDay: 19.18575649437857,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Lafayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 707.8849380089204,
        lastWeekAverageDeathPerMillionEachDay: 3.507274672215955,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Lafourche": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1618.620279877887,
        lastWeekAverageDeathPerMillionEachDay: 8.780941843822172,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1454.7944033203544,
        lastWeekAverageDeathPerMillionEachDay: 15.281453816390277,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Livingston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 802.6195228320395,
        lastWeekAverageDeathPerMillionEachDay: 6.088137973441513,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1461.053785042462,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Morehouse": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1648.307469647021,
        lastWeekAverageDeathPerMillionEachDay: 17.229694804672693,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Natchitoches": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1441.3753341370093,
        lastWeekAverageDeathPerMillionEachDay: 3.7438320367195046,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Orleans": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1701.9356955380583,
        lastWeekAverageDeathPerMillionEachDay: 3.2954865016872903,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Ouachita": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1624.4886775096395,
        lastWeekAverageDeathPerMillionEachDay: 19.572152741079996,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Plaquemines": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 862.1804543690994,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Pointe Coupee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2439.0243902439024,
        lastWeekAverageDeathPerMillionEachDay: 6.57418973111564,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Rapides": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1573.491299518697,
        lastWeekAverageDeathPerMillionEachDay: 3.3056539905854976,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Red River": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3435.204927742241,
        lastWeekAverageDeathPerMillionEachDay: 50.76657528683115,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Richland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1838.7834211311,
        lastWeekAverageDeathPerMillionEachDay: 28.398199554148263,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Sabine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1674.7613465081226,
        lastWeekAverageDeathPerMillionEachDay: 23.925162092973178,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - St. Bernard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 804.3349420032173,
        lastWeekAverageDeathPerMillionEachDay: 3.0238155714406663,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - St. Charles": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1374.764595103578,
        lastWeekAverageDeathPerMillionEachDay: 2.69034167339252,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - St. Helena": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 394.78878799842084,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - St. James": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1990.8987485779294,
        lastWeekAverageDeathPerMillionEachDay: 13.543528901890676,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - St. John the Baptist": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2707.9393981838134,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - St. Landry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2277.044469339048,
        lastWeekAverageDeathPerMillionEachDay: 12.176708392187424,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - St. Martin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1459.8266923695983,
        lastWeekAverageDeathPerMillionEachDay: 8.02102578225054,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - St. Mary": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2006.1603307124908,
        lastWeekAverageDeathPerMillionEachDay: 5.789784504220752,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - St. Tammany": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1228.788990050649,
        lastWeekAverageDeathPerMillionEachDay: 7.679931187816557,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Tangipahoa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1157.6307158016593,
        lastWeekAverageDeathPerMillionEachDay: 3.180304164290273,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Tensas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 692.201199815413,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Terrebonne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1303.6275246467078,
        lastWeekAverageDeathPerMillionEachDay: 2.586562548902198,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2849.64718653881,
        lastWeekAverageDeathPerMillionEachDay: 12.92357000697873,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Vermilion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1377.8965233318208,
        lastWeekAverageDeathPerMillionEachDay: 4.801033182340839,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Vernon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1496.9744249298953,
        lastWeekAverageDeathPerMillionEachDay: 15.060104878570375,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1775.1223102567433,
        lastWeekAverageDeathPerMillionEachDay: 3.0925475788445005,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Webster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1643.1924882629107,
        lastWeekAverageDeathPerMillionEachDay: 11.1781801922647,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - West Baton Rouge": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1775.930474211222,
        lastWeekAverageDeathPerMillionEachDay: 5.397964967207361,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - West Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2493.0747922437677,
        lastWeekAverageDeathPerMillionEachDay: 26.38174383326738,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - West Feliciana": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1605.858170606372,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Louisiana - Winn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2085.730724971231,
        lastWeekAverageDeathPerMillionEachDay: 30.823606772973857,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Androscoggin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 387.8940125788487,
        lastWeekAverageDeathPerMillionEachDay: 7.916204338343849,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Aroostook": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 44.739393035567815,
        lastWeekAverageDeathPerMillionEachDay: 2.130447287407991,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Cumberland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 281.35307098571883,
        lastWeekAverageDeathPerMillionEachDay: 1.937026306270009,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 165.56839630451339,
        lastWeekAverageDeathPerMillionEachDay: 4.730525608700382,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Hancock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 181.861167184971,
        lastWeekAverageDeathPerMillionEachDay: 5.196033348142029,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Kennebec": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 220.76499157822442,
        lastWeekAverageDeathPerMillionEachDay: 9.344549908072992,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Knox": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 100.57326762546515,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 57.746722873476934,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Oxford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 206.98576972833123,
        lastWeekAverageDeathPerMillionEachDay: 14.784697837737944,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Penobscot": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 105.16076451875804,
        lastWeekAverageDeathPerMillionEachDay: 0.9389353974889111,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Piscataquis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 59.57700327673518,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Somerset": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 396.1651216226923,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Waldo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 428.0498552184314,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 63.73689410115044,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maine - York": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 231.16821822279803,
        lastWeekAverageDeathPerMillionEachDay: 4.128003896835679,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Allegany": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1988.1845035219274,
        lastWeekAverageDeathPerMillionEachDay: 46.661473041841155,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Anne Arundel": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 567.9915198348162,
        lastWeekAverageDeathPerMillionEachDay: 4.439360554505729,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Baltimore": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1047.898763551978,
        lastWeekAverageDeathPerMillionEachDay: 7.9425511819724814,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Baltimore City": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1118.8057086050314,
        lastWeekAverageDeathPerMillionEachDay: 9.628276321902163,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Calvert": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 486.3550391786004,
        lastWeekAverageDeathPerMillionEachDay: 4.631952754081908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Caroline": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 299.34742261869127,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 926.1073215907677,
        lastWeekAverageDeathPerMillionEachDay: 2.5442508834911197,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Cecil": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 631.9576102279908,
        lastWeekAverageDeathPerMillionEachDay: 11.111342597415224,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Charles": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 716.6614601517851,
        lastWeekAverageDeathPerMillionEachDay: 4.375222589449238,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Dorchester": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 532.4313320179147,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Frederick": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 654.9873433328067,
        lastWeekAverageDeathPerMillionEachDay: 5.504095322124426,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Garrett": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1171.8480733439028,
        lastWeekAverageDeathPerMillionEachDay: 73.8559710090695,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Harford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 520.6681777788218,
        lastWeekAverageDeathPerMillionEachDay: 7.829596658328147,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Howard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 521.9687432834904,
        lastWeekAverageDeathPerMillionEachDay: 2.6317751762192794,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Kent": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1338.6880856760372,
        lastWeekAverageDeathPerMillionEachDay: 7.355429042176028,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1008.8627642078333,
        lastWeekAverageDeathPerMillionEachDay: 5.1666826199323,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Prince George's": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1139.3041227193298,
        lastWeekAverageDeathPerMillionEachDay: 6.755388482753887,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Queen Anne's": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 535.916317659435,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Somerset": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 624.6096189881323,
        lastWeekAverageDeathPerMillionEachDay: 22.307486392433297,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - St. Mary's": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 713.5935159897806,
        lastWeekAverageDeathPerMillionEachDay: 7.551254137457995,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Talbot": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 188.26820150076654,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 781.2034505359188,
        lastWeekAverageDeathPerMillionEachDay: 21.752638453179337,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Wicomico": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 694.9203254543523,
        lastWeekAverageDeathPerMillionEachDay: 11.03048135641829,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Maryland - Worcester": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 803.4279592929834,
        lastWeekAverageDeathPerMillionEachDay: 5.465496321720975,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Barnstable": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1018.8271749847409,
        lastWeekAverageDeathPerMillionEachDay: 7.377945309303588,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Berkshire": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 952.4266871558458,
        lastWeekAverageDeathPerMillionEachDay: 20.58064870204709,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Bristol": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1760.3858341132698,
        lastWeekAverageDeathPerMillionEachDay: 10.61539196450213,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Dukes and Nantucket": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 69.61122132887822,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Essex": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2010.0527987387106,
        lastWeekAverageDeathPerMillionEachDay: 9.776873638253502,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1196.9222000569962,
        lastWeekAverageDeathPerMillionEachDay: 8.142327891544191,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Hampden": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2150.6436921599065,
        lastWeekAverageDeathPerMillionEachDay: 12.252634622759759,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Hampshire": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1125.4119256357646,
        lastWeekAverageDeathPerMillionEachDay: 8.882493493573518,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Middlesex": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1611.3430609561713,
        lastWeekAverageDeathPerMillionEachDay: 6.027357288355775,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Norfolk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1764.3521629938805,
        lastWeekAverageDeathPerMillionEachDay: 6.67013648514126,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Plymouth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1839.9775902625083,
        lastWeekAverageDeathPerMillionEachDay: 6.304109127966288,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Suffolk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1635.7613505044735,
        lastWeekAverageDeathPerMillionEachDay: 3.0209606690468274,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Massachusetts - Worcester": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1692.7073927731267,
        lastWeekAverageDeathPerMillionEachDay: 8.255431299848615,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Alcona": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1249.3993272465161,
        lastWeekAverageDeathPerMillionEachDay: 54.91865174709961,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Alger": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 219.58717610891526,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Allegan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 465.7819632286312,
        lastWeekAverageDeathPerMillionEachDay: 14.517879373359932,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Alpena": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 809.7165991902834,
        lastWeekAverageDeathPerMillionEachDay: 20.11718258857847,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Antrim": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 342.9943405933802,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Arenac": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1545.3873546999937,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Baraga": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3410.890486051895,
        lastWeekAverageDeathPerMillionEachDay: 34.805004959713216,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Barry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 454.9147034930951,
        lastWeekAverageDeathPerMillionEachDay: 11.604966925844263,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Bay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1590.2876093322732,
        lastWeekAverageDeathPerMillionEachDay: 31.861162904740663,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Benzie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1069.458516267027,
        lastWeekAverageDeathPerMillionEachDay: 56.28729032984353,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Berrien": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1082.1311464723174,
        lastWeekAverageDeathPerMillionEachDay: 15.831522796927189,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Branch": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1309.8329388514833,
        lastWeekAverageDeathPerMillionEachDay: 32.82789320429783,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Calhoun": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1259.6993120103757,
        lastWeekAverageDeathPerMillionEachDay: 22.361526248704894,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Cass": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 753.0847509992855,
        lastWeekAverageDeathPerMillionEachDay: 8.275656604387752,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Charlevoix": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 344.26041387751985,
        lastWeekAverageDeathPerMillionEachDay: 10.928902027857772,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Cheboygan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1147.333438835259,
        lastWeekAverageDeathPerMillionEachDay: 11.303777722514866,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Chippewa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 321.2937428043589,
        lastWeekAverageDeathPerMillionEachDay: 7.649851019151401,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Clare": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 969.3053311793217,
        lastWeekAverageDeathPerMillionEachDay: 9.231479344564967,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Clinton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 414.59890696651803,
        lastWeekAverageDeathPerMillionEachDay: 7.179201852234078,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Crawford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 427.6855085893506,
        lastWeekAverageDeathPerMillionEachDay: 10.182988299746443,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Delta": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2040.0178850883074,
        lastWeekAverageDeathPerMillionEachDay: 27.945450480661744,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Dickinson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2535.758152066247,
        lastWeekAverageDeathPerMillionEachDay: 16.98052333972933,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Eaton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 716.4363187869552,
        lastWeekAverageDeathPerMillionEachDay: 15.5465385631889,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Emmet": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 987.5804279515188,
        lastWeekAverageDeathPerMillionEachDay: 12.825719843526217,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Federal Correctional Institution (FCI)": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 0,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Genesee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1325.7337739303573,
        lastWeekAverageDeathPerMillionEachDay: 12.672972878781962,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Gladwin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 903.7683209556367,
        lastWeekAverageDeathPerMillionEachDay: 39.294274824158116,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Gogebic": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2289.8032200357775,
        lastWeekAverageDeathPerMillionEachDay: 51.11167901865575,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Grand Traverse": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 418.9584049501546,
        lastWeekAverageDeathPerMillionEachDay: 10.742523203850117,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Gratiot": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1424.67637739186,
        lastWeekAverageDeathPerMillionEachDay: 35.09055116728719,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Hillsdale": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1096.371011950444,
        lastWeekAverageDeathPerMillionEachDay: 3.1324886055726973,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Houghton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 728.6178679520233,
        lastWeekAverageDeathPerMillionEachDay: 36.030553909715444,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Huron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1129.7246699590073,
        lastWeekAverageDeathPerMillionEachDay: 36.88896881498799,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Ingham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 567.7038090873649,
        lastWeekAverageDeathPerMillionEachDay: 9.771149898233476,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Ionia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 587.3533548696231,
        lastWeekAverageDeathPerMillionEachDay: 4.416190638117467,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Iosco": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1711.3065626616788,
        lastWeekAverageDeathPerMillionEachDay: 39.79782703864369,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Iron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3343.5749141514543,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Isabella": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 515.2278452026562,
        lastWeekAverageDeathPerMillionEachDay: 6.133664823841146,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 883.2250331209384,
        lastWeekAverageDeathPerMillionEachDay: 13.518750506953138,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Kalamazoo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 833.7546120588834,
        lastWeekAverageDeathPerMillionEachDay: 10.778986581239604,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Kalkaska": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 554.3851868278081,
        lastWeekAverageDeathPerMillionEachDay: 7.919788383254401,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Kent": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 765.6536596874977,
        lastWeekAverageDeathPerMillionEachDay: 11.090126851480369,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Keweenaw": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 945.179584120983,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 674.9346157091034,
        lastWeekAverageDeathPerMillionEachDay: 24.104807703896547,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Lapeer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 981.6567169290126,
        lastWeekAverageDeathPerMillionEachDay: 21.198566976872367,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Leelanau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 229.76885253435046,
        lastWeekAverageDeathPerMillionEachDay: 6.564824358124298,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Lenawee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 548.4962062345736,
        lastWeekAverageDeathPerMillionEachDay: 13.059433481775562,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Livingston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 333.34201411495087,
        lastWeekAverageDeathPerMillionEachDay: 5.208468970546107,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Mackinac": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 185.2023335494027,
        lastWeekAverageDeathPerMillionEachDay: 13.228738110671621,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Macomb": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1733.4651453364638,
        lastWeekAverageDeathPerMillionEachDay: 15.855362479739464,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Manistee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 570.0789966609658,
        lastWeekAverageDeathPerMillionEachDay: 23.268530475957785,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Marquette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 734.6436978065639,
        lastWeekAverageDeathPerMillionEachDay: 12.850910165712486,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Mason": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 514.6856986000549,
        lastWeekAverageDeathPerMillionEachDay: 4.90176855809576,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Mecosta": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 276.16044922099735,
        lastWeekAverageDeathPerMillionEachDay: 3.2876243954880637,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Menominee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1009.6575943810362,
        lastWeekAverageDeathPerMillionEachDay: 6.271165182490908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Michigan Department of Corrections (MDOC)": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 0,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Midland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 432.9212564336909,
        lastWeekAverageDeathPerMillionEachDay: 6.871765975137951,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Missaukee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 529.1705252017464,
        lastWeekAverageDeathPerMillionEachDay: 9.4494736643169,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 863.7873754152822,
        lastWeekAverageDeathPerMillionEachDay: 13.289036544850497,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Montcalm": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 876.5339343851742,
        lastWeekAverageDeathPerMillionEachDay: 35.77689528102752,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Montmorency": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 857.6329331046313,
        lastWeekAverageDeathPerMillionEachDay: 45.94462141631953,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Muskegon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1382.7592961755183,
        lastWeekAverageDeathPerMillionEachDay: 18.930633221450545,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Newaygo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 571.6619028174765,
        lastWeekAverageDeathPerMillionEachDay: 2.916642361313656,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Oakland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1226.955813687197,
        lastWeekAverageDeathPerMillionEachDay: 7.610965606614406,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Oceana": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1284.6185816299544,
        lastWeekAverageDeathPerMillionEachDay: 16.19267119701623,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Ogemaw": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1238.2721341143977,
        lastWeekAverageDeathPerMillionEachDay: 20.411079133753805,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Ontonagon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2622.377622377622,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Osceola": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 596.760443307758,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Oscoda": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1698.8229583788375,
        lastWeekAverageDeathPerMillionEachDay: 52.00478444016849,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Otsego": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 810.7669855683476,
        lastWeekAverageDeathPerMillionEachDay: 5.791192754059626,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Ottawa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 825.8232532638866,
        lastWeekAverageDeathPerMillionEachDay: 13.217088226511523,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Out of MI": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 0,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Presque Isle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 397.07750952986015,
        lastWeekAverageDeathPerMillionEachDay: 11.345071700853147,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Roscommon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1082.4763728714768,
        lastWeekAverageDeathPerMillionEachDay: 5.947672378414707,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Saginaw": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1821.1494759603017,
        lastWeekAverageDeathPerMillionEachDay: 26.991099684878908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Sanilac": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1287.3451542385233,
        lastWeekAverageDeathPerMillionEachDay: 34.699330302925155,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Schoolcraft": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 494.1932295527552,
        lastWeekAverageDeathPerMillionEachDay: 17.649758198312686,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Shiawassee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 807.3750036698864,
        lastWeekAverageDeathPerMillionEachDay: 8.388311726440378,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - St. Clair": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 842.0893871600222,
        lastWeekAverageDeathPerMillionEachDay: 15.261747954925774,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - St. Joseph": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 787.3499114231349,
        lastWeekAverageDeathPerMillionEachDay: 11.716516539034746,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Tuscola": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1550.3875968992247,
        lastWeekAverageDeathPerMillionEachDay: 24.60932693490833,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Van Buren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 964.625976188274,
        lastWeekAverageDeathPerMillionEachDay: 15.101776535237164,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Washtenaw": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 470.61895914320144,
        lastWeekAverageDeathPerMillionEachDay: 6.995162068189617,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2009.3257868811315,
        lastWeekAverageDeathPerMillionEachDay: 8.411321115576369,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Michigan - Wexford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 475.75153875888316,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Aitkin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1951.403751731084,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Anoka": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 815.3064683781566,
        lastWeekAverageDeathPerMillionEachDay: 11.607210399099925,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Becker": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 900.5606716439587,
        lastWeekAverageDeathPerMillionEachDay: 8.300098356165519,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Beltrami": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 699.3303382215819,
        lastWeekAverageDeathPerMillionEachDay: 18.164424369391735,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Benton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1760.8647802587493,
        lastWeekAverageDeathPerMillionEachDay: 24.456455281371518,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Big Stone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 601.0819475055101,
        lastWeekAverageDeathPerMillionEachDay: 28.62294988121477,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Blue Earth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 354.7514522637577,
        lastWeekAverageDeathPerMillionEachDay: 4.223231574568543,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Brown": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1039.6673064619322,
        lastWeekAverageDeathPerMillionEachDay: 17.13737318343844,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Carlton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 919.9632014719413,
        lastWeekAverageDeathPerMillionEachDay: 15.930098726786861,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Carver": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 275.9565701453054,
        lastWeekAverageDeathPerMillionEachDay: 9.515743798113979,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Cass": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 570.8720910708889,
        lastWeekAverageDeathPerMillionEachDay: 19.188977851122317,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Chippewa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1610.169491525424,
        lastWeekAverageDeathPerMillionEachDay: 12.106537530266346,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Chisago": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 441.86005408367066,
        lastWeekAverageDeathPerMillionEachDay: 2.5249145947638323,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1136.6821338482139,
        lastWeekAverageDeathPerMillionEachDay: 8.897707505661165,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Clearwater": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1360.8528010886823,
        lastWeekAverageDeathPerMillionEachDay: 32.40125716877815,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Cottonwood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 982.4937477670595,
        lastWeekAverageDeathPerMillionEachDay: 76.55795437145919,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Crow Wing": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 783.9520405810466,
        lastWeekAverageDeathPerMillionEachDay: 8.783776365053743,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Dakota": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 594.376499052494,
        lastWeekAverageDeathPerMillionEachDay: 6.992664694735223,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Dodge": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 143.30753797649754,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1442.017776146404,
        lastWeekAverageDeathPerMillionEachDay: 18.727503586316935,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Faribault": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 585.9518054640006,
        lastWeekAverageDeathPerMillionEachDay: 31.390275292714314,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Freeborn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 561.4081437204848,
        lastWeekAverageDeathPerMillionEachDay: 14.153146480348356,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Goodhue": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 798.4462667242123,
        lastWeekAverageDeathPerMillionEachDay: 6.165608237252604,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1172.136637642331,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Hennepin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1037.2534350626418,
        lastWeekAverageDeathPerMillionEachDay: 7.89987383901479,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Houston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 483.87096774193554,
        lastWeekAverageDeathPerMillionEachDay: 30.721966205837173,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Hubbard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1582.0576055092827,
        lastWeekAverageDeathPerMillionEachDay: 6.64730086348438,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Isanti": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 665.0901566656813,
        lastWeekAverageDeathPerMillionEachDay: 10.556986613740973,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Itasca": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 686.9044981165523,
        lastWeekAverageDeathPerMillionEachDay: 6.3309170333322795,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 507.8204346942921,
        lastWeekAverageDeathPerMillionEachDay: 29.018310553959545,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Kanabec": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1101.7934749341985,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Kandiyohi": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1273.1776198523112,
        lastWeekAverageDeathPerMillionEachDay: 26.455638854073996,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Kittson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3955.3280595625874,
        lastWeekAverageDeathPerMillionEachDay: 132.95220368277603,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Koochiching": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 654.1826805135333,
        lastWeekAverageDeathPerMillionEachDay: 11.681833580598807,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Lac qui Parle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1962.8567114600633,
        lastWeekAverageDeathPerMillionEachDay: 129.41912383253162,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1315.6658208814958,
        lastWeekAverageDeathPerMillionEachDay: 40.27548431269885,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Lake of the Woods": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 267.3796791443851,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Le Sueur": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 484.64707307785505,
        lastWeekAverageDeathPerMillionEachDay: 4.945378296712806,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 177.33640716439086,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Lyon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1020.648504357384,
        lastWeekAverageDeathPerMillionEachDay: 16.823876445451383,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Mahnomen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1085.5798805862132,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Marshall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1178.2347900599827,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Martin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1168.5210587816898,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - McLeod": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 947.2599114033378,
        lastWeekAverageDeathPerMillionEachDay: 15.920334645434249,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Meeker": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1291.878391180777,
        lastWeekAverageDeathPerMillionEachDay: 49.21441490212484,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Mille Lacs": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1598.3559767096701,
        lastWeekAverageDeathPerMillionEachDay: 21.746339819179184,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Morrison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1138.2016414065777,
        lastWeekAverageDeathPerMillionEachDay: 8.557907078244945,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Mower": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 574.1101292995857,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Murray": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 610.20258725897,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Nicollet": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 875.2990605123416,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Nobles": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1988.0715705765413,
        lastWeekAverageDeathPerMillionEachDay: 13.20977787758499,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Norman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1254.9019607843138,
        lastWeekAverageDeathPerMillionEachDay: 22.408963585434172,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Olmsted": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 309.5525386466868,
        lastWeekAverageDeathPerMillionEachDay: 2.707456606239243,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Otter Tail": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 731.9647295134989,
        lastWeekAverageDeathPerMillionEachDay: 14.590659060069745,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Pennington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 849.9185494723423,
        lastWeekAverageDeathPerMillionEachDay: 20.23615593981767,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Pine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 371.8854592785422,
        lastWeekAverageDeathPerMillionEachDay: 4.829681289331717,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Pipestone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1972.386587771203,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1466.6496620329042,
        lastWeekAverageDeathPerMillionEachDay: 31.883688305063135,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Pope": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 266.6903724775536,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Ramsey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1146.6035277592534,
        lastWeekAverageDeathPerMillionEachDay: 10.64313892644994,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Red Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 739.8273736128236,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Redwood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1713.909030982202,
        lastWeekAverageDeathPerMillionEachDay: 37.66833035125718,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Renville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2543.304921638713,
        lastWeekAverageDeathPerMillionEachDay: 29.45913036647158,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Rice": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 701.7858209400944,
        lastWeekAverageDeathPerMillionEachDay: 4.266175203283248,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Rock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1073.5373054213633,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Roseau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 857.2370590174744,
        lastWeekAverageDeathPerMillionEachDay: 37.68074984692195,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Scott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 489.8901438129559,
        lastWeekAverageDeathPerMillionEachDay: 2.87606738050659,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Sherburne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 545.0544025998066,
        lastWeekAverageDeathPerMillionEachDay: 5.87659733261247,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Sibley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 269.08846283215604,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - St. Louis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 899.1811925453359,
        lastWeekAverageDeathPerMillionEachDay: 15.070075852715126,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Stearns": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 987.1178022660254,
        lastWeekAverageDeathPerMillionEachDay: 12.416576129132395,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Steele": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 245.57286692679205,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Stevens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 611.9326874043855,
        lastWeekAverageDeathPerMillionEachDay: 14.569825890580608,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Swift": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1510.9000647528596,
        lastWeekAverageDeathPerMillionEachDay: 15.417347599518974,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Todd": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 932.5332468374961,
        lastWeekAverageDeathPerMillionEachDay: 28.960659839673795,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Traverse": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 613.6851795029149,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Wabasha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 92.47699634715865,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Wadena": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 730.8872971787749,
        lastWeekAverageDeathPerMillionEachDay: 10.441247102553927,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Waseca": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 644.7453255963896,
        lastWeekAverageDeathPerMillionEachDay: 7.6755395904332095,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 647.7671086724584,
        lastWeekAverageDeathPerMillionEachDay: 10.886842162562328,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Watonwan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 458.84188308708826,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Wilkin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 966.6505558240694,
        lastWeekAverageDeathPerMillionEachDay: 23.015489424382604,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Winona": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 831.9467554076539,
        lastWeekAverageDeathPerMillionEachDay: 8.48925260620055,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Wright": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 541.9975863040823,
        lastWeekAverageDeathPerMillionEachDay: 10.323763548649188,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Minnesota - Yellow Medicine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1441.9610670511895,
        lastWeekAverageDeathPerMillionEachDay: 14.713888439297852,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1726.778092724726,
        lastWeekAverageDeathPerMillionEachDay: 4.654388390093601,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Alcorn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1028.3332882309962,
        lastWeekAverageDeathPerMillionEachDay: 11.597743852229282,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Amite": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1626.4129462470523,
        lastWeekAverageDeathPerMillionEachDay: 46.468941321344346,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Attala": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2366.0173874766147,
        lastWeekAverageDeathPerMillionEachDay: 31.442091527928433,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Benton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2421.60067804819,
        lastWeekAverageDeathPerMillionEachDay: 17.297147700344212,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Bolivar": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3003.787384092987,
        lastWeekAverageDeathPerMillionEachDay: 13.992798373103975,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Calhoun": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1114.1285425806,
        lastWeekAverageDeathPerMillionEachDay: 9.947576273041072,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1709.058007439429,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Chickasaw": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2046.424603870666,
        lastWeekAverageDeathPerMillionEachDay: 8.35275348518639,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Choctaw": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1218.026796589525,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Claiborne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2002.6702269692926,
        lastWeekAverageDeathPerMillionEachDay: 31.78841630109988,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Clarke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3539.0258027153977,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1553.116587285152,
        lastWeekAverageDeathPerMillionEachDay: 14.791586545572875,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Coahoma": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2169.5895859699867,
        lastWeekAverageDeathPerMillionEachDay: 6.457111863005913,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Copiah": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1496.5259219668624,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Covington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3380.553766902769,
        lastWeekAverageDeathPerMillionEachDay: 15.33130960046607,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - DeSoto": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 692.097650652897,
        lastWeekAverageDeathPerMillionEachDay: 10.814025791451515,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Forrest": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1241.7052752446693,
        lastWeekAverageDeathPerMillionEachDay: 1.9073813751838236,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 777.9074290159472,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - George": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1346.9387755102043,
        lastWeekAverageDeathPerMillionEachDay: 17.49271137026239,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1840.129545119976,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Grenada": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2601.406686578668,
        lastWeekAverageDeathPerMillionEachDay: 20.646084814116413,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Hancock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1049.714477662076,
        lastWeekAverageDeathPerMillionEachDay: 11.99673688756658,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Harrison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 648.7889273356402,
        lastWeekAverageDeathPerMillionEachDay: 8.238589553468445,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Hinds": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1017.9434092477571,
        lastWeekAverageDeathPerMillionEachDay: 8.62663906142167,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Holmes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3703.7037037037035,
        lastWeekAverageDeathPerMillionEachDay: 8.398421096833795,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Humphreys": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2728.1746031746034,
        lastWeekAverageDeathPerMillionEachDay: 17.715419501133788,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Issaquena": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3014.3180105501124,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Itawamba": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1966.652415562206,
        lastWeekAverageDeathPerMillionEachDay: 18.322848592194465,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1023.5557071934379,
        lastWeekAverageDeathPerMillionEachDay: 4.97354571036656,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Jasper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1464.9331624244642,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1859.7997138769672,
        lastWeekAverageDeathPerMillionEachDay: 20.437359493153483,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Jefferson Davis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1976.9949676491735,
        lastWeekAverageDeathPerMillionEachDay: 25.675259320119135,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Jones": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1365.6788745631297,
        lastWeekAverageDeathPerMillionEachDay: 4.195634023235422,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Kemper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1950.31820981318,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Lafayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1295.840352468576,
        lastWeekAverageDeathPerMillionEachDay: 7.933716443685158,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Lamar": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 852.5014603034274,
        lastWeekAverageDeathPerMillionEachDay: 4.510589737055171,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Lauderdale": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2212.47892074199,
        lastWeekAverageDeathPerMillionEachDay: 13.49072512647555,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Lawrence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1271.2537740346418,
        lastWeekAverageDeathPerMillionEachDay: 11.3504801253093,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Leake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2018.783463530238,
        lastWeekAverageDeathPerMillionEachDay: 6.269513861895148,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Lee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1275.8087925464677,
        lastWeekAverageDeathPerMillionEachDay: 6.688381612301272,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Leflore": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3477.2735336905225,
        lastWeekAverageDeathPerMillionEachDay: 10.137823713383447,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2137.4403419904547,
        lastWeekAverageDeathPerMillionEachDay: 8.365715624228784,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Lowndes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1314.105299086953,
        lastWeekAverageDeathPerMillionEachDay: 12.190216132532031,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1185.6368563685637,
        lastWeekAverageDeathPerMillionEachDay: 5.377037897363101,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2360.3141659545026,
        lastWeekAverageDeathPerMillionEachDay: 23.25432675817244,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Marshall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1671.672238907463,
        lastWeekAverageDeathPerMillionEachDay: 12.14289761918254,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2354.4763417678428,
        lastWeekAverageDeathPerMillionEachDay: 12.157364931675607,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3069.053708439897,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Neshoba": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4567.621402568858,
        lastWeekAverageDeathPerMillionEachDay: 9.812290875550714,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Newton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1522.504519935294,
        lastWeekAverageDeathPerMillionEachDay: 6.796895178282562,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Noxubee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1727.9447057694156,
        lastWeekAverageDeathPerMillionEachDay: 13.713846871185837,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Oktibbeha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1351.1605864440278,
        lastWeekAverageDeathPerMillionEachDay: 8.642818250175017,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Panola": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1959.522695367337,
        lastWeekAverageDeathPerMillionEachDay: 12.534260311518146,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Pearl River": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1386.5130098136312,
        lastWeekAverageDeathPerMillionEachDay: 10.289521408635482,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Perry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2505.6376847907795,
        lastWeekAverageDeathPerMillionEachDay: 35.7948240684397,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Pike": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1654.4491956831605,
        lastWeekAverageDeathPerMillionEachDay: 7.2723041568490565,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Pontotoc": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1087.8348977435196,
        lastWeekAverageDeathPerMillionEachDay: 13.320427319308402,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Prentiss": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1591.9764387487066,
        lastWeekAverageDeathPerMillionEachDay: 5.6856301383882375,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Quitman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1030.6242638398114,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Rankin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 920.970432340875,
        lastWeekAverageDeathPerMillionEachDay: 12.880705347424826,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Scott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1280.0455127293417,
        lastWeekAverageDeathPerMillionEachDay: 25.39772842716948,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Sharkey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3934.274473501504,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Simpson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2138.1949133468374,
        lastWeekAverageDeathPerMillionEachDay: 16.07665348381081,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Smith": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1068.1075647147522,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Stone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 981.6753926701574,
        lastWeekAverageDeathPerMillionEachDay: 15.582149090002497,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Sunflower": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2270.011947431302,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Tallahatchie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2100.0796581939317,
        lastWeekAverageDeathPerMillionEachDay: 10.34521999110311,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Tate": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1906.712333604039,
        lastWeekAverageDeathPerMillionEachDay: 10.088425045524016,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Tippah": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1726.0958437429026,
        lastWeekAverageDeathPerMillionEachDay: 12.97816423866844,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Tishomingo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2579.5800443687763,
        lastWeekAverageDeathPerMillionEachDay: 22.110686094589507,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Tunica": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1972.5913621262455,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 937.0119729307653,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Walthall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2239.955200895982,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1520.4601044489984,
        lastWeekAverageDeathPerMillionEachDay: 12.591802107238081,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2527.955544421417,
        lastWeekAverageDeathPerMillionEachDay: 6.506964078304806,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1238.6662042312837,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Webster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1754.5670347817113,
        lastWeekAverageDeathPerMillionEachDay: 14.744260796484967,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Wilkinson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2549.2468134414835,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Winston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2450.5708716235035,
        lastWeekAverageDeathPerMillionEachDay: 55.6947925368978,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Yalobusha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2477.700693756194,
        lastWeekAverageDeathPerMillionEachDay: 11.798574732172352,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Mississippi - Yazoo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1684.068710003368,
        lastWeekAverageDeathPerMillionEachDay: 38.49299908579127,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Adair": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 157.83451051572425,
        lastWeekAverageDeathPerMillionEachDay: 5.636946804133009,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Andrew": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 733.9656729900632,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Atchison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 777.7561734396267,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Audrain": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 905.9398140853947,
        lastWeekAverageDeathPerMillionEachDay: 39.3886875689302,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Barry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 977.9541199809997,
        lastWeekAverageDeathPerMillionEachDay: 7.983298938620405,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Barton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 595.5419431682831,
        lastWeekAverageDeathPerMillionEachDay: 12.153917207515981,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Bates": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 618.3527083848628,
        lastWeekAverageDeathPerMillionEachDay: 8.833610119783753,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Benton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 771.4858818083632,
        lastWeekAverageDeathPerMillionEachDay: 29.389938354604308,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Bollinger": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 906.6183136899366,
        lastWeekAverageDeathPerMillionEachDay: 11.77426381415502,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Boone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 199.4868754259876,
        lastWeekAverageDeathPerMillionEachDay: 5.541302095166322,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Buchanan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 984.3871617599929,
        lastWeekAverageDeathPerMillionEachDay: 22.89272469209286,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Butler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 282.4991760440699,
        lastWeekAverageDeathPerMillionEachDay: 6.726170858192139,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Caldwell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 332.59423503325945,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Callaway": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 379.94770131640706,
        lastWeekAverageDeathPerMillionEachDay: 12.771351304753178,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Camden": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1252.5645178706402,
        lastWeekAverageDeathPerMillionEachDay: 21.5959399632869,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Cape Girardeau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1153.7827591890557,
        lastWeekAverageDeathPerMillionEachDay: 3.6225518341885574,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1036.9858278603522,
        lastWeekAverageDeathPerMillionEachDay: 16.460092505719874,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Carter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1003.0090270812439,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Cass": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 434.86481376441674,
        lastWeekAverageDeathPerMillionEachDay: 6.752559219944359,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Cedar": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 557.5301414732734,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Chariton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 269.323996768112,
        lastWeekAverageDeathPerMillionEachDay: 38.47485668115886,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Christian": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 395.0561544105198,
        lastWeekAverageDeathPerMillionEachDay: 9.674844597808647,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 588.4949242312787,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 348.0723990590043,
        lastWeekAverageDeathPerMillionEachDay: 3.429284719793145,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Clinton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2452.5432873890227,
        lastWeekAverageDeathPerMillionEachDay: 14.014533070794414,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Cole": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1003.3226920320537,
        lastWeekAverageDeathPerMillionEachDay: 22.33742542557448,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Cooper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 959.9638601840873,
        lastWeekAverageDeathPerMillionEachDay: 56.46846236376984,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Crawford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 627.0903010033444,
        lastWeekAverageDeathPerMillionEachDay: 5.972288580984232,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Dade": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1058.0611030287,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Dallas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 829.4821661334281,
        lastWeekAverageDeathPerMillionEachDay: 8.464103736055389,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Daviess": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1087.2191350567766,
        lastWeekAverageDeathPerMillionEachDay: 17.257446588202804,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - DeKalb": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1354.9055551127756,
        lastWeekAverageDeathPerMillionEachDay: 11.38576096733425,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Dent": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 385.28221922558276,
        lastWeekAverageDeathPerMillionEachDay: 9.173386172037684,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1441.031475161168,
        lastWeekAverageDeathPerMillionEachDay: 10.83482312151254,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Dunklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 480.58769008959524,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1019.5542816470612,
        lastWeekAverageDeathPerMillionEachDay: 13.74062374187414,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Gasconade": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1971.984224126207,
        lastWeekAverageDeathPerMillionEachDay: 19.428416001243416,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Gentry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2282.7575711459444,
        lastWeekAverageDeathPerMillionEachDay: 21.74054829662804,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 812.0483407600499,
        lastWeekAverageDeathPerMillionEachDay: 4.386815766410834,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Grundy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1928.9340101522841,
        lastWeekAverageDeathPerMillionEachDay: 29.006526468455398,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Harrison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 838.1226053639847,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Henry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 870.6011730205279,
        lastWeekAverageDeathPerMillionEachDay: 19.637620444072056,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Hickory": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 628.6672254819781,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Holt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1589.8251192368841,
        lastWeekAverageDeathPerMillionEachDay: 64.8908211933422,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Howard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 199.98000199980007,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Howell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 872.4480893386843,
        lastWeekAverageDeathPerMillionEachDay: 3.5610126095456502,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Iron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 98.76543209876543,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 294.44773979354517,
        lastWeekAverageDeathPerMillionEachDay: 2.0320754989202565,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Jasper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1195.107477251747,
        lastWeekAverageDeathPerMillionEachDay: 11.7744579039581,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 537.5842474486964,
        lastWeekAverageDeathPerMillionEachDay: 5.0775371659853255,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 462.4320224926936,
        lastWeekAverageDeathPerMillionEachDay: 7.927406099874747,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Kansas City": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 668.7896135132318,
        lastWeekAverageDeathPerMillionEachDay: 3.2139299906708385,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Knox": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 252.58903763576663,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Laclede": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1091.7336169974528,
        lastWeekAverageDeathPerMillionEachDay: 3.999024238085908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Lafayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1100.6481594716886,
        lastWeekAverageDeathPerMillionEachDay: 4.367651426474954,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Lawrence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1068.961022031026,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Lewis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 204.58265139116202,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 288.07211970243844,
        lastWeekAverageDeathPerMillionEachDay: 9.683096460586166,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Linn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 922.8187919463085,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Livingston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1247.7835423918043,
        lastWeekAverageDeathPerMillionEachDay: 9.38183114580304,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Macon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 330.7534563736191,
        lastWeekAverageDeathPerMillionEachDay: 9.450098753531973,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 827.2667107875578,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Maries": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 689.8930665746809,
        lastWeekAverageDeathPerMillionEachDay: 32.85205078927052,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 806.1689449702067,
        lastWeekAverageDeathPerMillionEachDay: 15.021781583295777,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - McDonald": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 788.1945964881553,
        lastWeekAverageDeathPerMillionEachDay: 6.255512670540915,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Mercer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 276.47221454243845,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Miller": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1639.4082516882002,
        lastWeekAverageDeathPerMillionEachDay: 61.33840397472858,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Mississippi": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 758.7253414264037,
        lastWeekAverageDeathPerMillionEachDay: 21.677866897897246,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Moniteau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 991.8175055789734,
        lastWeekAverageDeathPerMillionEachDay: 8.855513442669405,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 578.4359093012494,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 519.4355467059129,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Morgan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 775.6823580743686,
        lastWeekAverageDeathPerMillionEachDay: 13.851470679899437,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - New Madrid": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1522.6048254860623,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Newton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 686.86036128855,
        lastWeekAverageDeathPerMillionEachDay: 2.4530727188876784,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Nodaway": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 662.7370564167894,
        lastWeekAverageDeathPerMillionEachDay: 5.259817908069757,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Oregon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 284.9273435274005,
        lastWeekAverageDeathPerMillionEachDay: 13.567968739400023,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Osage": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 440.6904149834741,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Ozark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 436.01482450403313,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Pemiscot": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1265.4223347042073,
        lastWeekAverageDeathPerMillionEachDay: 9.03873096217291,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Perry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 783.8628762541805,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Pettis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1180.9442830487258,
        lastWeekAverageDeathPerMillionEachDay: 40.48951827595631,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Phelps": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1705.0680905480897,
        lastWeekAverageDeathPerMillionEachDay: 51.28024332475457,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Pike": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 601.0272101409682,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Platte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 181.96096458465013,
        lastWeekAverageDeathPerMillionEachDay: 2.7362551065360923,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 497.6826650906716,
        lastWeekAverageDeathPerMillionEachDay: 4.443595224023853,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Pulaski": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 494.2308057863022,
        lastWeekAverageDeathPerMillionEachDay: 5.43110775589343,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Putnam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 212.94718909710394,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Ralls": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 388.0104762828596,
        lastWeekAverageDeathPerMillionEachDay: 27.715034020204257,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Randolph": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 606.1095846128981,
        lastWeekAverageDeathPerMillionEachDay: 23.08988893763421,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Ray": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 304.10982709184105,
        lastWeekAverageDeathPerMillionEachDay: 6.206323001874307,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Reynolds": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 318.9792663476875,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Ripley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 602.0469596628537,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Saline": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 702.9568120908572,
        lastWeekAverageDeathPerMillionEachDay: 12.552800215908164,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Schuyler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 214.59227467811155,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Scotland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 611.9951040391677,
        lastWeekAverageDeathPerMillionEachDay: 29.142624001865126,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Scott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1384.5350052246606,
        lastWeekAverageDeathPerMillionEachDay: 29.855202268995374,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Shannon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1102.1307861866278,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Shelby": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 168.63406408094437,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - St. Charles": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 649.2182019889459,
        lastWeekAverageDeathPerMillionEachDay: 9.594357664861269,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - St. Clair": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 425.66776630839615,
        lastWeekAverageDeathPerMillionEachDay: 15.202420225299862,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - St. Francois": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 862.9026259019564,
        lastWeekAverageDeathPerMillionEachDay: 14.877631481068214,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - St. Louis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1288.466664319733,
        lastWeekAverageDeathPerMillionEachDay: 18.39229765059951,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - St. Louis City": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 938.1986585755348,
        lastWeekAverageDeathPerMillionEachDay: 5.703335310489573,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Ste. Genevieve": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 111.76930814798258,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Stoddard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1068.0447889750214,
        lastWeekAverageDeathPerMillionEachDay: 29.531192321889993,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Stone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 719.8297446169254,
        lastWeekAverageDeathPerMillionEachDay: 13.412976607768794,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Sullivan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1478.0752176055184,
        lastWeekAverageDeathPerMillionEachDay: 23.461511390563782,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Taney": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 840.3661850951224,
        lastWeekAverageDeathPerMillionEachDay: 2.554304513966937,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Texas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 511.8513268761319,
        lastWeekAverageDeathPerMillionEachDay: 16.87421956734501,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Vernon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 632.2034722559937,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 224.4102218856069,
        lastWeekAverageDeathPerMillionEachDay: 20.036626954072045,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1374.8483623129807,
        lastWeekAverageDeathPerMillionEachDay: 34.660042747386065,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 310.72788005903834,
        lastWeekAverageDeathPerMillionEachDay: 11.097424287822797,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Webster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 884.016973125884,
        lastWeekAverageDeathPerMillionEachDay: 21.649395260225727,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Worth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 496.7709885742673,
        lastWeekAverageDeathPerMillionEachDay: 70.96728408203818,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Missouri - Wright": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1038.875826999836,
        lastWeekAverageDeathPerMillionEachDay: 15.622192887215578,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Beaverhead": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 846.2921823759654,
        lastWeekAverageDeathPerMillionEachDay: 15.11236039957081,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Big Horn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4279.600570613409,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Blaine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3143.242029636282,
        lastWeekAverageDeathPerMillionEachDay: 21.38259884106314,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Broadwater": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 320.666987333654,
        lastWeekAverageDeathPerMillionEachDay: 22.90478480954671,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Carbon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 652.6806526806527,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Carter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3194.8881789137376,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Cascade": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1032.3722439348132,
        lastWeekAverageDeathPerMillionEachDay: 5.267205326198026,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Chouteau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 532.3868677905946,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Custer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 701.6312927556569,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Daniels": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2958.5798816568044,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Dawson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2554.278416347381,
        lastWeekAverageDeathPerMillionEachDay: 99.5173408966512,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Deer Lodge": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1094.0919037199121,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Fallon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 351.37034434293747,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Fergus": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1176.4705882352944,
        lastWeekAverageDeathPerMillionEachDay: 12.928248222365871,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Flathead": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 491.30108086237794,
        lastWeekAverageDeathPerMillionEachDay: 9.633354526713292,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Gallatin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 262.15984759774193,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Garfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1589.825119236884,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Glacier": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1599.6509852395839,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Granite": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 591.8910920390648,
        lastWeekAverageDeathPerMillionEachDay: 42.27793514564748,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Hill": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1941.276389225916,
        lastWeekAverageDeathPerMillionEachDay: 25.999237355704235,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 245.47909336388184,
        lastWeekAverageDeathPerMillionEachDay: 23.37896127275065,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 525.313546523081,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Lewis and Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 446.48000921765185,
        lastWeekAverageDeathPerMillionEachDay: 8.23004625285994,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Liberty": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 427.8990158322637,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 500.5005005005005,
        lastWeekAverageDeathPerMillionEachDay: 7.150007150007149,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 348.83720930232556,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Meagher": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3759.3984962406016,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Missoula": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 384.61538461538447,
        lastWeekAverageDeathPerMillionEachDay: 1.1944577161968462,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Musselshell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1510.9000647528596,
        lastWeekAverageDeathPerMillionEachDay: 30.83469519903795,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Park": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 240.876791521137,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Petroleum": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2053.388090349076,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Phillips": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2529.084471421346,
        lastWeekAverageDeathPerMillionEachDay: 36.12977816316209,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Pondera": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 507.5283369988158,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Powder River": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2972.651605231867,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Powell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 725.6894049346878,
        lastWeekAverageDeathPerMillionEachDay: 20.733982998133936,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Prairie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1857.0102135561744,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Ravalli": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 388.0746929644341,
        lastWeekAverageDeathPerMillionEachDay: 6.522263747301413,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Richland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 833.1019161344072,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Roosevelt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3816.793893129771,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Rosebud": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3021.1480362537764,
        lastWeekAverageDeathPerMillionEachDay: 15.984910244728974,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Sanders": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 247.66779493106583,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Sheridan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1208.8244182532485,
        lastWeekAverageDeathPerMillionEachDay: 43.172300651901736,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Silver Bow": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 973.7934984963482,
        lastWeekAverageDeathPerMillionEachDay: 12.274707964239683,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Stillwater": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 829.7033810412777,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Sweet Grass": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1337.971635001338,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Teton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 488.0429477794046,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Toole": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1689.1891891891892,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Treasure": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1436.7816091954023,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Valley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 946.4575446187129,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Wheatland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2822.2013170272817,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Wibaux": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4127.966976264191,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Montana - Yellowstone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 923.7445753254805,
        lastWeekAverageDeathPerMillionEachDay: 4.428305730227615,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 860.8870324905142,
        lastWeekAverageDeathPerMillionEachDay: 4.554957843865155,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Antelope": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 476.3416957764369,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Boone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 577.8120184899847,
        lastWeekAverageDeathPerMillionEachDay: 27.5148580233326,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Box Butte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 741.9085597700085,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Boyd": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1042.2094841063054,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Brown": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 338.409475465313,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Buffalo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 946.4548218852574,
        lastWeekAverageDeathPerMillionEachDay: 5.7535247531018685,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Burt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1238.5818238117354,
        lastWeekAverageDeathPerMillionEachDay: 66.35259770420011,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Butler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1122.754491017964,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Cass": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 342.88326729655597,
        lastWeekAverageDeathPerMillionEachDay: 10.885183088779554,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Cedar": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 952.1542489883361,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Chase": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 764.525993883792,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Cherry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 703.1112673580594,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Cheyenne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1010.1010101010099,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 644.8492664839594,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Colfax": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1587.449808572229,
        lastWeekAverageDeathPerMillionEachDay: 40.019743073249465,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Cuming": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1017.4089984173636,
        lastWeekAverageDeathPerMillionEachDay: 32.29869836245599,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Custer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1299.0628189663169,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Dakota": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2546.6893039049237,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Dawes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2095.7038071952497,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Dawson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1017.1646535282898,
        lastWeekAverageDeathPerMillionEachDay: 18.163654527290888,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Dixon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1064.58481192335,
        lastWeekAverageDeathPerMillionEachDay: 50.69451485349285,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Dodge": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1093.9422945439628,
        lastWeekAverageDeathPerMillionEachDay: -3.9069367662284384,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 815.6449808953541,
        lastWeekAverageDeathPerMillionEachDay: 12.252177824608324,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Dundy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2362.6698168930893,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Fillmore": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1098.4987184181616,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 671.3662302786171,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Frontier": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 380.6623524933384,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Furnas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 855.4319931565441,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Gage": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 464.8352159159577,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Garfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2031.4880650076177,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Gosper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 502.51256281407035,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3210.272873194221,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Hall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1450.621811484361,
        lastWeekAverageDeathPerMillionEachDay: 2.328445925336053,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Hamilton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1394.2513942513938,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Hitchcock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 724.112961622013,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Holt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 993.3445912387008,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Hooker": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1466.2756598240471,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Howard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1241.2723041117144,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 788.7990534411357,
        lastWeekAverageDeathPerMillionEachDay: -28.171394765754847,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Kearney": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 615.8583525789069,
        lastWeekAverageDeathPerMillionEachDay: 21.99494116353239,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Keith": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 248.94199651481208,
        lastWeekAverageDeathPerMillionEachDay: 17.78157117962943,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Kimball": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1101.3215859030836,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Knox": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 360.0576092174747,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Lancaster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 363.5337992415934,
        lastWeekAverageDeathPerMillionEachDay: 5.372420678447193,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 773.3287506444406,
        lastWeekAverageDeathPerMillionEachDay: 16.366746045384986,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1025.6702470155844,
        lastWeekAverageDeathPerMillionEachDay: 4.070120027839621,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Merrick": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1289.4906511927788,
        lastWeekAverageDeathPerMillionEachDay: 36.84259003407939,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Morrill": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1723.3950883239982,
        lastWeekAverageDeathPerMillionEachDay: 30.774912291499966,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Nance": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1705.029838022166,
        lastWeekAverageDeathPerMillionEachDay: 81.19189704867456,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Nemaha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1004.0160642570281,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Nuckolls": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 241.08003857280625,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Otoe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 124.90632025980516,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Pawnee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 382.7018752391886,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Perkins": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3804.911795226565,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Phelps": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 774.8505645339827,
        lastWeekAverageDeathPerMillionEachDay: 15.813276827224136,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Pierce": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2518.1869054280924,
        lastWeekAverageDeathPerMillionEachDay: 19.985610360540413,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Platte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1165.222587391694,
        lastWeekAverageDeathPerMillionEachDay: 21.341073029151907,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1726.4530980241707,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Red Willow": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 466.24393882879514,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Richardson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1525.7469802924347,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Rock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1473.8393515106854,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Saline": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 210.9111361079865,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Sarpy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 357.9136306331332,
        lastWeekAverageDeathPerMillionEachDay: 2.289426208740724,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Saunders": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 463.43498007229584,
        lastWeekAverageDeathPerMillionEachDay: 6.620499715318512,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Scotts Bluff": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1179.1790667639962,
        lastWeekAverageDeathPerMillionEachDay: 4.01081315225849,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Seward": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1099.2825734783614,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Sheridan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1715.592832634388,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Sherman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1332.8890369876706,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Stanton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1182.4324324324325,
        lastWeekAverageDeathPerMillionEachDay: 144.78764478764478,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Thayer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 399.76014391365175,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Thurston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1245.8471760797347,
        lastWeekAverageDeathPerMillionEachDay: 39.550704002531255,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Valley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 721.5007215007215,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 675.3823146316754,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 639.3180607352157,
        lastWeekAverageDeathPerMillionEachDay: 15.221858588933706,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - Webster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 860.3383997705764,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nebraska - York": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 438.6285547189122,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Carson City": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 769.0106588454111,
        lastWeekAverageDeathPerMillionEachDay: 22.993674184746514,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Churchill": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1204.3839576056846,
        lastWeekAverageDeathPerMillionEachDay: 5.735161702884213,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 953.8031909613692,
        lastWeekAverageDeathPerMillionEachDay: 10.27289018942138,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 245.37368367242615,
        lastWeekAverageDeathPerMillionEachDay: 5.842230563629194,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Elko": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 606.3132365758458,
        lastWeekAverageDeathPerMillionEachDay: 8.12026656128365,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Humboldt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 415.8992335571268,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Lander": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 723.065798987708,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 192.93845263360987,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Lyon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 591.2015301686665,
        lastWeekAverageDeathPerMillionEachDay: 19.872320341803913,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Mineral": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 887.9023307436183,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Nye": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 795.3055477935644,
        lastWeekAverageDeathPerMillionEachDay: 12.282711162834971,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Pershing": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 148.6988847583643,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Storey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 485.0836769342712,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - Washoe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 863.1677620626103,
        lastWeekAverageDeathPerMillionEachDay: 15.148609372808187,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Nevada - White Pine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 313.1524008350731,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Hampshire - Belknap": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 668.8090305531541,
        lastWeekAverageDeathPerMillionEachDay: 9.321380216768699,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Hampshire - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 61.33714986710283,
        lastWeekAverageDeathPerMillionEachDay: 2.9208166603382297,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Hampshire - Cheshire": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 78.85917066438851,
        lastWeekAverageDeathPerMillionEachDay: 3.7551986030661193,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Hampshire - Coos": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 475.23999619808,
        lastWeekAverageDeathPerMillionEachDay: 13.578285605659428,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Hampshire - Grafton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 89.00162427964311,
        lastWeekAverageDeathPerMillionEachDay: 6.357258877117364,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Hampshire - Hillsborough": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 925.6039805767041,
        lastWeekAverageDeathPerMillionEachDay: 8.564063476838491,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Hampshire - Merrimack": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 310.45438632415403,
        lastWeekAverageDeathPerMillionEachDay: 7.549042828550857,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Hampshire - Rockingham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 403.5264987781217,
        lastWeekAverageDeathPerMillionEachDay: 2.305865707303553,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Hampshire - Strafford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 153.10067134644385,
        lastWeekAverageDeathPerMillionEachDay: 1.0935762239031703,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Hampshire - Sullivan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 115.8855977379131,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Atlantic": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1262.942314256457,
        lastWeekAverageDeathPerMillionEachDay: 9.210647508519848,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Bergen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2409.3490466658513,
        lastWeekAverageDeathPerMillionEachDay: 6.742867195859144,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Burlington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1324.8036932832454,
        lastWeekAverageDeathPerMillionEachDay: 7.377841391165773,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Camden": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1494.6561599775703,
        lastWeekAverageDeathPerMillionEachDay: 6.769531579441722,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Cape May": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1379.8498462608243,
        lastWeekAverageDeathPerMillionEachDay: 6.208548239643753,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Cumberland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1350.926588509099,
        lastWeekAverageDeathPerMillionEachDay: 8.598542642561451,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Essex": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2917.488031540411,
        lastWeekAverageDeathPerMillionEachDay: 6.436818602405761,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Gloucester": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1189.839388827168,
        lastWeekAverageDeathPerMillionEachDay: 12.246185558122356,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Hudson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2468.8016347631074,
        lastWeekAverageDeathPerMillionEachDay: 6.161381016190199,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Hunterdon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1077.4215854178226,
        lastWeekAverageDeathPerMillionEachDay: 1.1486370846671883,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Mercer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1905.1247856734617,
        lastWeekAverageDeathPerMillionEachDay: 5.832014650020801,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Middlesex": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1930.763991069762,
        lastWeekAverageDeathPerMillionEachDay: 8.137916561768344,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Monmouth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1557.8664985980818,
        lastWeekAverageDeathPerMillionEachDay: 5.771586020295206,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Morris": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1888.8064329209406,
        lastWeekAverageDeathPerMillionEachDay: 9.003998065592675,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Ocean": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2065.2650094040378,
        lastWeekAverageDeathPerMillionEachDay: 10.352205560922494,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Passaic": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2773.8698273903706,
        lastWeekAverageDeathPerMillionEachDay: 5.693493077566441,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Salem": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1667.067404023403,
        lastWeekAverageDeathPerMillionEachDay: 4.5798555055587995,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Somerset": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1954.7994430493654,
        lastWeekAverageDeathPerMillionEachDay: 6.514550465616637,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Sussex": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1466.317407892489,
        lastWeekAverageDeathPerMillionEachDay: 3.050591001163292,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2703.377964234166,
        lastWeekAverageDeathPerMillionEachDay: 6.933055189430327,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Jersey - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1690.9382807527525,
        lastWeekAverageDeathPerMillionEachDay: 2.714186646473118,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Bernalillo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 697.961040815996,
        lastWeekAverageDeathPerMillionEachDay: 11.14886532948999,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Catron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 850.5812305075134,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Chaves": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1222.6263251566972,
        lastWeekAverageDeathPerMillionEachDay: 22.108975138457456,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Cibola": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2286.7853795688848,
        lastWeekAverageDeathPerMillionEachDay: 32.13281563797028,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Colfax": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 921.1958797420651,
        lastWeekAverageDeathPerMillionEachDay: 35.89074856137916,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Curry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 796.6662581198676,
        lastWeekAverageDeathPerMillionEachDay: 14.590957108422485,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Dona Ana": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1187.0116180480766,
        lastWeekAverageDeathPerMillionEachDay: 15.05861401826021,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Eddy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1043.448511802942,
        lastWeekAverageDeathPerMillionEachDay: 7.331019989247835,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 333.358026520483,
        lastWeekAverageDeathPerMillionEachDay: 5.2913972463568735,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Guadalupe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 697.6744186046511,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Hidalgo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1429.2520247737016,
        lastWeekAverageDeathPerMillionEachDay: 34.02981011365956,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Lea": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1055.297593921486,
        lastWeekAverageDeathPerMillionEachDay: 14.070634585619812,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 510.933987328837,
        lastWeekAverageDeathPerMillionEachDay: 7.299056961840529,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Luna": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1602.7668817748531,
        lastWeekAverageDeathPerMillionEachDay: 18.07631821550586,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - McKinley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4525.901326943826,
        lastWeekAverageDeathPerMillionEachDay: 26.022431335811472,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Otero": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 518.5953474588828,
        lastWeekAverageDeathPerMillionEachDay: 6.350147111741421,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Quay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 727.0083605961469,
        lastWeekAverageDeathPerMillionEachDay: 17.309722871336827,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Rio Arriba": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 899.2574702602709,
        lastWeekAverageDeathPerMillionEachDay: 22.022631924741326,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Roosevelt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1135.1351351351352,
        lastWeekAverageDeathPerMillionEachDay: 15.444015444015443,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - San Juan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2234.627857822811,
        lastWeekAverageDeathPerMillionEachDay: 16.134497168395747,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - San Miguel": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 109.98276936613267,
        lastWeekAverageDeathPerMillionEachDay: 5.237274731720603,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Sandoval": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 790.4707389538529,
        lastWeekAverageDeathPerMillionEachDay: 15.575778107465082,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Santa Fe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 405.6983998190984,
        lastWeekAverageDeathPerMillionEachDay: 12.351473530792225,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Sierra": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2502.085070892411,
        lastWeekAverageDeathPerMillionEachDay: 172.10108953228223,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Socorro": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2404.279617719541,
        lastWeekAverageDeathPerMillionEachDay: 60.106990442988526,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Taos": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1130.703175136754,
        lastWeekAverageDeathPerMillionEachDay: 17.462597299409325,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Torrance": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 194.036608240088,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1724.5627001724563,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New Mexico - Valencia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 678.0721886083871,
        lastWeekAverageDeathPerMillionEachDay: 9.314178414950373,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Albany": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 589.1864644229573,
        lastWeekAverageDeathPerMillionEachDay: 6.078907966268607,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Allegany": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1475.3422577075785,
        lastWeekAverageDeathPerMillionEachDay: 12.397834098383012,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Bronx": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3577.7569846996944,
        lastWeekAverageDeathPerMillionEachDay: 2.2160778665294574,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Broome": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 976.4394607534332,
        lastWeekAverageDeathPerMillionEachDay: 7.499535028828212,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Cattaraugus": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 433.5430981252545,
        lastWeekAverageDeathPerMillionEachDay: 7.5072397943767015,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Cayuga": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 182.82490597576262,
        lastWeekAverageDeathPerMillionEachDay: 1.8655602650588021,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Chautauqua": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 102.44044664034733,
        lastWeekAverageDeathPerMillionEachDay: -3.377157581549912,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Chemung": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 994.5360429447851,
        lastWeekAverageDeathPerMillionEachDay: 22.25295793163891,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Chenango": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 317.74948630499716,
        lastWeekAverageDeathPerMillionEachDay: 6.052371167714231,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Clinton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 62.123377026775174,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Columbia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 992.2470190545062,
        lastWeekAverageDeathPerMillionEachDay: 2.4025351550956566,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Cortland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 399.3190559256846,
        lastWeekAverageDeathPerMillionEachDay: 39.03118591754812,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Delaware": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 158.60428231562256,
        lastWeekAverageDeathPerMillionEachDay: -6.473644176147858,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Dutchess": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 730.7506678721221,
        lastWeekAverageDeathPerMillionEachDay: 4.369937548736943,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Erie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1150.5363001277892,
        lastWeekAverageDeathPerMillionEachDay: 14.305898041864657,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Essex": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 433.7806696489088,
        lastWeekAverageDeathPerMillionEachDay: 7.7460833865876575,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 179.92083483267365,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Fulton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 543.2441039282168,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Genesee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 523.7430167597765,
        lastWeekAverageDeathPerMillionEachDay: 27.4341580207502,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 381.4529117572264,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Hamilton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 226.44927536231884,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Herkimer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 293.54686149480585,
        lastWeekAverageDeathPerMillionEachDay: 11.648684979952614,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 54.6278930021669,
        lastWeekAverageDeathPerMillionEachDay: 1.3006641190992119,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Kings": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2974.3314492775703,
        lastWeekAverageDeathPerMillionEachDay: 2.8460899829854047,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Lewis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 418.31457255856407,
        lastWeekAverageDeathPerMillionEachDay: 10.865313572949715,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Livingston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 333.78898178465835,
        lastWeekAverageDeathPerMillionEachDay: 4.541346690947733,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 563.8488321281064,
        lastWeekAverageDeathPerMillionEachDay: 34.23367909349217,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 567.5613734715612,
        lastWeekAverageDeathPerMillionEachDay: 11.170193302120989,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 589.1794152902216,
        lastWeekAverageDeathPerMillionEachDay: 26.12125486508372,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Nassau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1713.4342085481574,
        lastWeekAverageDeathPerMillionEachDay: 2.947843799652744,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - New York": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2006.5008663319222,
        lastWeekAverageDeathPerMillionEachDay: 1.8419530596682272,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Niagara": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 745.40928225687,
        lastWeekAverageDeathPerMillionEachDay: 15.017403122391155,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Oneida": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 922.723038776233,
        lastWeekAverageDeathPerMillionEachDay: 14.368740617368557,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Onondaga": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 705.7117048257653,
        lastWeekAverageDeathPerMillionEachDay: 14.889741464455707,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Ontario": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 419.03130892627786,
        lastWeekAverageDeathPerMillionEachDay: 11.712055218436339,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Orange": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1485.9458616927316,
        lastWeekAverageDeathPerMillionEachDay: 5.937845601169756,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Orleans": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1462.1332275971452,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Oswego": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 196.37307469007203,
        lastWeekAverageDeathPerMillionEachDay: 4.878834153790609,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Otsego": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 134.46960146571865,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Putnam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 671.2774613506917,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Queens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3309.4365306066306,
        lastWeekAverageDeathPerMillionEachDay: 3.105785723856605,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Rensselaer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 441.04489837065415,
        lastWeekAverageDeathPerMillionEachDay: 2.7002748879835967,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Richmond": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2566.4558756508027,
        lastWeekAverageDeathPerMillionEachDay: 11.401136693328326,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Rockland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2268.3393239182415,
        lastWeekAverageDeathPerMillionEachDay: 10.085405847693709,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Saratoga": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 121.81168783144743,
        lastWeekAverageDeathPerMillionEachDay: 2.485952812886682,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Schenectady": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 457.1825961532271,
        lastWeekAverageDeathPerMillionEachDay: 8.278960493720412,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Schoharie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 96.77731539727087,
        lastWeekAverageDeathPerMillionEachDay: 4.608443590346232,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Schuyler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 449.261526366036,
        lastWeekAverageDeathPerMillionEachDay: 24.06758176960907,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Seneca": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 558.5606773283162,
        lastWeekAverageDeathPerMillionEachDay: 29.39793038570085,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - St. Lawrence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 297.0113235567106,
        lastWeekAverageDeathPerMillionEachDay: 9.281603861147206,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Steuben": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1184.747166567064,
        lastWeekAverageDeathPerMillionEachDay: 20.968976399417063,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Suffolk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1470.9457734350715,
        lastWeekAverageDeathPerMillionEachDay: 5.708090017934045,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Sullivan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 530.2789267154523,
        lastWeekAverageDeathPerMillionEachDay: -15.15082647758435,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Tioga": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1120.2622243428834,
        lastWeekAverageDeathPerMillionEachDay: 5.927313356311552,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Tompkins": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 97.86651008025056,
        lastWeekAverageDeathPerMillionEachDay: 5.592372004585746,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Ulster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 737.7247667156607,
        lastWeekAverageDeathPerMillionEachDay: 12.067471647475365,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 469.16051545101965,
        lastWeekAverageDeathPerMillionEachDay: -11.170488463119515,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 228.743219397425,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 322.51607019729084,
        lastWeekAverageDeathPerMillionEachDay: 6.354996457089475,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Westchester": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1659.938026224127,
        lastWeekAverageDeathPerMillionEachDay: 6.496822020446681,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Wyoming": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 376.3265510926015,
        lastWeekAverageDeathPerMillionEachDay: 10.752187174074328,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - New York - Yates": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 361.2571749688917,
        lastWeekAverageDeathPerMillionEachDay: 11.468481745044182,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Alamance": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 672.5306620887387,
        lastWeekAverageDeathPerMillionEachDay: 0.8427702532440334,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Alexander": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 693.3888044376883,
        lastWeekAverageDeathPerMillionEachDay: 19.049142979057372,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Alleghany": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 179.58157493041216,
        lastWeekAverageDeathPerMillionEachDay: 12.827255352172296,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Anson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 777.2232676102432,
        lastWeekAverageDeathPerMillionEachDay: 5.843783966994309,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Ashe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 624.9310737786274,
        lastWeekAverageDeathPerMillionEachDay: 73.52130279748556,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Avery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 854.3600842968618,
        lastWeekAverageDeathPerMillionEachDay: 16.27352541517832,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Beaufort": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1234.2001106524237,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Bertie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1425.027708872117,
        lastWeekAverageDeathPerMillionEachDay: 7.539829147471519,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Bladen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 733.4515005195282,
        lastWeekAverageDeathPerMillionEachDay: 13.097348223563001,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Brunswick": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 490.1274331326145,
        lastWeekAverageDeathPerMillionEachDay: 4.001040270470322,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Buncombe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 543.6634493531554,
        lastWeekAverageDeathPerMillionEachDay: 3.8286158405151793,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Burke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 806.7635519699398,
        lastWeekAverageDeathPerMillionEachDay: 3.1575872875535804,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Cabarrus": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 572.8726328579414,
        lastWeekAverageDeathPerMillionEachDay: 7.25990663760064,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Caldwell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 328.5550877363771,
        lastWeekAverageDeathPerMillionEachDay: 3.476773415199758,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Camden": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 368.0868685009662,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Carteret": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 230.30529846127274,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Caswell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 353.91966023712615,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Catawba": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 802.2513177604654,
        lastWeekAverageDeathPerMillionEachDay: 20.593504808583376,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Chatham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 872.8346985363236,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Cherokee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 524.2555571089054,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Chowan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1362.6909560352865,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 445.1963315822277,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Cleveland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1337.4580130070344,
        lastWeekAverageDeathPerMillionEachDay: 17.50217683324363,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Columbus": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1423.218274843266,
        lastWeekAverageDeathPerMillionEachDay: 12.868158000391194,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Craven": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 773.4557808476685,
        lastWeekAverageDeathPerMillionEachDay: 2.797308429828819,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Cumberland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 390.4515229099667,
        lastWeekAverageDeathPerMillionEachDay: 2.5547536940673936,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Currituck": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 360.19162194287367,
        lastWeekAverageDeathPerMillionEachDay: 20.582378396735635,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Dare": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 135.1022724202221,
        lastWeekAverageDeathPerMillionEachDay: 3.86006492629206,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Davidson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 429.57120441026433,
        lastWeekAverageDeathPerMillionEachDay: 11.932533455840677,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Davie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 303.4122205106661,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Duplin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1106.5524931478863,
        lastWeekAverageDeathPerMillionEachDay: 4.863967002847851,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Durham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 407.4802169909919,
        lastWeekAverageDeathPerMillionEachDay: 1.3330868603849244,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Edgecombe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1534.8150450730495,
        lastWeekAverageDeathPerMillionEachDay: 8.326302233669345,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Forsyth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 523.1562013628219,
        lastWeekAverageDeathPerMillionEachDay: 5.605245014601663,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 530.9607519552271,
        lastWeekAverageDeathPerMillionEachDay: 2.0500415133406453,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Gaston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 966.467583252052,
        lastWeekAverageDeathPerMillionEachDay: 14.633808041341142,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Gates": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1037.8827192527244,
        lastWeekAverageDeathPerMillionEachDay: 24.71149331554106,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Graham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1303.1631323302925,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Granville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 992.6707807355689,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1281.5036309269542,
        lastWeekAverageDeathPerMillionEachDay: -6.7804424916770065,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Guilford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 545.4470990777661,
        lastWeekAverageDeathPerMillionEachDay: 4.521014473097037,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Halifax": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 879.8240351929614,
        lastWeekAverageDeathPerMillionEachDay: 8.56971462850287,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Harnett": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 654.5272695181501,
        lastWeekAverageDeathPerMillionEachDay: 5.253027845250001,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Haywood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 898.6311921305584,
        lastWeekAverageDeathPerMillionEachDay: 6.877279531611415,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Henderson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 757.9822342590937,
        lastWeekAverageDeathPerMillionEachDay: 4.866659610010232,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Hertford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1858.343540144444,
        lastWeekAverageDeathPerMillionEachDay: 12.067165845093792,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Hoke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 615.5628779375023,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Hyde": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1012.7607859023695,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Iredell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 352.0235855802339,
        lastWeekAverageDeathPerMillionEachDay: 2.357300796296209,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 364.1494833629205,
        lastWeekAverageDeathPerMillionEachDay: 19.508008037299312,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Johnston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 429.9246676443472,
        lastWeekAverageDeathPerMillionEachDay: 4.776940751603858,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Jones": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1592.5257458328908,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Lee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 615.0957445086518,
        lastWeekAverageDeathPerMillionEachDay: 4.624780033899638,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Lenoir": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1108.1520670610735,
        lastWeekAverageDeathPerMillionEachDay: 7.660037329915254,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 232.25836420434098,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Macon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 250.99001617491209,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 873.3624454148471,
        lastWeekAverageDeathPerMillionEachDay: 13.133269855862363,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Martin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1069.51871657754,
        lastWeekAverageDeathPerMillionEachDay: 19.098548510313215,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - McDowell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 830.4921758895009,
        lastWeekAverageDeathPerMillionEachDay: 6.244302074357149,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Mecklenburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 456.6103123682855,
        lastWeekAverageDeathPerMillionEachDay: 2.959153898132028,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Mitchell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 334.1352579524191,
        lastWeekAverageDeathPerMillionEachDay: -9.546721655783404,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1692.8568799911677,
        lastWeekAverageDeathPerMillionEachDay: 10.51463900615632,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Moore": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 723.6320380650277,
        lastWeekAverageDeathPerMillionEachDay: 5.664438654129375,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Nash": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1230.142738976436,
        lastWeekAverageDeathPerMillionEachDay: 4.544862336119838,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - New Hanover": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 302.8067197502485,
        lastWeekAverageDeathPerMillionEachDay: 1.8278071614703126,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Northampton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1385.8235384694349,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Onslow": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 287.96895997736664,
        lastWeekAverageDeathPerMillionEachDay: 5.7738137338820374,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Orange": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 417.57590452328986,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Pamlico": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 392.89643250039285,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Pasquotank": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 954.1984732824428,
        lastWeekAverageDeathPerMillionEachDay: 7.174424611146186,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Pender": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 333.0161750713606,
        lastWeekAverageDeathPerMillionEachDay: 4.530832313896062,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Perquimans": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 371.3882492757929,
        lastWeekAverageDeathPerMillionEachDay: 10.611092836451224,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Person": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 531.7801975183592,
        lastWeekAverageDeathPerMillionEachDay: 18.087761820352352,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Pitt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 381.7596352812296,
        lastWeekAverageDeathPerMillionEachDay: 7.903926196298749,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 627.2920285659139,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Randolph": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 758.6989357333279,
        lastWeekAverageDeathPerMillionEachDay: 6.960540694801173,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Richmond": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 981.5075063017243,
        lastWeekAverageDeathPerMillionEachDay: 9.560138048393418,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Robeson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 888.0382775119618,
        lastWeekAverageDeathPerMillionEachDay: 2.1872863978127137,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Rockingham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 274.69508845181855,
        lastWeekAverageDeathPerMillionEachDay: 3.1393724394493545,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Rowan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1097.9111536512585,
        lastWeekAverageDeathPerMillionEachDay: 7.03789201058499,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Rutherford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1402.3780751614972,
        lastWeekAverageDeathPerMillionEachDay: 29.837831386414834,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Sampson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 897.1997922274164,
        lastWeekAverageDeathPerMillionEachDay: 8.99448413260568,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Scotland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1464.5492921345092,
        lastWeekAverageDeathPerMillionEachDay: 20.51189484782226,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Stanly": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1241.9195618253036,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Stokes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 504.4855344256541,
        lastWeekAverageDeathPerMillionEachDay: 3.1334505243829445,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Surry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 738.336374907708,
        lastWeekAverageDeathPerMillionEachDay: 1.9901249997512342,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Swain": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 420.4330460374186,
        lastWeekAverageDeathPerMillionEachDay: 10.010310619938537,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Transylvania": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 290.82448742184096,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Tyrrell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 747.0119521912351,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 371.0513259873509,
        lastWeekAverageDeathPerMillionEachDay: 2.9779400159498466,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Vance": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1481.980464802964,
        lastWeekAverageDeathPerMillionEachDay: 16.0387496190797,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Wake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 303.1227035307049,
        lastWeekAverageDeathPerMillionEachDay: 1.7989477954344504,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 456.1350159647256,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 863.5578583765111,
        lastWeekAverageDeathPerMillionEachDay: 24.673081667900316,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Watauga": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 249.2123110881678,
        lastWeekAverageDeathPerMillionEachDay: 15.257896597234762,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1096.3932722060244,
        lastWeekAverageDeathPerMillionEachDay: 9.281636166823487,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Wilkes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 979.3603461381047,
        lastWeekAverageDeathPerMillionEachDay: 10.440941856482992,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Wilson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1222.4789428002102,
        lastWeekAverageDeathPerMillionEachDay: 19.210383386860446,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Yadkin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 637.1625029866992,
        lastWeekAverageDeathPerMillionEachDay: 11.3779018390482,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Carolina - Yancey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 332.06043499916984,
        lastWeekAverageDeathPerMillionEachDay: 15.812401666627133,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 902.5270758122743,
        lastWeekAverageDeathPerMillionEachDay: 128.93243940175347,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Barnes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2208.353336533846,
        lastWeekAverageDeathPerMillionEachDay: 13.716480351141898,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Benson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2341.9203747072597,
        lastWeekAverageDeathPerMillionEachDay: 20.91000334560053,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Bottineau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3024.5144858325375,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Bowman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1322.751322751323,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Burke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 472.81323877068564,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Burleigh": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1673.1851170183843,
        lastWeekAverageDeathPerMillionEachDay: 17.926983396625545,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Cass": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 879.4929723014685,
        lastWeekAverageDeathPerMillionEachDay: 7.852615824120254,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Cavalier": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1063.264221158958,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Dickey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 6568.14449917898,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Divide": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 883.39222614841,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Dunn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1582.2784810126582,
        lastWeekAverageDeathPerMillionEachDay: 32.2913975716869,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Eddy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2623.5242675994755,
        lastWeekAverageDeathPerMillionEachDay: 62.46486351427322,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Emmons": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3085.4674483184194,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Foster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 5919.003115264798,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Grand Forks": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 878.3170868669997,
        lastWeekAverageDeathPerMillionEachDay: 12.341692087124118,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3078.2761653474054,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Griggs": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 896.4589870013446,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Hettinger": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1200.4801920768307,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Kidder": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3225.8064516129025,
        lastWeekAverageDeathPerMillionEachDay: 57.60368663594469,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - LaMoure": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 8.846807574194303,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Logan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3783.783783783784,
        lastWeekAverageDeathPerMillionEachDay: 77.22007722007721,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - McHenry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3307.22367275892,
        lastWeekAverageDeathPerMillionEachDay: 24.866343404202407,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - McIntosh": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2803.364036844213,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - McKenzie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 665.6017039403623,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - McLean": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2433.8624338624336,
        lastWeekAverageDeathPerMillionEachDay: 75.58578987150415,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Mercer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1099.3037742762915,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Morton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2678.2298176253025,
        lastWeekAverageDeathPerMillionEachDay: 18.219250460036072,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Mountrail": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1422.475106685633,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Nelson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3473.428273706148,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Oliver": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1020.9290454313426,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Pembina": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1323.334803705338,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Pierce": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 5283.018867924528,
        lastWeekAverageDeathPerMillionEachDay: 35.93890386343216,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Ramsey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1389.009462626964,
        lastWeekAverageDeathPerMillionEachDay: 37.20561060607939,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Ransom": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2108.0873898045224,
        lastWeekAverageDeathPerMillionEachDay: 27.377758309149645,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Renville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 5156.854318865491,
        lastWeekAverageDeathPerMillionEachDay: 61.39112284363679,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Richland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 927.2423811584348,
        lastWeekAverageDeathPerMillionEachDay: 17.661759641113044,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Rolette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1410.8352144469527,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Sargent": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1282.7090815802978,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Sheridan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1520.9125475285173,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Sioux": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2364.066193853427,
        lastWeekAverageDeathPerMillionEachDay: 33.7723741979061,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Stark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1429.070469052685,
        lastWeekAverageDeathPerMillionEachDay: 22.683658238931507,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Steele": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 529.1005291005291,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Stutsman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3380.9891808346215,
        lastWeekAverageDeathPerMillionEachDay: 27.59991168028262,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Towner": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4568.296025582458,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Traill": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2115.4803384768543,
        lastWeekAverageDeathPerMillionEachDay: 35.55429140297234,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Walsh": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1973.4987313222439,
        lastWeekAverageDeathPerMillionEachDay: 40.27548431269885,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Ward": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2439.3489155985267,
        lastWeekAverageDeathPerMillionEachDay: 14.783932821809254,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Wells": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1564.9452269170579,
        lastWeekAverageDeathPerMillionEachDay: 74.52120128176466,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - North Dakota - Williams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 718.2952459496129,
        lastWeekAverageDeathPerMillionEachDay: 3.800503946823348,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Northern Mariana Islands": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 36.26867836936022,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 505.45165715936173,
        lastWeekAverageDeathPerMillionEachDay: 5.157669971013895,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Allen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1025.8815253392734,
        lastWeekAverageDeathPerMillionEachDay: 4.187271531997034,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Ashland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 673.0984967466907,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Ashtabula": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 668.4423237111915,
        lastWeekAverageDeathPerMillionEachDay: 4.407312024469395,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Athens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 91.84563809757068,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Auglaize": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1073.2433853162781,
        lastWeekAverageDeathPerMillionEachDay: 9.38696838469048,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Belmont": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 477.5691729098887,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Brown": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 230.24498065942163,
        lastWeekAverageDeathPerMillionEachDay: 3.289214009420309,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Butler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 467.1994654611703,
        lastWeekAverageDeathPerMillionEachDay: 3.7286469709590606,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 445.86460578137775,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Champaign": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 514.3371480005143,
        lastWeekAverageDeathPerMillionEachDay: 29.39069417145796,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1282.787527128719,
        lastWeekAverageDeathPerMillionEachDay: 23.439639200026427,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Clermont": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 314.8797643730502,
        lastWeekAverageDeathPerMillionEachDay: 4.152260629095167,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Clinton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 857.796416317194,
        lastWeekAverageDeathPerMillionEachDay: 40.84744839605686,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Columbiana": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1138.5608982852882,
        lastWeekAverageDeathPerMillionEachDay: 8.413011563684394,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Coshocton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 409.8360655737705,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Crawford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 795.2957054031907,
        lastWeekAverageDeathPerMillionEachDay: 17.21419275764482,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Cuyahoga": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 676.0739454865792,
        lastWeekAverageDeathPerMillionEachDay: 1.272337621959345,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Darke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1447.77258231761,
        lastWeekAverageDeathPerMillionEachDay: 5.589855530183822,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Defiance": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1654.1077007902957,
        lastWeekAverageDeathPerMillionEachDay: 45.00973335483798,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Delaware": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 262.93521754303777,
        lastWeekAverageDeathPerMillionEachDay: 4.7806403189643225,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Erie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1225.3251824522663,
        lastWeekAverageDeathPerMillionEachDay: 13.465111895079849,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Fairfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 444.23572416769275,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Fayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 911.4811568799299,
        lastWeekAverageDeathPerMillionEachDay: 20.032552898459997,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 526.2934059157506,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Fulton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 878.3174286663818,
        lastWeekAverageDeathPerMillionEachDay: 16.95593491633942,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Gallia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 602.0469596628536,
        lastWeekAverageDeathPerMillionEachDay: 14.334451420544132,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Geauga": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 544.5867014063151,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 603.7753718841933,
        lastWeekAverageDeathPerMillionEachDay: 11.838732782043007,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Guernsey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 411.5755627009646,
        lastWeekAverageDeathPerMillionEachDay: 3.674781809830041,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Hamilton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 495.42920683618894,
        lastWeekAverageDeathPerMillionEachDay: 2.097054843751064,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Hancock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 910.4944380665848,
        lastWeekAverageDeathPerMillionEachDay: 7.540326609247079,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Hardin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 956.4801530368246,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Harrison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 199.46808510638303,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Henry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1296.0082944530845,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Highland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 417.0431639674706,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Hocking": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 778.3753184262666,
        lastWeekAverageDeathPerMillionEachDay: 15.16315555375844,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Holmes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1137.3976342129208,
        lastWeekAverageDeathPerMillionEachDay: 12.998830105290523,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Huron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 635.0187073078639,
        lastWeekAverageDeathPerMillionEachDay: 9.807238722901372,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 617.0363742942646,
        lastWeekAverageDeathPerMillionEachDay: 30.851818714713236,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 642.9391504018371,
        lastWeekAverageDeathPerMillionEachDay: 21.868678585096497,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Knox": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 353.0053592631815,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 425.8111049798174,
        lastWeekAverageDeathPerMillionEachDay: 2.482863585888148,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Lawrence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 756.7731194187984,
        lastWeekAverageDeathPerMillionEachDay: 4.8049086947225295,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Licking": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 582.3749590075879,
        lastWeekAverageDeathPerMillionEachDay: 5.6541258156076495,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Logan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 459.8003152916449,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Lorain": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 461.538958083871,
        lastWeekAverageDeathPerMillionEachDay: 9.682635484277014,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Lucas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1176.6134077899278,
        lastWeekAverageDeathPerMillionEachDay: 8.671187245617382,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 469.4730723659207,
        lastWeekAverageDeathPerMillionEachDay: 6.3873887396723905,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Mahoning": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1359.9611689544042,
        lastWeekAverageDeathPerMillionEachDay: 0.6246950707186055,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1428.7250549214202,
        lastWeekAverageDeathPerMillionEachDay: 28.530607855573674,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Medina": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 506.26995871952647,
        lastWeekAverageDeathPerMillionEachDay: 15.100673807960758,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Meigs": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 611.166892216353,
        lastWeekAverageDeathPerMillionEachDay: 6.23639685935054,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Mercer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1845.9146993102108,
        lastWeekAverageDeathPerMillionEachDay: 20.818586834325686,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Miami": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 654.2851000588856,
        lastWeekAverageDeathPerMillionEachDay: 1.335275714405889,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1757.7266735022704,
        lastWeekAverageDeathPerMillionEachDay: 20.925317541693694,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 613.1426948561841,
        lastWeekAverageDeathPerMillionEachDay: 6.717163615865296,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Morgan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1654.259718775848,
        lastWeekAverageDeathPerMillionEachDay: 59.08070424199456,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Morrow": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 56.61231884057971,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Muskingum": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 255.17601345473525,
        lastWeekAverageDeathPerMillionEachDay: 4.9709613010662705,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Noble": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2079.866888519135,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Ottawa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 814.3121529919803,
        lastWeekAverageDeathPerMillionEachDay: 3.525160835463118,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Paulding": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 964.0102827763495,
        lastWeekAverageDeathPerMillionEachDay: 7.650875260129758,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Perry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 415.1214922233907,
        lastWeekAverageDeathPerMillionEachDay: 3.9535380211751496,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Pickaway": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 855.3295584788818,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Pike": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 180.03744778914012,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Portage": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 547.8069257567737,
        lastWeekAverageDeathPerMillionEachDay: 0.8793048567524457,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Preble": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 709.3586419451102,
        lastWeekAverageDeathPerMillionEachDay: 10.483132639582909,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Putnam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1831.0150320427626,
        lastWeekAverageDeathPerMillionEachDay: 16.875714581039286,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Richland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 486.98350859237013,
        lastWeekAverageDeathPerMillionEachDay: 11.791368246788622,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Ross": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 991.3129679388514,
        lastWeekAverageDeathPerMillionEachDay: 18.63370240486563,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Sandusky": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 803.171673673058,
        lastWeekAverageDeathPerMillionEachDay: 17.08875901432038,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Scioto": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 464.7210346018004,
        lastWeekAverageDeathPerMillionEachDay: 7.587282197580414,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Seneca": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 525.5717858566819,
        lastWeekAverageDeathPerMillionEachDay: 15.534141453892078,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Shelby": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 370.44659394937236,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Stark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 696.1571048498945,
        lastWeekAverageDeathPerMillionEachDay: 2.312814301826892,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Summit": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 905.7083656030447,
        lastWeekAverageDeathPerMillionEachDay: 11.35436143467374,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Trumbull": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1161.768717104266,
        lastWeekAverageDeathPerMillionEachDay: 41.85253763481207,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Tuscarawas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1206.692249991847,
        lastWeekAverageDeathPerMillionEachDay: 21.7422027025558,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 305.1468095205804,
        lastWeekAverageDeathPerMillionEachDay: 4.843600151120324,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Van Wert": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1273.2095490716179,
        lastWeekAverageDeathPerMillionEachDay: 20.20967538208917,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Vinton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 534.9636988918611,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 447.56651690948934,
        lastWeekAverageDeathPerMillionEachDay: 6.0893403661155014,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 534.1256196691761,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1149.4252873563219,
        lastWeekAverageDeathPerMillionEachDay: 9.876909021321778,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Williams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 953.886405756023,
        lastWeekAverageDeathPerMillionEachDay: 62.29462241671987,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Wood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1016.6874335904354,
        lastWeekAverageDeathPerMillionEachDay: 14.19649477623594,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Ohio - Wyandot": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1240.1249311041704,
        lastWeekAverageDeathPerMillionEachDay: 26.24603028791895,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Adair": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 585.7438947463278,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Alfalfa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 526.1311820413889,
        lastWeekAverageDeathPerMillionEachDay: 25.053865811494706,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Atoka": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 218.05494984736154,
        lastWeekAverageDeathPerMillionEachDay: 10.38356904035055,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Beaver": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 376.5769158350593,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Beckham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 914.9549384692804,
        lastWeekAverageDeathPerMillionEachDay: 13.070784835275433,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Blaine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 318.16735602927145,
        lastWeekAverageDeathPerMillionEachDay: 15.150826477584355,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Bryan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 562.5585998541515,
        lastWeekAverageDeathPerMillionEachDay: 5.953001056657687,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Caddo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1355.9557749808775,
        lastWeekAverageDeathPerMillionEachDay: 9.93374194125185,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Canadian": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 303.4266988523728,
        lastWeekAverageDeathPerMillionEachDay: 3.8530374457444165,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Carter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 332.5642784394421,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Cherokee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 308.2804118626303,
        lastWeekAverageDeathPerMillionEachDay: 5.87200784500248,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Choctaw": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 408.9422028353326,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Cimarron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 467.9457182966777,
        lastWeekAverageDeathPerMillionEachDay: 66.84938832809681,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Cleveland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 499.9753533276529,
        lastWeekAverageDeathPerMillionEachDay: 2.011973252827577,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Coal": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 727.9344858962693,
        lastWeekAverageDeathPerMillionEachDay: 25.997660210581042,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Comanche": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 414.08210419962074,
        lastWeekAverageDeathPerMillionEachDay: 4.732366905138522,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Cotton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1588.4221673138013,
        lastWeekAverageDeathPerMillionEachDay: 50.42610054964449,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Craig": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 353.5567812190638,
        lastWeekAverageDeathPerMillionEachDay: 10.10162232054468,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Creek": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 796.9575794860324,
        lastWeekAverageDeathPerMillionEachDay: 3.994774834516453,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Custer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 930.9381788090884,
        lastWeekAverageDeathPerMillionEachDay: 39.40479063742173,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Delaware": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 999.790741472715,
        lastWeekAverageDeathPerMillionEachDay: 13.286255700634086,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Dewey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 613.3714986710283,
        lastWeekAverageDeathPerMillionEachDay: 29.208166603382303,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Garfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 671.514675052411,
        lastWeekAverageDeathPerMillionEachDay: 2.3397723869421982,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Garvin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 649.5615459564792,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Grady": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 608.9479528602643,
        lastWeekAverageDeathPerMillionEachDay: 5.117209687901381,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1153.934918070621,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Greer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1575.6302521008404,
        lastWeekAverageDeathPerMillionEachDay: 25.010004001600638,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Harper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 813.4490238611714,
        lastWeekAverageDeathPerMillionEachDay: 38.73566780291293,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Haskell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 554.3676249307041,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Hughes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 602.4550041418782,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1426.8242967794536,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 333.22225924691764,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Johnston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 451.05999097880016,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Kay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 643.1163581239379,
        lastWeekAverageDeathPerMillionEachDay: 3.281205908795601,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Kingfisher": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 507.453219156359,
        lastWeekAverageDeathPerMillionEachDay: 9.061664627792124,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Kiowa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1148.3693155718877,
        lastWeekAverageDeathPerMillionEachDay: 32.8105518734825,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Latimer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 397.10116152089745,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Le Flore": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 561.6512546887851,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 860.1657252630674,
        lastWeekAverageDeathPerMillionEachDay: 8.192054526314926,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Logan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 124.97136072983277,
        lastWeekAverageDeathPerMillionEachDay: 2.975508588805542,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Love": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 97.53242953281966,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Major": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 524.3151133831433,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Marshall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 236.2530269919084,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Mayes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 535.2798053527981,
        lastWeekAverageDeathPerMillionEachDay: 3.4758428919012863,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - McClain": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 518.8516084399861,
        lastWeekAverageDeathPerMillionEachDay: 3.529602778503307,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - McCurtain": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1522.9044834307995,
        lastWeekAverageDeathPerMillionEachDay: 8.702311333890282,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - McIntosh": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 714.4315166360482,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Murray": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 426.3483265828181,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Muskogee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 705.9134961836552,
        lastWeekAverageDeathPerMillionEachDay: 12.605598146136698,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Noble": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 449.195939268709,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Nowata": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 793.9658594680429,
        lastWeekAverageDeathPerMillionEachDay: 28.355923552430102,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Okfuskee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1250.7295922621527,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Oklahoma": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 499.10086602778404,
        lastWeekAverageDeathPerMillionEachDay: 4.478650987327567,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Okmulgee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 649.9415052645262,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Osage": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 511.04060643485315,
        lastWeekAverageDeathPerMillionEachDay: 9.125725114908091,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Ottawa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 803.1612426510746,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Pawnee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 549.5847581827063,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Payne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 305.68326322997166,
        lastWeekAverageDeathPerMillionEachDay: 1.7467615041712665,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Pittsburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 526.8703898840886,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Pontotoc": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 444.0497335701599,
        lastWeekAverageDeathPerMillionEachDay: 3.731510366135797,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Pottawatomie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 358.1661891117479,
        lastWeekAverageDeathPerMillionEachDay: 1.9679460940205928,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Pushmataha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 540.7354001441961,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Roger Mills": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1674.5743790120016,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Rogers": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 865.2483803631881,
        lastWeekAverageDeathPerMillionEachDay: 6.1803455740227715,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Seminole": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 618.3527083848628,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Sequoyah": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 384.90221078207315,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Stephens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 463.5746239250863,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Texas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 750.6380423359856,
        lastWeekAverageDeathPerMillionEachDay: 14.297867473066391,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Tillman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1103.448275862069,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Tulsa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 537.1789204852414,
        lastWeekAverageDeathPerMillionEachDay: 8.331754685077213,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Wagoner": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 516.6750728880907,
        lastWeekAverageDeathPerMillionEachDay: 1.757398207102349,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1028.5869544122497,
        lastWeekAverageDeathPerMillionEachDay: 5.544943150470349,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Washita": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 274.82594356907293,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Woods": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 454.9073126350507,
        lastWeekAverageDeathPerMillionEachDay: 16.246689736966093,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oklahoma - Woodward": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 494.7800702587699,
        lastWeekAverageDeathPerMillionEachDay: 14.136573435964854,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Baker": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 248.0774001488464,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Benton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 96.71907407606419,
        lastWeekAverageDeathPerMillionEachDay: 1.5352233980327648,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Clackamas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 255.8663946990223,
        lastWeekAverageDeathPerMillionEachDay: 4.099328085965644,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Clatsop": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 49.721559268098645,
        lastWeekAverageDeathPerMillionEachDay: 3.5515399477213316,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Columbia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 171.906635596134,
        lastWeekAverageDeathPerMillionEachDay: 5.457353510988381,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Coos": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 93.04200846682275,
        lastWeekAverageDeathPerMillionEachDay: 4.430571831753464,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Crook": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 245.86133420750699,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Curry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 87.24100327153762,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Deschutes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 96.1090990024887,
        lastWeekAverageDeathPerMillionEachDay: 1.4452496090599805,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 324.38277167057123,
        lastWeekAverageDeathPerMillionEachDay: 9.010632546404757,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Gilliam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 523.0125523012554,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 138.90818169190163,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Harney": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 135.26308670363858,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Hood River": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 342.1435292105038,
        lastWeekAverageDeathPerMillionEachDay: 24.438823515035985,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 262.5099572742415,
        lastWeekAverageDeathPerMillionEachDay: 5.172609995551556,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 567.7670532889933,
        lastWeekAverageDeathPerMillionEachDay: 11.587082720183536,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Josephine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 205.7448535210946,
        lastWeekAverageDeathPerMillionEachDay: 13.063165302926642,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Klamath": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 205.16427796828748,
        lastWeekAverageDeathPerMillionEachDay: 6.280539121478188,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 508.32380226204094,
        lastWeekAverageDeathPerMillionEachDay: 54.46326452807582,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Lane": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 209.38735876168315,
        lastWeekAverageDeathPerMillionEachDay: 4.4868719734646385,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 320.24338497257924,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Linn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 238.92284333597945,
        lastWeekAverageDeathPerMillionEachDay: 4.404107711262294,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Malheur": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1602.826207844035,
        lastWeekAverageDeathPerMillionEachDay: 18.691850820338598,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 560.63803483431,
        lastWeekAverageDeathPerMillionEachDay: 6.160857525651759,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Morrow": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 603.2922520037922,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Multnomah": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 449.03457566232606,
        lastWeekAverageDeathPerMillionEachDay: 10.544843264085936,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 278.79421502003834,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Umatilla": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 692.7517639512507,
        lastWeekAverageDeathPerMillionEachDay: 3.665353248419316,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 484.44196012670017,
        lastWeekAverageDeathPerMillionEachDay: 15.970614070110994,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Wallowa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 416.20421753607104,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Wasco": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 749.5689978262499,
        lastWeekAverageDeathPerMillionEachDay: 5.354064270187499,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 222.7423237011131,
        lastWeekAverageDeathPerMillionEachDay: 4.274372949488311,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Oregon - Yamhill": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 289.44911297852474,
        lastWeekAverageDeathPerMillionEachDay: 16.00640256102441,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 621.3049345202847,
        lastWeekAverageDeathPerMillionEachDay: 13.868413716970643,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Allegheny": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 676.7841650596812,
        lastWeekAverageDeathPerMillionEachDay: 15.5069449380104,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Armstrong": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 973.1984243454084,
        lastWeekAverageDeathPerMillionEachDay: 15.447594037228704,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Beaver": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1323.7438159203068,
        lastWeekAverageDeathPerMillionEachDay: 23.529350250064702,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Bedford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1649.6825927163384,
        lastWeekAverageDeathPerMillionEachDay: 35.79781394682832,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Berks": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1225.1759409636154,
        lastWeekAverageDeathPerMillionEachDay: 8.819095920557587,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Blair": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1165.5681323822737,
        lastWeekAverageDeathPerMillionEachDay: 34.00550889244058,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Bradford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 828.8712431410905,
        lastWeekAverageDeathPerMillionEachDay: 9.47281420732675,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Bucks": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1330.6381014531967,
        lastWeekAverageDeathPerMillionEachDay: 13.415524262771463,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Butler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 777.2034516350551,
        lastWeekAverageDeathPerMillionEachDay: 16.73040698235931,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Cambria": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1520.8307730121667,
        lastWeekAverageDeathPerMillionEachDay: 59.25314700047402,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Cameron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 224.87069934787505,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Carbon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1339.9395469134645,
        lastWeekAverageDeathPerMillionEachDay: 26.70975841023517,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Centre": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 763.617329186809,
        lastWeekAverageDeathPerMillionEachDay: 17.59486933610159,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Chester": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 874.3040330368824,
        lastWeekAverageDeathPerMillionEachDay: 5.1701763547154584,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Clarion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 780.477652323222,
        lastWeekAverageDeathPerMillionEachDay: 33.4490422424238,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Clearfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 403.76001514100057,
        lastWeekAverageDeathPerMillionEachDay: 12.617500473156268,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Clinton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 465.9349761855457,
        lastWeekAverageDeathPerMillionEachDay: 11.093689909179657,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Columbia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 877.4090265377748,
        lastWeekAverageDeathPerMillionEachDay: 4.398040233272054,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Crawford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 720.7931087452292,
        lastWeekAverageDeathPerMillionEachDay: 27.008641077104606,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Cumberland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 994.5928878714924,
        lastWeekAverageDeathPerMillionEachDay: 20.861642205921324,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Dauphin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 980.9593279170965,
        lastWeekAverageDeathPerMillionEachDay: 14.373030445671743,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Delaware": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1565.072245640471,
        lastWeekAverageDeathPerMillionEachDay: 9.074343830416646,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Elk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 434.63724506853896,
        lastWeekAverageDeathPerMillionEachDay: 4.7762334622916365,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Erie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 600.6050539803059,
        lastWeekAverageDeathPerMillionEachDay: 20.126095283290674,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Fayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 464.1304515989293,
        lastWeekAverageDeathPerMillionEachDay: 20.996377572332516,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Forest": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 275.9762660411205,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1206.2414934172757,
        lastWeekAverageDeathPerMillionEachDay: 24.8804586113571,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Fulton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 481.76187198898833,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 248.39234951563495,
        lastWeekAverageDeathPerMillionEachDay: 7.88547141319476,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Huntingdon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1683.5016835016838,
        lastWeekAverageDeathPerMillionEachDay: 22.15133794081163,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Indiana": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1153.7592330474706,
        lastWeekAverageDeathPerMillionEachDay: 32.284868082329815,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 598.7334484743811,
        lastWeekAverageDeathPerMillionEachDay: 26.317953779093674,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Juniata": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1373.0161935145177,
        lastWeekAverageDeathPerMillionEachDay: 46.1518048240174,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Lackawanna": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1220.9429876856454,
        lastWeekAverageDeathPerMillionEachDay: 6.13196813011764,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Lancaster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1205.7376989100717,
        lastWeekAverageDeathPerMillionEachDay: 13.350547686585683,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Lawrence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1251.2863691645614,
        lastWeekAverageDeathPerMillionEachDay: 30.07096748326049,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Lebanon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 994.4073402777286,
        lastWeekAverageDeathPerMillionEachDay: 9.067544136271081,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Lehigh": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1202.2159764755577,
        lastWeekAverageDeathPerMillionEachDay: 7.736267544887759,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Luzerne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1272.7736699672669,
        lastWeekAverageDeathPerMillionEachDay: 18.902579256939607,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Lycoming": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 820.8369005904731,
        lastWeekAverageDeathPerMillionEachDay: 36.565699104644736,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - McKean": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 295.38461538461536,
        lastWeekAverageDeathPerMillionEachDay: 21.0989010989011,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Mercer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 950.4313496125166,
        lastWeekAverageDeathPerMillionEachDay: 37.86058947632278,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Mifflin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1994.0179461615157,
        lastWeekAverageDeathPerMillionEachDay: 49.54081853817431,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1092.3762707683634,
        lastWeekAverageDeathPerMillionEachDay: 10.067984062381228,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1278.1090725284776,
        lastWeekAverageDeathPerMillionEachDay: 9.284085272603953,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Montour": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1042.2380691168403,
        lastWeekAverageDeathPerMillionEachDay: 15.67275291905023,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Northampton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1257.8410337880994,
        lastWeekAverageDeathPerMillionEachDay: 10.762776702799956,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Northumberland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2157.56855233755,
        lastWeekAverageDeathPerMillionEachDay: 42.459439441045085,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Perry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 518.6721991701245,
        lastWeekAverageDeathPerMillionEachDay: 24.698676150958306,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Philadelphia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1400.1959516787201,
        lastWeekAverageDeathPerMillionEachDay: 8.206739121651651,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Pike": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 519.6294504470604,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Potter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 605.1071039574006,
        lastWeekAverageDeathPerMillionEachDay: 8.644387199391437,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Schuylkill": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1584.6178877892453,
        lastWeekAverageDeathPerMillionEachDay: 19.201364711731927,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Snyder": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 817.3981967700387,
        lastWeekAverageDeathPerMillionEachDay: 10.615560997013489,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Somerset": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 707.9935191362479,
        lastWeekAverageDeathPerMillionEachDay: 35.010668528715556,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Sullivan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 329.70656116056705,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Susquehanna": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 917.4766911327117,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Tioga": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1108.6201374688972,
        lastWeekAverageDeathPerMillionEachDay: 24.636003054864382,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 578.768114328963,
        lastWeekAverageDeathPerMillionEachDay: 22.260312089575496,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Venango": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 611.8260045788268,
        lastWeekAverageDeathPerMillionEachDay: 28.194746754784646,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 382.7409354188462,
        lastWeekAverageDeathPerMillionEachDay: 36.45151765893773,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 531.7477581997922,
        lastWeekAverageDeathPerMillionEachDay: 11.739885570644763,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 292.0503884270166,
        lastWeekAverageDeathPerMillionEachDay: 2.7814322707334913,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Westmoreland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 997.423323082038,
        lastWeekAverageDeathPerMillionEachDay: 23.748174359096147,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - Wyoming": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1082.3318653429876,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Pennsylvania - York": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 759.3673868408979,
        lastWeekAverageDeathPerMillionEachDay: 14.951934303109427,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Rhode Island - Bristol": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1175.7668268734915,
        lastWeekAverageDeathPerMillionEachDay: 11.787136109007433,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Rhode Island - Kent": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 998.2226766975873,
        lastWeekAverageDeathPerMillionEachDay: 9.564851431771306,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Rhode Island - Newport": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 134.01232913428035,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Rhode Island - Providence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1862.485933535859,
        lastWeekAverageDeathPerMillionEachDay: 16.99266878135958,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Rhode Island - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 812.250650995007,
        lastWeekAverageDeathPerMillionEachDay: 6.825635722647118,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Abbeville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 937.742080156562,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Aiken": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 696.4277353808701,
        lastWeekAverageDeathPerMillionEachDay: 6.688381612301273,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Allendale": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1035.9116022099447,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Anderson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1303.3304041311624,
        lastWeekAverageDeathPerMillionEachDay: 9.873715182811836,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Bamberg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2701.5498364851414,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Barnwell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1246.0461995590913,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Beaufort": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 515.297571334881,
        lastWeekAverageDeathPerMillionEachDay: 1.4871502780227448,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Berkeley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 478.2652573198717,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Calhoun": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1305.5727341441627,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Charleston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 760.8056275309548,
        lastWeekAverageDeathPerMillionEachDay: 3.125171450378181,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Cherokee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1308.9005235602092,
        lastWeekAverageDeathPerMillionEachDay: 7.479431563201195,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Chester": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1302.567919612951,
        lastWeekAverageDeathPerMillionEachDay: 8.861006255870416,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Chesterfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1248.630887185104,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Clarendon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2192.9174692547044,
        lastWeekAverageDeathPerMillionEachDay: 8.46686281565523,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Colleton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1406.693738885792,
        lastWeekAverageDeathPerMillionEachDay: 7.583254657066264,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Darlington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1260.9204719445195,
        lastWeekAverageDeathPerMillionEachDay: 4.288845142668434,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Dillon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1771.7116703303916,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Dorchester": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 743.2021571289055,
        lastWeekAverageDeathPerMillionEachDay: 2.632357109075226,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Edgefield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 953.7784299339693,
        lastWeekAverageDeathPerMillionEachDay: 5.240540823813018,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Fairfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2282.1855282588267,
        lastWeekAverageDeathPerMillionEachDay: 25.57070619897845,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Florence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1706.5216605323483,
        lastWeekAverageDeathPerMillionEachDay: 12.39604111766839,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Georgetown": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1132.7377153797065,
        lastWeekAverageDeathPerMillionEachDay: 2.2791503327559486,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Greenville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 886.2708244992759,
        lastWeekAverageDeathPerMillionEachDay: 5.730199296331525,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Greenwood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1299.2331699877136,
        lastWeekAverageDeathPerMillionEachDay: 10.08721405269964,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Hampton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1820.830298616169,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Horry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 833.1426989869551,
        lastWeekAverageDeathPerMillionEachDay: 4.438048275475306,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Jasper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 798.0580587237721,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Kershaw": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1081.877056693363,
        lastWeekAverageDeathPerMillionEachDay: 2.1465814616931804,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Lancaster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 795.8209198873606,
        lastWeekAverageDeathPerMillionEachDay: 7.287737361605866,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Laurens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1081.5936467485517,
        lastWeekAverageDeathPerMillionEachDay: 2.1166216179032324,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Lee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2436.415497979558,
        lastWeekAverageDeathPerMillionEachDay: 8.48925260620055,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Lexington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 846.8619246861925,
        lastWeekAverageDeathPerMillionEachDay: 3.8254632396891814,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1761.4247969468636,
        lastWeekAverageDeathPerMillionEachDay: 9.319707920353775,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Marlboro": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 880.6187303775173,
        lastWeekAverageDeathPerMillionEachDay: 5.469681555139859,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - McCormick": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 951.0725985416888,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Newberry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1821.0197710718005,
        lastWeekAverageDeathPerMillionEachDay: 3.716366879738368,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Oconee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 716.5665149724687,
        lastWeekAverageDeathPerMillionEachDay: 14.367248420500625,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Orangeburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1671.0182767624021,
        lastWeekAverageDeathPerMillionEachDay: 6.631024907787309,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Pickens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1016.6766495381609,
        lastWeekAverageDeathPerMillionEachDay: 20.265979725013175,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Richland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 767.2714240701945,
        lastWeekAverageDeathPerMillionEachDay: 1.7180282670626836,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Saluda": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1318.8101401846338,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Spartanburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1078.8498522444768,
        lastWeekAverageDeathPerMillionEachDay: 9.828031780280948,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Sumter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1040.0952015067326,
        lastWeekAverageDeathPerMillionEachDay: 5.354415451772111,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1317.908917850344,
        lastWeekAverageDeathPerMillionEachDay: 10.459594586113841,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - Williamsburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1844.0463645943098,
        lastWeekAverageDeathPerMillionEachDay: 9.408399819358722,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Carolina - York": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 587.2324978023269,
        lastWeekAverageDeathPerMillionEachDay: 4.575837645212937,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Aurora": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2908.033442384588,
        lastWeekAverageDeathPerMillionEachDay: 51.92916861401049,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Beadle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1842.5188316262938,
        lastWeekAverageDeathPerMillionEachDay: 15.483351526271376,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Bennett": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2377.4145616641904,
        lastWeekAverageDeathPerMillionEachDay: 127.36149437486733,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Bon Homme": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3187.9437762643097,
        lastWeekAverageDeathPerMillionEachDay: 20.700933612105906,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Brookings": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 684.2090258573996,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Brown": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1313.1131079584957,
        lastWeekAverageDeathPerMillionEachDay: 25.74731584232344,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Brule": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1132.7166320558808,
        lastWeekAverageDeathPerMillionEachDay: 26.96944362037811,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Buffalo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 5096.839959225282,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Butte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1534.1835267043818,
        lastWeekAverageDeathPerMillionEachDay: 13.698067202717695,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Campbell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2180.2325581395353,
        lastWeekAverageDeathPerMillionEachDay: 207.64119601328906,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Charles Mix": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1076.1945759793368,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 535.3319057815846,
        lastWeekAverageDeathPerMillionEachDay: 38.237993270113186,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 781.8052594171995,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Codington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2356.385447534721,
        lastWeekAverageDeathPerMillionEachDay: 35.702809811132134,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Corson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1713.1669114047968,
        lastWeekAverageDeathPerMillionEachDay: 69.92518005733864,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Custer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 891.6629514043691,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Davison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2629.5828065739574,
        lastWeekAverageDeathPerMillionEachDay: 7.224128589488894,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Day": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3318.58407079646,
        lastWeekAverageDeathPerMillionEachDay: 105.35187526337967,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Deuel": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1378.9933348655481,
        lastWeekAverageDeathPerMillionEachDay: 32.83317463965591,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Dewey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1697.2165648336734,
        lastWeekAverageDeathPerMillionEachDay: 72.737852778586,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2396.4395754878465,
        lastWeekAverageDeathPerMillionEachDay: 48.906930111996864,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Edmunds": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1044.6591799425437,
        lastWeekAverageDeathPerMillionEachDay: 37.30925642651941,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Fall River": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1489.6469536719796,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Faulk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4349.717268377557,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2977.878615995462,
        lastWeekAverageDeathPerMillionEachDay: 20.257677659833075,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Gregory": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 5973.715651135006,
        lastWeekAverageDeathPerMillionEachDay: 34.135518006485746,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Haakon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2632.964718272776,
        lastWeekAverageDeathPerMillionEachDay: 150.45512675844432,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Hamlin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 5191.434133679428,
        lastWeekAverageDeathPerMillionEachDay: 92.70418095856121,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Hand": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 626.7627702914447,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Hanson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 868.8097306689837,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Hughes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1426.4521282665753,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Hutchinson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1920.1755589082431,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2392.3444976076557,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Jerauld": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 7451.56482861401,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Kingsbury": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2632.1117635148817,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1015.8630929124014,
        lastWeekAverageDeathPerMillionEachDay: 11.16333069134507,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Lawrence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1044.7299179693546,
        lastWeekAverageDeathPerMillionEachDay: 5.527671523647378,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 981.5469179426775,
        lastWeekAverageDeathPerMillionEachDay: 7.011049413876267,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Lyman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2380.3226659613856,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Marshall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 810.5369807497466,
        lastWeekAverageDeathPerMillionEachDay: 28.947749312490945,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - McCook": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3759.3984962406016,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - McPherson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 420.34468263976447,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Meade": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 741.2113511224056,
        lastWeekAverageDeathPerMillionEachDay: 5.042254089268066,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Mellette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 970.4027171276078,
        lastWeekAverageDeathPerMillionEachDay: 69.31447979482913,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Miner": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2707.581227436823,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Minnehaha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1278.904801847422,
        lastWeekAverageDeathPerMillionEachDay: 11.09518335900019,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Moody": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2128.9537712895376,
        lastWeekAverageDeathPerMillionEachDay: 21.724018074383036,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Oglala Lakota": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2327.7139028003103,
        lastWeekAverageDeathPerMillionEachDay: 20.15336712381221,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Pennington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1081.0810810810813,
        lastWeekAverageDeathPerMillionEachDay: 8.78927708196001,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Perkins": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3141.3612565445023,
        lastWeekAverageDeathPerMillionEachDay: 299.17726252804783,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Potter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 928.9363678588015,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Roberts": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2501.4431402732343,
        lastWeekAverageDeathPerMillionEachDay: 41.23257923527309,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Sanborn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1279.863481228669,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Spink": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3293.6010037641154,
        lastWeekAverageDeathPerMillionEachDay: 22.405449005198065,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Stanley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 645.5777921239509,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Sully": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2156.721782890007,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Todd": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1670.4333300579738,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Tripp": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1837.8974453225512,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Turner": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 5605.916030534349,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1883.0027617373842,
        lastWeekAverageDeathPerMillionEachDay: 44.83339908898534,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Walworth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2575.896964121435,
        lastWeekAverageDeathPerMillionEachDay: 26.284662899198313,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Yankton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1008.1528885771893,
        lastWeekAverageDeathPerMillionEachDay: 37.5709151022555,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - South Dakota - Ziebach": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2539.9129172714074,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Anderson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 623.5547818857335,
        lastWeekAverageDeathPerMillionEachDay: 14.846542425850798,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Bedford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1086.234988835918,
        lastWeekAverageDeathPerMillionEachDay: 22.989100292823665,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Benton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1608.910891089109,
        lastWeekAverageDeathPerMillionEachDay: 35.360678925035366,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Bledsoe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 464.6840148698885,
        lastWeekAverageDeathPerMillionEachDay: 9.48334724224262,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Blount": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 646.1889877374368,
        lastWeekAverageDeathPerMillionEachDay: 19.321265414076187,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Bradley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 554.9902876699658,
        lastWeekAverageDeathPerMillionEachDay: 15.856865361999022,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Campbell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 803.1725314994228,
        lastWeekAverageDeathPerMillionEachDay: 17.927958292397832,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Cannon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 545.0333832947267,
        lastWeekAverageDeathPerMillionEachDay: 29.1982169622175,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1656.6427773976304,
        lastWeekAverageDeathPerMillionEachDay: 36.013973421687616,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Carter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1347.7327942402158,
        lastWeekAverageDeathPerMillionEachDay: 2.5333323200004054,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Cheatham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 467.2092851697936,
        lastWeekAverageDeathPerMillionEachDay: 3.5128517681939364,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Chester": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1503.1508354049831,
        lastWeekAverageDeathPerMillionEachDay: 8.259070524203205,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Claiborne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 375.48108514033606,
        lastWeekAverageDeathPerMillionEachDay: 4.470012918337334,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2757.7150361129347,
        lastWeekAverageDeathPerMillionEachDay: 37.51993246412156,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Cocke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1027.6635929341185,
        lastWeekAverageDeathPerMillionEachDay: 19.839065500658656,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Coffee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 990.7997169143665,
        lastWeekAverageDeathPerMillionEachDay: 20.22040238600748,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Crockett": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2037.9479971890376,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Cumberland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 793.126239259749,
        lastWeekAverageDeathPerMillionEachDay: 9.441979038806535,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Davidson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 724.6335054397933,
        lastWeekAverageDeathPerMillionEachDay: 5.762493085008297,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - DeKalb": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1317.715959004392,
        lastWeekAverageDeathPerMillionEachDay: 13.944084222268698,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Decatur": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1457.6009603018092,
        lastWeekAverageDeathPerMillionEachDay: 12.248747565561422,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Dickson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 945.3547860903093,
        lastWeekAverageDeathPerMillionEachDay: 10.592210488406826,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Dyer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1749.2397534917513,
        lastWeekAverageDeathPerMillionEachDay: 19.22241487353573,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Fayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1069.7007269102664,
        lastWeekAverageDeathPerMillionEachDay: 6.946108616300431,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Fentress": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1457.6472493656534,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1042.456406368461,
        lastWeekAverageDeathPerMillionEachDay: 10.153796165926568,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Gibson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1648.5864897319518,
        lastWeekAverageDeathPerMillionEachDay: 11.630239786468794,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Giles": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1900.624490904154,
        lastWeekAverageDeathPerMillionEachDay: 14.545595593654241,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Grainger": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 943.3962264150944,
        lastWeekAverageDeathPerMillionEachDay: 6.125949522175937,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1187.2185785229262,
        lastWeekAverageDeathPerMillionEachDay: 6.204975149074527,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Grundy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1117.152007149773,
        lastWeekAverageDeathPerMillionEachDay: 10.639542925235933,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Hamblen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1093.4179320540857,
        lastWeekAverageDeathPerMillionEachDay: 8.800144322366885,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Hamilton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 638.9272547334994,
        lastWeekAverageDeathPerMillionEachDay: 8.933329397489656,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Hancock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 604.2296072507554,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Hardeman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1596.8063872255489,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Hardin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1286.4493996569465,
        lastWeekAverageDeathPerMillionEachDay: 11.138090040319883,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Hawkins": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 845.2787658930017,
        lastWeekAverageDeathPerMillionEachDay: 12.578553063883955,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Haywood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1964.8636153490522,
        lastWeekAverageDeathPerMillionEachDay: 24.767188428769565,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Henderson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1422.6268805349077,
        lastWeekAverageDeathPerMillionEachDay: 10.16162057524934,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Henry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 989.3337455557273,
        lastWeekAverageDeathPerMillionEachDay: 26.500011041671268,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Hickman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 992.9303360076259,
        lastWeekAverageDeathPerMillionEachDay: 22.69555053731716,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Houston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2560.663333739788,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Humphreys": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 591.9707243569046,
        lastWeekAverageDeathPerMillionEachDay: 23.063794455463814,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1527.2357033768878,
        lastWeekAverageDeathPerMillionEachDay: 12.12091828076895,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 990.916597853014,
        lastWeekAverageDeathPerMillionEachDay: 36.70061473529682,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1517.8772205981559,
        lastWeekAverageDeathPerMillionEachDay: 16.062192810562497,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Knox": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 518.8034351591388,
        lastWeekAverageDeathPerMillionEachDay: 11.846214268856212,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1710.3762827822122,
        lastWeekAverageDeathPerMillionEachDay: 40.723244828147905,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Lauderdale": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1053.329692193657,
        lastWeekAverageDeathPerMillionEachDay: 11.146345949139226,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Lawrence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1178.0164016129763,
        lastWeekAverageDeathPerMillionEachDay: 25.890470365120358,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Lewis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1548.7447016628626,
        lastWeekAverageDeathPerMillionEachDay: 34.93409101495178,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 785.660245591573,
        lastWeekAverageDeathPerMillionEachDay: 4.1569325163575295,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Loudon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 536.3616187023747,
        lastWeekAverageDeathPerMillionEachDay: 15.853052769528317,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Macon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1463.2956670189417,
        lastWeekAverageDeathPerMillionEachDay: 23.22691534950701,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1439.010450685826,
        lastWeekAverageDeathPerMillionEachDay: 18.953531771951102,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 761.0613346248313,
        lastWeekAverageDeathPerMillionEachDay: 4.94195671834306,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Marshall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 814.5454545454545,
        lastWeekAverageDeathPerMillionEachDay: 20.77922077922078,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Maury": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 985.6100926473486,
        lastWeekAverageDeathPerMillionEachDay: 4.446361320213602,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - McMinn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1115.3660259508495,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - McNairy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1362.1857242936092,
        lastWeekAverageDeathPerMillionEachDay: 5.55994173181065,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Meigs": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1127.0326839478344,
        lastWeekAverageDeathPerMillionEachDay: 11.50033350967178,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1095.7138253303253,
        lastWeekAverageDeathPerMillionEachDay: 9.207679204456515,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 507.1940208523729,
        lastWeekAverageDeathPerMillionEachDay: 2.7341995733281554,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Moore": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 616.5228113440199,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Morgan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 513.9466429939729,
        lastWeekAverageDeathPerMillionEachDay: 20.023895181583356,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Obion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2228.2084538893882,
        lastWeekAverageDeathPerMillionEachDay: 19.003910054493716,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Out of TN": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 0,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Overton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1753.5182770558874,
        lastWeekAverageDeathPerMillionEachDay: 6.423143871999588,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Perry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2847.9445269935613,
        lastWeekAverageDeathPerMillionEachDay: 17.689096440953797,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Pickett": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3367.670364500792,
        lastWeekAverageDeathPerMillionEachDay: 28.299750962191528,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 891.1596958174905,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Putnam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1345.8782478659105,
        lastWeekAverageDeathPerMillionEachDay: 32.04472018728359,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Rhea": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1326.6198329665028,
        lastWeekAverageDeathPerMillionEachDay: 21.536036249456213,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Roane": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 824.2478738151439,
        lastWeekAverageDeathPerMillionEachDay: 13.380647302193893,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Robertson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 960.8288192945566,
        lastWeekAverageDeathPerMillionEachDay: 9.946468108639303,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Rutherford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 644.0254600719261,
        lastWeekAverageDeathPerMillionEachDay: 9.02839430007373,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Scott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 725.031720137756,
        lastWeekAverageDeathPerMillionEachDay: 32.36748750614982,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Sequatchie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 465.8591774257953,
        lastWeekAverageDeathPerMillionEachDay: 19.014660303093684,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Sevier": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 590.3307888040712,
        lastWeekAverageDeathPerMillionEachDay: 15.99418393311523,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Shelby": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 869.6431582024956,
        lastWeekAverageDeathPerMillionEachDay: 10.822903458786534,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Smith": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1141.0428139108,
        lastWeekAverageDeathPerMillionEachDay: 21.261667339952798,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Stewart": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1385.3445133065989,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Sullivan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1016.747922297724,
        lastWeekAverageDeathPerMillionEachDay: 23.456473806336138,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Sumner": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 951.4698117449014,
        lastWeekAverageDeathPerMillionEachDay: 16.430404912392333,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Tipton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 795.4674588873196,
        lastWeekAverageDeathPerMillionEachDay: 13.914882662751946,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Trousdale": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1063.4526763559022,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Unicoi": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2013.0850528434823,
        lastWeekAverageDeathPerMillionEachDay: 47.93059649627339,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Union": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 350.49068696174646,
        lastWeekAverageDeathPerMillionEachDay: 7.152871162484621,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Van Buren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 851.4986376021799,
        lastWeekAverageDeathPerMillionEachDay: 24.328532502919423,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 702.5704387431258,
        lastWeekAverageDeathPerMillionEachDay: 20.765628731323915,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1113.0434782608697,
        lastWeekAverageDeathPerMillionEachDay: 16.563146997929607,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 959.6353385713427,
        lastWeekAverageDeathPerMillionEachDay: 8.56817266581556,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Weakley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1200.1920307249159,
        lastWeekAverageDeathPerMillionEachDay: 8.572800219463684,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - White": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1316.5112452002195,
        lastWeekAverageDeathPerMillionEachDay: 41.79400778413395,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Williamson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 457.1917520930155,
        lastWeekAverageDeathPerMillionEachDay: 5.392825385107654,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Tennessee - Wilson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 774.2452836710288,
        lastWeekAverageDeathPerMillionEachDay: 11.850693117413705,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Anderson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 987.2694206287347,
        lastWeekAverageDeathPerMillionEachDay: 7.4230783505919895,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Andrews": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1176.1561079925154,
        lastWeekAverageDeathPerMillionEachDay: 30.54950929850689,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Angelina": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1453.0358069538142,
        lastWeekAverageDeathPerMillionEachDay: 23.06406042783832,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Aransas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 935.7720119098253,
        lastWeekAverageDeathPerMillionEachDay: 6.076441635778086,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Archer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 467.67216181456797,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Armstrong": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2649.7085320614733,
        lastWeekAverageDeathPerMillionEachDay: 75.70595805889923,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Atascosa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1036.107364181964,
        lastWeekAverageDeathPerMillionEachDay: 8.378226664544183,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Austin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 466.1694192860948,
        lastWeekAverageDeathPerMillionEachDay: 4.756830809041784,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Bailey": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1571.4285714285716,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Bandera": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 475.9432329525787,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Bastrop": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 450.8413827305209,
        lastWeekAverageDeathPerMillionEachDay: 1.6101477954661458,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Baylor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1709.8888572242804,
        lastWeekAverageDeathPerMillionEachDay: 40.71163945772096,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Bee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1351.143866113926,
        lastWeekAverageDeathPerMillionEachDay: 17.54732293654449,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Bell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 410.5542758263438,
        lastWeekAverageDeathPerMillionEachDay: 3.5426543455772723,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Bexar": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 813.5543139840506,
        lastWeekAverageDeathPerMillionEachDay: 3.7789990044833197,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Blanco": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 586.706897996815,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Bosque": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 856.3018464008564,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Bowie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1265.4834039358682,
        lastWeekAverageDeathPerMillionEachDay: 10.724435626575154,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Brazoria": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 598.5080050445675,
        lastWeekAverageDeathPerMillionEachDay: 5.34382147361221,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Brazos": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 567.1630070110074,
        lastWeekAverageDeathPerMillionEachDay: 4.362792361623133,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Brewster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 760.6215364555037,
        lastWeekAverageDeathPerMillionEachDay: 15.522888499091911,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Briscoe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1293.6610608020699,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Brooks": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3806.5698576060904,
        lastWeekAverageDeathPerMillionEachDay: 20.14058125717508,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Brown": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1267.694908092119,
        lastWeekAverageDeathPerMillionEachDay: 33.956113609610334,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Burleson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 704.8744781217806,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Burnet": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 477.62433807496615,
        lastWeekAverageDeathPerMillionEachDay: 8.899832386490052,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Caldwell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 916.0864785635764,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Calhoun": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 469.7040864255519,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Callahan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 932.3674962346697,
        lastWeekAverageDeathPerMillionEachDay: 30.73738998575834,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Cameron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2736.5341487795486,
        lastWeekAverageDeathPerMillionEachDay: 5.063904790487691,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Camp": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1603.7879945012983,
        lastWeekAverageDeathPerMillionEachDay: 32.730367234720376,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Carson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 843.7394532568344,
        lastWeekAverageDeathPerMillionEachDay: 24.10684152162384,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Cass": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1432.0921867714649,
        lastWeekAverageDeathPerMillionEachDay: 9.51556270280043,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Castro": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1593.6254980079682,
        lastWeekAverageDeathPerMillionEachDay: 18.971732119142477,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Chambers": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 250.9295800351302,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Cherokee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1310.640884397675,
        lastWeekAverageDeathPerMillionEachDay: 2.7135422037218944,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Childress": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 821.2428141253764,
        lastWeekAverageDeathPerMillionEachDay: 58.660201008955454,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 859.5167605768313,
        lastWeekAverageDeathPerMillionEachDay: 13.643123183759226,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Cochran": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2103.049421661409,
        lastWeekAverageDeathPerMillionEachDay: 50.07260527765259,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Coke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2952.4653085326254,
        lastWeekAverageDeathPerMillionEachDay: 42.17807583618036,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Coleman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1590.2140672782878,
        lastWeekAverageDeathPerMillionEachDay: 34.94975972040193,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Collin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 315.0580344630967,
        lastWeekAverageDeathPerMillionEachDay: 3.589618271709252,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Collingsworth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1712.3287671232877,
        lastWeekAverageDeathPerMillionEachDay: 97.84735812133073,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Colorado": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 558.3213139161587,
        lastWeekAverageDeathPerMillionEachDay: 13.293364617051397,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Comal": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 800.2099750974656,
        lastWeekAverageDeathPerMillionEachDay: 5.48715411495405,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Comanche": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1613.4946828016136,
        lastWeekAverageDeathPerMillionEachDay: 83.81790560008382,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Concho": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1100.513573000734,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Cooke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 557.4811547131395,
        lastWeekAverageDeathPerMillionEachDay: 10.387847603350426,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Coryell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 368.65874050374583,
        lastWeekAverageDeathPerMillionEachDay: 11.28547164807385,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Cottle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3576.53791130186,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Crane": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1876.172607879925,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Crockett": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2309.4688221709007,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Crosby": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2963.2211957469062,
        lastWeekAverageDeathPerMillionEachDay: 24.90101845165467,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Culberson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1381.851681252879,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Dallam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2332.92164127899,
        lastWeekAverageDeathPerMillionEachDay: 39.20876708031916,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Dallas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 694.7406124645041,
        lastWeekAverageDeathPerMillionEachDay: 5.583075843320896,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Dawson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3614.0791954745446,
        lastWeekAverageDeathPerMillionEachDay: 67.34309059890455,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - DeWitt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2281.746031746032,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Deaf Smith": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2695.9991372802756,
        lastWeekAverageDeathPerMillionEachDay: 38.51427338971823,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Delta": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 562.7462014631401,
        lastWeekAverageDeathPerMillionEachDay: 26.797438164911433,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Denton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 337.0126700984099,
        lastWeekAverageDeathPerMillionEachDay: 5.6356633795720725,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Dickens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2713.7042062415203,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Dimmit": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 987.7518767285657,
        lastWeekAverageDeathPerMillionEachDay: 14.110741096122366,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Donley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1830.3843807199514,
        lastWeekAverageDeathPerMillionEachDay: 43.58058049333217,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Duval": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2420.0053777897283,
        lastWeekAverageDeathPerMillionEachDay: 38.41278377444013,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Eastland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 653.5947712418301,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Ector": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1203.2029261895163,
        lastWeekAverageDeathPerMillionEachDay: 28.36121183161003,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Edwards": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1035.1966873706,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - El Paso": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1802.8258968254534,
        lastWeekAverageDeathPerMillionEachDay: 23.660919616536493,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Ellis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 773.7006698191815,
        lastWeekAverageDeathPerMillionEachDay: 3.8646387103855218,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Erath": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 632.3481193498524,
        lastWeekAverageDeathPerMillionEachDay: 6.691514490474628,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Falls": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 925.0158987107591,
        lastWeekAverageDeathPerMillionEachDay: 8.259070524203207,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Fannin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1351.5796587261364,
        lastWeekAverageDeathPerMillionEachDay: 12.067675524340501,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Fayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1262.5266314211315,
        lastWeekAverageDeathPerMillionEachDay: 11.272559209117246,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Fisher": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1827.6762402088773,
        lastWeekAverageDeathPerMillionEachDay: 74.59903021260723,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Floyd": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3676.470588235294,
        lastWeekAverageDeathPerMillionEachDay: 125.0500200080032,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Foard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3463.2034632034633,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Fort Bend": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 445.98417125792173,
        lastWeekAverageDeathPerMillionEachDay: 1.5840006082562335,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1025.6410256410254,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Freestone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1115.7884059441092,
        lastWeekAverageDeathPerMillionEachDay: 21.736137778131997,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Frio": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 492.4652811976756,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Gaines": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1209.7524660338731,
        lastWeekAverageDeathPerMillionEachDay: 19.94097471484406,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Galveston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 584.5577382291991,
        lastWeekAverageDeathPerMillionEachDay: 2.0877062079614253,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Garza": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2568.6305988120084,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Gillespie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 592.8560841855639,
        lastWeekAverageDeathPerMillionEachDay: 15.88007368354189,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Glasscock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 709.7232079489,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Goliad": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 914.0767824497258,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Gonzales": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1151.7972836780725,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Gray": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1370.7392853879192,
        lastWeekAverageDeathPerMillionEachDay: 19.581989791255985,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Grayson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1035.151088009867,
        lastWeekAverageDeathPerMillionEachDay: 9.439067671822496,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Gregg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1129.533260720481,
        lastWeekAverageDeathPerMillionEachDay: 14.983604478945155,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Grimes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1419.6675900277007,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Guadalupe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 599.3515016751875,
        lastWeekAverageDeathPerMillionEachDay: 6.849731447716428,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hale": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3412.5606178530807,
        lastWeekAverageDeathPerMillionEachDay: 25.658350510173538,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1686.9095816464237,
        lastWeekAverageDeathPerMillionEachDay: 96.3948332369385,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hamilton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1654.6507505023048,
        lastWeekAverageDeathPerMillionEachDay: 33.76838266331234,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hansford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2222.6338210779772,
        lastWeekAverageDeathPerMillionEachDay: 26.45992644140449,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hardeman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1779.8118484617341,
        lastWeekAverageDeathPerMillionEachDay: 36.32269078493335,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hardin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 815.9438908371236,
        lastWeekAverageDeathPerMillionEachDay: 4.9601452330524225,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Harris": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 687.8371425692054,
        lastWeekAverageDeathPerMillionEachDay: 2.788447039586097,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Harrison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 871.4858834312505,
        lastWeekAverageDeathPerMillionEachDay: 6.4395508627924904,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Haskell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1413.9271827500884,
        lastWeekAverageDeathPerMillionEachDay: 75.74609907589759,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hays": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 516.9620011208084,
        lastWeekAverageDeathPerMillionEachDay: 7.447231708823169,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hemphill": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 261.8486514794449,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Henderson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 821.881383178022,
        lastWeekAverageDeathPerMillionEachDay: 12.086490929088558,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hidalgo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2479.54718909828,
        lastWeekAverageDeathPerMillionEachDay: 5.426784536426797,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hill": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 764.004474883353,
        lastWeekAverageDeathPerMillionEachDay: 11.693946044132954,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hockley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3040.7019677685594,
        lastWeekAverageDeathPerMillionEachDay: 31.027571099679175,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1070.6811803448893,
        lastWeekAverageDeathPerMillionEachDay: 20.85742559113421,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hopkins": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1806.7090928702405,
        lastWeekAverageDeathPerMillionEachDay: 26.965807356272247,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Houston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 957.8544061302681,
        lastWeekAverageDeathPerMillionEachDay: 24.879335224162805,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Howard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1445.559677067423,
        lastWeekAverageDeathPerMillionEachDay: 23.37832361834107,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hudspeth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1432.6647564469915,
        lastWeekAverageDeathPerMillionEachDay: 58.476112508040465,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hunt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 750.5527719739539,
        lastWeekAverageDeathPerMillionEachDay: 7.244717876196466,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Hutchinson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1910.4021396503965,
        lastWeekAverageDeathPerMillionEachDay: 54.58291827572561,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Jack": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1007.2747621712368,
        lastWeekAverageDeathPerMillionEachDay: 31.97697657686466,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1287.2628726287262,
        lastWeekAverageDeathPerMillionEachDay: 19.357336430507157,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Jasper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1125.840862394101,
        lastWeekAverageDeathPerMillionEachDay: 4.020860222836074,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Jeff Davis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 439.7537379067723,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 814.8987339256256,
        lastWeekAverageDeathPerMillionEachDay: 5.678736821781364,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Jim Hogg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1923.076923076923,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Jim Wells": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1506.8425473049751,
        lastWeekAverageDeathPerMillionEachDay: 10.586715789027927,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 784.9070340183258,
        lastWeekAverageDeathPerMillionEachDay: 8.937864776606196,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Jones": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1095.453866454215,
        lastWeekAverageDeathPerMillionEachDay: 28.45334718062896,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Karnes": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1217.8706493173515,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Kaufman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 785.8748182205443,
        lastWeekAverageDeathPerMillionEachDay: 10.49232067050126,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Kendall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 632.4977335497881,
        lastWeekAverageDeathPerMillionEachDay: 24.09515175427764,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Kenedy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4950.49504950495,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Kent": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1312.3359580052495,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Kerr": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 608.3650190114068,
        lastWeekAverageDeathPerMillionEachDay: 8.14774579033134,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Kimble": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1383.4447774959651,
        lastWeekAverageDeathPerMillionEachDay: 32.939161368951545,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Kinney": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 818.1074447777474,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Kleberg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1857.887874837027,
        lastWeekAverageDeathPerMillionEachDay: 4.656360588563977,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Knox": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3275.109170305677,
        lastWeekAverageDeathPerMillionEachDay: 77.97878976918277,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - La Salle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1861.7021276595742,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Lamar": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1724.8641168094025,
        lastWeekAverageDeathPerMillionEachDay: 28.652227853976786,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Lamb": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4886.3724501667575,
        lastWeekAverageDeathPerMillionEachDay: 66.48125782539805,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Lampasas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 653.3507560201606,
        lastWeekAverageDeathPerMillionEachDay: 13.333688898370623,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Lavaca": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2431.279150540836,
        lastWeekAverageDeathPerMillionEachDay: 14.176554813649187,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Lee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1334.1841174082022,
        lastWeekAverageDeathPerMillionEachDay: 16.57371574420127,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Leon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1206.6191680073546,
        lastWeekAverageDeathPerMillionEachDay: 16.416587319827954,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Liberty": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 952.1758351375555,
        lastWeekAverageDeathPerMillionEachDay: 16.193466583972032,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Limestone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 896.0191150744549,
        lastWeekAverageDeathPerMillionEachDay: 6.095368129758197,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Lipscomb": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1855.8614290133,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Live Oak": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 819.2020971573687,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Llano": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 275.2924982794218,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Lubbock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1616.3879846346545,
        lastWeekAverageDeathPerMillionEachDay: 19.77936350008257,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Lynn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1848.4288354898335,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1120.1344161299355,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1725.1877410188754,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Martin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2079.3623288858084,
        lastWeekAverageDeathPerMillionEachDay: 24.754313439116768,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Mason": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 467.94571829667757,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Matagorda": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1746.581884671015,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Maverick": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3150.4376553932098,
        lastWeekAverageDeathPerMillionEachDay: 9.731081561060106,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - McCulloch": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1252.5050100200401,
        lastWeekAverageDeathPerMillionEachDay: 17.892928714572,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - McLennan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 946.9143451678141,
        lastWeekAverageDeathPerMillionEachDay: 11.1336195786927,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - McMullen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1345.8950201884252,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Medina": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1046.8362282878413,
        lastWeekAverageDeathPerMillionEachDay: 2.7694080113434953,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Menard": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1403.1805425631433,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Midland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 972.6746290264206,
        lastWeekAverageDeathPerMillionEachDay: 13.733777984592317,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Milam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 402.8521935301937,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Mills": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2052.123948286476,
        lastWeekAverageDeathPerMillionEachDay: 58.632112808185035,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Mitchell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1404.330017554125,
        lastWeekAverageDeathPerMillionEachDay: 83.59107247345983,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Montague": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1614.6937127863562,
        lastWeekAverageDeathPerMillionEachDay: 21.62536222481727,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 403.36455429863133,
        lastWeekAverageDeathPerMillionEachDay: 2.822375890136196,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Moore": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2053.486150907355,
        lastWeekAverageDeathPerMillionEachDay: 27.28885250375222,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Morris": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 645.7862447529868,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Motley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2500,
        lastWeekAverageDeathPerMillionEachDay: 119.04761904761905,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Nacogdoches": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1395.6199006195943,
        lastWeekAverageDeathPerMillionEachDay: 4.381852121254613,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Navarro": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 937.8803903178817,
        lastWeekAverageDeathPerMillionEachDay: 11.402801098089748,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Newton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1250.4597278411181,
        lastWeekAverageDeathPerMillionEachDay: 10.508064939841328,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Nolan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1019.4372706266141,
        lastWeekAverageDeathPerMillionEachDay: 29.126779160760403,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Nueces": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1446.339161012879,
        lastWeekAverageDeathPerMillionEachDay: 4.731752980412908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Ochiltree": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1118.340788938593,
        lastWeekAverageDeathPerMillionEachDay: 29.047812699703712,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Oldham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 946.9696969696969,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Orange": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 779.4138807616672,
        lastWeekAverageDeathPerMillionEachDay: 8.564987700677662,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Palo Pinto": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1473.157696392477,
        lastWeekAverageDeathPerMillionEachDay: 9.7884232318437,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Panola": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1681.4693455203928,
        lastWeekAverageDeathPerMillionEachDay: 6.15922837186957,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Parker": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 573.9162082335979,
        lastWeekAverageDeathPerMillionEachDay: 5.999124127877329,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Parmer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3227.4856845393024,
        lastWeekAverageDeathPerMillionEachDay: 44.61961775860786,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Pecos": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1327.1819503254756,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 934.706833096411,
        lastWeekAverageDeathPerMillionEachDay: 5.563731149383398,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Potter": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2393.2206276881143,
        lastWeekAverageDeathPerMillionEachDay: 26.767083787055675,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Presidio": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1939.1408114558476,
        lastWeekAverageDeathPerMillionEachDay: 21.309239686327995,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Rains": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1118.7470033562413,
        lastWeekAverageDeathPerMillionEachDay: 45.6631429941323,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Randall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1125.5291802516829,
        lastWeekAverageDeathPerMillionEachDay: 15.560311247719117,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Reagan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1558.8464536243177,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Real": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2896.871378910776,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Red River": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1663.4783331947099,
        lastWeekAverageDeathPerMillionEachDay: 11.881988094247927,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Reeves": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1564.8472709063599,
        lastWeekAverageDeathPerMillionEachDay: 44.709922025896,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Refugio": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2446.747265400115,
        lastWeekAverageDeathPerMillionEachDay: 20.560901389916932,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Roberts": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1170.9601873536303,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Robertson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 702.8230057397215,
        lastWeekAverageDeathPerMillionEachDay: 25.100821633561477,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Rockwall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 447.98169947100035,
        lastWeekAverageDeathPerMillionEachDay: 6.8082325147568445,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Runnels": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1656.2743569758377,
        lastWeekAverageDeathPerMillionEachDay: 55.67308762943992,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Rusk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 882.2556335698268,
        lastWeekAverageDeathPerMillionEachDay: 23.63184732776322,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Sabine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1233.1625877442614,
        lastWeekAverageDeathPerMillionEachDay: 27.10247445591783,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - San Augustine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2549.4718951074424,
        lastWeekAverageDeathPerMillionEachDay: 34.68669245044139,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - San Jacinto": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 623.7222356977027,
        lastWeekAverageDeathPerMillionEachDay: 9.900352947582583,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - San Patricio": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1408.6617713172486,
        lastWeekAverageDeathPerMillionEachDay: 6.422470081993534,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - San Saba": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1816.6804293971925,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Schleicher": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1790.1897601145724,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Scurry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1616.476082140933,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Shackelford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 306.2787136294028,
        lastWeekAverageDeathPerMillionEachDay: 43.754101947057535,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Shelby": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1582.6541109440534,
        lastWeekAverageDeathPerMillionEachDay: 5.652336110514476,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Sherman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1323.6267372600928,
        lastWeekAverageDeathPerMillionEachDay: 47.27238347357474,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Smith": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1009.6626867338915,
        lastWeekAverageDeathPerMillionEachDay: 9.820427348171588,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Somervell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 657.3181419807186,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Starr": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3047.97858679003,
        lastWeekAverageDeathPerMillionEachDay: 6.630845366475772,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Stephens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1174.4608157164214,
        lastWeekAverageDeathPerMillionEachDay: 15.25273786644703,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Sterling": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 774.5933384972889,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Stonewall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2222.2222222222217,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Sutton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1588.9830508474577,
        lastWeekAverageDeathPerMillionEachDay: 37.832929782082324,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Swisher": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1622.2793024199002,
        lastWeekAverageDeathPerMillionEachDay: 19.312848838332144,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Tarrant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 604.0384967526987,
        lastWeekAverageDeathPerMillionEachDay: 6.115125388947456,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Taylor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1144.6455221177391,
        lastWeekAverageDeathPerMillionEachDay: 13.454242122541238,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Terrell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1288.6597938144332,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Terry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2836.994407068169,
        lastWeekAverageDeathPerMillionEachDay: 11.579569008441506,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Throckmorton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1998.6675549633576,
        lastWeekAverageDeathPerMillionEachDay: 95.1746454744456,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Titus": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1312.9770992366414,
        lastWeekAverageDeathPerMillionEachDay: 4.362050163576882,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Tom Green": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1384.2281879194632,
        lastWeekAverageDeathPerMillionEachDay: 16.778523489932887,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Travis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 405.03817249288437,
        lastWeekAverageDeathPerMillionEachDay: 1.2335049549894042,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Trinity": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 614.2925397583783,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Tyler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 415.28239202657807,
        lastWeekAverageDeathPerMillionEachDay: 13.183568000843747,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Upshur": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 670.6104950542476,
        lastWeekAverageDeathPerMillionEachDay: 10.264446352871136,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Upton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1640.6890894175551,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Uvalde": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1458.4346135148278,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Val Verde": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2978.072412034676,
        lastWeekAverageDeathPerMillionEachDay: 8.741895534348362,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Van Zandt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1007.245096306768,
        lastWeekAverageDeathPerMillionEachDay: 20.19539040214071,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Victoria": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1205.4211372225361,
        lastWeekAverageDeathPerMillionEachDay: 3.102757109967918,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Walker": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1068.9177892587468,
        lastWeekAverageDeathPerMillionEachDay: 7.830899555009133,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Waller": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 398.21887557470217,
        lastWeekAverageDeathPerMillionEachDay: 2.5858368543811827,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Ward": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 666.7777962993831,
        lastWeekAverageDeathPerMillionEachDay: 11.906746362488985,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1588.5402151496573,
        lastWeekAverageDeathPerMillionEachDay: 3.9813037973675622,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Webb": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1565.1432124112603,
        lastWeekAverageDeathPerMillionEachDay: 11.360326846930956,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Wharton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1756.6657041101164,
        lastWeekAverageDeathPerMillionEachDay: 6.875403929980886,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Wheeler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1384.4936708860762,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Wichita": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1383.9522044921728,
        lastWeekAverageDeathPerMillionEachDay: 14.044792083058741,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Wilbarger": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2036.181376771869,
        lastWeekAverageDeathPerMillionEachDay: 44.751239049931186,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Willacy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2856.0726659799607,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Williamson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 328.50676740874195,
        lastWeekAverageDeathPerMillionEachDay: 2.902858033066939,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Wilson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 587.4290189935383,
        lastWeekAverageDeathPerMillionEachDay: 2.7972810428263726,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Winkler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1248.4394506866415,
        lastWeekAverageDeathPerMillionEachDay: 35.669698591046895,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Wise": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 928.7837219935985,
        lastWeekAverageDeathPerMillionEachDay: 16.33026324384349,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Wood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1361.4703880190605,
        lastWeekAverageDeathPerMillionEachDay: 12.548114175290879,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Yoakum": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2065.878572248364,
        lastWeekAverageDeathPerMillionEachDay: 49.18758505353247,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Young": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1499.1671293725703,
        lastWeekAverageDeathPerMillionEachDay: 15.864202427222965,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Zapata": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 634.7415191480358,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Texas - Zavala": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2111.4864864864867,
        lastWeekAverageDeathPerMillionEachDay: 12.065637065637066,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - Bear River": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 224.8177370488925,
        lastWeekAverageDeathPerMillionEachDay: 3.058744721753639,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - Central Utah": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 353.8570417551309,
        lastWeekAverageDeathPerMillionEachDay: 6.972552546899132,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - Davis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 219.42100984300154,
        lastWeekAverageDeathPerMillionEachDay: 2.0093499069871936,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - Salt Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 437.7661174195583,
        lastWeekAverageDeathPerMillionEachDay: 4.431827960377981,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - San Juan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2090.4102430101907,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - Southeast Utah": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 298.29227671580196,
        lastWeekAverageDeathPerMillionEachDay: 10.653295596992928,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - Southwest Utah": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 499.9166805532411,
        lastWeekAverageDeathPerMillionEachDay: 6.801587490520287,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - Summit": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 118.63803535413453,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - Tooele": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 152.2301720200944,
        lastWeekAverageDeathPerMillionEachDay: 1.9770152210401866,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - TriCounty": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 229.59273780509346,
        lastWeekAverageDeathPerMillionEachDay: 2.522997118737291,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - Utah": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 304.9187800105308,
        lastWeekAverageDeathPerMillionEachDay: 3.8170981297341844,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - Wasatch": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 439.9988266697955,
        lastWeekAverageDeathPerMillionEachDay: 8.380930031805628,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Utah - Weber-Morgan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 330.47290672953005,
        lastWeekAverageDeathPerMillionEachDay: 7.868402541179286,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Addison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 108.7636294423145,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Bennington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 56.385678037778405,
        lastWeekAverageDeathPerMillionEachDay: 4.027548431269886,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Caledonia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 66.68222585269896,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Chittenden": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 348.0405925238438,
        lastWeekAverageDeathPerMillionEachDay: 5.233693120659305,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 384.5998137727217,
        lastWeekAverageDeathPerMillionEachDay: 14.458639615515853,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Lamoille": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 39.42906710827223,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Orange": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 34.61165720614702,
        lastWeekAverageDeathPerMillionEachDay: 4.9445224580210025,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Orleans": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 110.95905610829604,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Rutland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 137.47830420511764,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 102.7238952901094,
        lastWeekAverageDeathPerMillionEachDay: 4.89161406143378,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Windham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 118.42167590355736,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Vermont - Windsor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 54.48403617740003,
        lastWeekAverageDeathPerMillionEachDay: 2.5944779132095253,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virgin Islands": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 214.41622851176493,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Accomack": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 773.6105953707145,
        lastWeekAverageDeathPerMillionEachDay: 17.682527894187757,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Albemarle": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 265.2519893899204,
        lastWeekAverageDeathPerMillionEachDay: 1.3066600462557656,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Alexandria": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 520.6111849863262,
        lastWeekAverageDeathPerMillionEachDay: 0.8960605593568436,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Alleghany": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1345.8950201884254,
        lastWeekAverageDeathPerMillionEachDay: 19.227071716977505,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Amelia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 532.5218714340053,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Amherst": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 253.12450561619997,
        lastWeekAverageDeathPerMillionEachDay: 4.520080457432142,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Appomattox": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 377.0976054302055,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Arlington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 709.3336485927325,
        lastWeekAverageDeathPerMillionEachDay: 2.4126994850092944,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Augusta": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 291.16705047777873,
        lastWeekAverageDeathPerMillionEachDay: 11.344170797835533,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Bedford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 341.78513108092716,
        lastWeekAverageDeathPerMillionEachDay: 1.8083869369361223,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Bland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 955.4140127388532,
        lastWeekAverageDeathPerMillionEachDay: 22.747952684258408,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Botetourt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 448.846464586014,
        lastWeekAverageDeathPerMillionEachDay: 4.274728234152514,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Bristol": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 417.61126357236617,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Brunswick": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 492.88398743145837,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Buchanan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1142.6394972386208,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Buckingham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 583.1583858175879,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Buena Vista": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 617.4745291756714,
        lastWeekAverageDeathPerMillionEachDay: 44.105323512547955,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Campbell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 327.9584585952447,
        lastWeekAverageDeathPerMillionEachDay: 7.808534728458206,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Caroline": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 260.3742880390562,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Carroll": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1074.1499110469604,
        lastWeekAverageDeathPerMillionEachDay: 4.795312102888216,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Charles City": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1148.9300588826657,
        lastWeekAverageDeathPerMillionEachDay: 20.516608194333315,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Charlotte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 505.0505050505051,
        lastWeekAverageDeathPerMillionEachDay: 12.025012025012025,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Charlottesville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 719.3331358693351,
        lastWeekAverageDeathPerMillionEachDay: 9.067224401714308,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Chesapeake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 383.932035860886,
        lastWeekAverageDeathPerMillionEachDay: 3.5009000230475924,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Chesterfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 442.1743640909065,
        lastWeekAverageDeathPerMillionEachDay: 3.2393726307026114,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Clarke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 273.6165264381969,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Colonial Heights": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1554.40414507772,
        lastWeekAverageDeathPerMillionEachDay: 8.224360555966772,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Covington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 722.2824124232575,
        lastWeekAverageDeathPerMillionEachDay: 25.795800443687764,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Craig": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 194.89378288832586,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Culpeper": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 342.1727972626176,
        lastWeekAverageDeathPerMillionEachDay: -2.7156571211318856,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Cumberland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 402.7386226339106,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Danville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1448.406752572171,
        lastWeekAverageDeathPerMillionEachDay: 10.702512950040672,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Dickenson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 279.36862690319884,
        lastWeekAverageDeathPerMillionEachDay: 9.97745096082853,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Dinwiddie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 490.47085201793715,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Emporia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 5424.616535727648,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Essex": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 182.59837487446364,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Fairfax": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 569.918747363908,
        lastWeekAverageDeathPerMillionEachDay: 1.6183800165423337,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Fairfax City": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 416.3370664890295,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Falls Church": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 410.4809468427174,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Fauquier": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 393.13695206537307,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Floyd": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1079.433614832688,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Fluvanna": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 330.03300330033005,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 356.8752007423003,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Franklin City": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2133.801932973516,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Frederick": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 268.71787981592826,
        lastWeekAverageDeathPerMillionEachDay: 3.1990223787610508,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Fredericksburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 275.52004408320704,
        lastWeekAverageDeathPerMillionEachDay: 9.840001574400251,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Galax": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 4884.197258547345,
        lastWeekAverageDeathPerMillionEachDay: 22.507821467960113,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Giles": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 119.61722488038278,
        lastWeekAverageDeathPerMillionEachDay: 8.544087491455914,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Gloucester": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 187.42636821248794,
        lastWeekAverageDeathPerMillionEachDay: 7.650055845407671,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Goochland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 294.699616890498,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Grayson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1414.7909967845658,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Greene": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 201.82653009738127,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Greensville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1411.4326040931544,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Halifax": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 825.6907788033383,
        lastWeekAverageDeathPerMillionEachDay: 16.85083222047629,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Hampton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 319.6788342874136,
        lastWeekAverageDeathPerMillionEachDay: 5.310279639325808,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Hanover": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 603.1586956925189,
        lastWeekAverageDeathPerMillionEachDay: 6.628117535082626,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Harrisonburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 754.4892108042854,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Henrico": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 831.2727844313188,
        lastWeekAverageDeathPerMillionEachDay: 2.590980107318396,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Henry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 929.6437684198035,
        lastWeekAverageDeathPerMillionEachDay: 5.651329899208531,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Hopewell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 399.48510808291536,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Isle of Wight": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 781.4815812875582,
        lastWeekAverageDeathPerMillionEachDay: 7.699325924015351,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - James City": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 339.7671288370817,
        lastWeekAverageDeathPerMillionEachDay: 1.8668523562477015,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - King George": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 335.37039797287224,
        lastWeekAverageDeathPerMillionEachDay: 10.646679300726102,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - King William": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 291.579192908794,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - King and Queen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 284.69750889679705,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Lancaster": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 188.62586060548904,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Lee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1110.0200657473424,
        lastWeekAverageDeathPerMillionEachDay: 67.08912485286135,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Lexington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 402.90088638195004,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Loudoun": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 374.814406414888,
        lastWeekAverageDeathPerMillionEachDay: 1.3818042632806933,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Louisa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 239.4190098693836,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Lunenburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 245.98228927517218,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Lynchburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 547.6584558465582,
        lastWeekAverageDeathPerMillionEachDay: 5.215794817586269,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Madison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 301.63637734710807,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Manassas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 705.8537179019106,
        lastWeekAverageDeathPerMillionEachDay: 3.4771119108468502,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Manassas Park": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 457.7182744021056,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Martinsville": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2150.7089373904732,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Mathews": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 226.39800769753222,
        lastWeekAverageDeathPerMillionEachDay: 16.171286264109444,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Mecklenburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1242.3578644522183,
        lastWeekAverageDeathPerMillionEachDay: 4.670518287414354,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Middlesex": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1039.5010395010395,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Montgomery": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 324.7577003095347,
        lastWeekAverageDeathPerMillionEachDay: 4.349433486288411,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Nelson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 267.91694574681844,
        lastWeekAverageDeathPerMillionEachDay: 9.568462348100658,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - New Kent": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 216.53458057251743,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Newport News": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 357.09303947551956,
        lastWeekAverageDeathPerMillionEachDay: 4.782496064404279,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Norfolk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 416.07962363332257,
        lastWeekAverageDeathPerMillionEachDay: 1.1770286382837978,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Northampton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2647.309991460291,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Northumberland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 826.787928896238,
        lastWeekAverageDeathPerMillionEachDay: 11.811256127089115,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Nottoway": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1575.63025210084,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Orange": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 269.89824836036814,
        lastWeekAverageDeathPerMillionEachDay: 3.8556892622909733,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Page": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1380.6376035478202,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Patrick": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1590.1862789641073,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Petersburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 988.9619090155044,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Pittsylvania": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 513.6362130099083,
        lastWeekAverageDeathPerMillionEachDay: 4.733974313455375,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Poquoson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 244.47885257925185,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Portsmouth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 794.5083582279285,
        lastWeekAverageDeathPerMillionEachDay: 3.0266985075349653,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Powhatan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 202.34722784297855,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Prince Edward": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 701.692833961933,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Prince George": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 182.51505749224313,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Prince William": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 525.1576004337334,
        lastWeekAverageDeathPerMillionEachDay: 1.5186743795076152,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Pulaski": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 793.4875246128073,
        lastWeekAverageDeathPerMillionEachDay: 33.58677352858443,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Radford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 383.5826620636747,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Rappahannock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 271.3704206241519,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Richmond": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 554.139421478444,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Richmond City": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 390.5639743790031,
        lastWeekAverageDeathPerMillionEachDay: 2.4797712658984326,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Roanoke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 562.7163272673221,
        lastWeekAverageDeathPerMillionEachDay: 18.201067189239527,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Roanoke City": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 867.4339085966736,
        lastWeekAverageDeathPerMillionEachDay: 7.2046005697398146,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Rockbridge": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 132.90213972444957,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Rockingham": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 463.7086933177137,
        lastWeekAverageDeathPerMillionEachDay: 1.743265764352307,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Russell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 526.5929436545551,
        lastWeekAverageDeathPerMillionEachDay: 16.12019215269046,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Salem": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 948.5791075451564,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Scott": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1205.6014096262634,
        lastWeekAverageDeathPerMillionEachDay: 26.49673427750029,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Shenandoah": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1581.9882611885544,
        lastWeekAverageDeathPerMillionEachDay: 3.2753380148831353,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Smyth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1694.1270263087965,
        lastWeekAverageDeathPerMillionEachDay: 66.43635397289398,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Southampton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2268.73121206965,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Spotsylvania": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 484.5281356678781,
        lastWeekAverageDeathPerMillionEachDay: 8.390097587322563,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Stafford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 150.44282518543716,
        lastWeekAverageDeathPerMillionEachDay: 0.9344274856238333,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Staunton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 882.4001283491097,
        lastWeekAverageDeathPerMillionEachDay: 22.919483853223625,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Suffolk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 944.5433621400964,
        lastWeekAverageDeathPerMillionEachDay: 6.203897288276496,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Surry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 778.5736530675803,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Sussex": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1164.9789407653016,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Tazewell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 320.23648232540955,
        lastWeekAverageDeathPerMillionEachDay: 17.595411116780745,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Virginia Beach": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 293.3502824607644,
        lastWeekAverageDeathPerMillionEachDay: 2.2223506247027602,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Warren": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 771.8354745543273,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 744.324525493115,
        lastWeekAverageDeathPerMillionEachDay: 10.6332075070445,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Waynesboro": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 441.8912947414935,
        lastWeekAverageDeathPerMillionEachDay: 6.312732782021335,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Westmoreland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 499.58368026644473,
        lastWeekAverageDeathPerMillionEachDay: 7.929899686768963,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Williamsburg": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 534.9739200213988,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Winchester": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 213.69043379158063,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Wise": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1417.7567343444882,
        lastWeekAverageDeathPerMillionEachDay: 7.642893446601014,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - Wythe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 592.6649002928461,
        lastWeekAverageDeathPerMillionEachDay: 24.90188656692631,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Virginia - York": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 146.45577035735212,
        lastWeekAverageDeathPerMillionEachDay: 2.0922252908193157,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 800.6805784917179,
        lastWeekAverageDeathPerMillionEachDay: 21.446801209599585,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Asotin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 708.5289168364184,
        lastWeekAverageDeathPerMillionEachDay: 31.630755215911535,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Benton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 748.5689123734038,
        lastWeekAverageDeathPerMillionEachDay: 4.892607270414404,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Chelan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 427.461139896373,
        lastWeekAverageDeathPerMillionEachDay: 5.551443375277572,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Clallam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 51.72569862021699,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 278.55096151285943,
        lastWeekAverageDeathPerMillionEachDay: 1.4629777390381273,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Columbia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1003.7641154328732,
        lastWeekAverageDeathPerMillionEachDay: 35.84871840831689,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Cowlitz": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 171.80110856925845,
        lastWeekAverageDeathPerMillionEachDay: 5.166950633661908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 322.3652398167123,
        lastWeekAverageDeathPerMillionEachDay: 9.86832366785854,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Ferry": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 131.1131506490101,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Franklin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 798.1348847955303,
        lastWeekAverageDeathPerMillionEachDay: 3.0005070856974823,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Garfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 449.43820224719104,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 409.27833996705317,
        lastWeekAverageDeathPerMillionEachDay: 5.846833428100759,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Grays Harbor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 266.4499540373829,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Island": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 246.64967524459428,
        lastWeekAverageDeathPerMillionEachDay: 8.38944473621069,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 31.035659973309325,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - King": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 424.3641861485044,
        lastWeekAverageDeathPerMillionEachDay: 1.2682731205872817,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Kitsap": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 121.5590500712778,
        lastWeekAverageDeathPerMillionEachDay: 1.5786889619646467,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Kittitas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 479.8164180661312,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Klickitat": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 133.77926421404683,
        lastWeekAverageDeathPerMillionEachDay: 6.370441153049849,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Lewis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 284.9814762040467,
        lastWeekAverageDeathPerMillionEachDay: 7.080285123081906,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 365.6641374897158,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Mason": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 194.70404984423678,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Okanogan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 615.4865894941173,
        lastWeekAverageDeathPerMillionEachDay: 3.381794447769875,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Pacific": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 133.5054069689822,
        lastWeekAverageDeathPerMillionEachDay: 6.357400331856295,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Pend Oreille": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 291.46021568055954,
        lastWeekAverageDeathPerMillionEachDay: 10.409293417162841,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Pierce": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 337.02402263033434,
        lastWeekAverageDeathPerMillionEachDay: 1.4207101656548053,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Skagit": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 255.4080724430169,
        lastWeekAverageDeathPerMillionEachDay: 3.316987953805414,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Skamania": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 82.76090374906894,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Snohomish": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 411.15069889536704,
        lastWeekAverageDeathPerMillionEachDay: 3.649266558242903,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Spokane": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 587.2248937448115,
        lastWeekAverageDeathPerMillionEachDay: 7.104628774948861,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Stevens": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 262.44997047437835,
        lastWeekAverageDeathPerMillionEachDay: 12.497617641637062,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Thurston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 172.09571275160397,
        lastWeekAverageDeathPerMillionEachDay: 1.9668081457326165,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Walla Walla": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 493.7458854509546,
        lastWeekAverageDeathPerMillionEachDay: 11.755854415498918,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Whatcom": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 226.82957683197597,
        lastWeekAverageDeathPerMillionEachDay: 3.738949068658944,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Whitman": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 498.9621587098834,
        lastWeekAverageDeathPerMillionEachDay: 14.25606167742524,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Washington - Yakima": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1215.7545849892174,
        lastWeekAverageDeathPerMillionEachDay: 3.986080606522024,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Barbour": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 486.58840703120256,
        lastWeekAverageDeathPerMillionEachDay: 26.06723609095728,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Berkeley": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 385.9999496521804,
        lastWeekAverageDeathPerMillionEachDay: 11.987576076154673,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Boone": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 885.4919140606795,
        lastWeekAverageDeathPerMillionEachDay: 6.657833940305861,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Braxton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 71.64863509350147,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Brooke": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 866.037649847304,
        lastWeekAverageDeathPerMillionEachDay: 26.04624510818959,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Cabell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 870.0853771276306,
        lastWeekAverageDeathPerMillionEachDay: 12.429791101823294,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Clay": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 352.60930888575456,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Doddridge": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 355.1136363636363,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Fayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 990.4258831297458,
        lastWeekAverageDeathPerMillionEachDay: 10.106386562548426,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Gilmer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 127.82819890067749,
        lastWeekAverageDeathPerMillionEachDay: 18.261171271525356,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1383.1258644536654,
        lastWeekAverageDeathPerMillionEachDay: 49.39735230191661,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Greenbrier": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 634.7008251110728,
        lastWeekAverageDeathPerMillionEachDay: 24.728603575756082,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Hampshire": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 345.1995685005394,
        lastWeekAverageDeathPerMillionEachDay: 12.328556017876405,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Hancock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 694.2034015966677,
        lastWeekAverageDeathPerMillionEachDay: 39.66876580552387,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Hardy": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 435.5400696864111,
        lastWeekAverageDeathPerMillionEachDay: 41.48000663680106,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Harrison": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 446.05685738075414,
        lastWeekAverageDeathPerMillionEachDay: 10.620401366208432,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 979.8432250839867,
        lastWeekAverageDeathPerMillionEachDay: 4.999200127979523,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 384.9788261645609,
        lastWeekAverageDeathPerMillionEachDay: 9.999450030248335,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Kanawha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1027.3741887673755,
        lastWeekAverageDeathPerMillionEachDay: 13.634161755695068,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Lewis": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 188.5962155026089,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 342.98593757655937,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Logan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1686.49864143165,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Marion": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 178.34213154515626,
        lastWeekAverageDeathPerMillionEachDay: 5.09548947271875,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Marshall": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1572.1725459369165,
        lastWeekAverageDeathPerMillionEachDay: 51.46993453960144,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Mason": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 452.5569467491325,
        lastWeekAverageDeathPerMillionEachDay: 5.387582699394434,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - McDowell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 226.96323195642304,
        lastWeekAverageDeathPerMillionEachDay: 8.105829712729394,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Mercer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 833.9289969025496,
        lastWeekAverageDeathPerMillionEachDay: 9.725119497405826,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Mineral": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2047.0448116718776,
        lastWeekAverageDeathPerMillionEachDay: 42.535996086688364,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Mingo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 853.8251366120219,
        lastWeekAverageDeathPerMillionEachDay: 6.098750975800156,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Monongalia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 123.09207287050714,
        lastWeekAverageDeathPerMillionEachDay: 6.763300707170722,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 828.6252354048963,
        lastWeekAverageDeathPerMillionEachDay: 10.76136669357008,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Morgan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 447.32721986132856,
        lastWeekAverageDeathPerMillionEachDay: 47.92791641371377,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Nicholas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 244.9379490529066,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Ohio": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 748.5933689116416,
        lastWeekAverageDeathPerMillionEachDay: 13.798956108970351,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Pendleton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 143.4926101305783,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Pleasants": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 670.2412868632706,
        lastWeekAverageDeathPerMillionEachDay: 19.1497510532363,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Pocahontas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 606.2810719049351,
        lastWeekAverageDeathPerMillionEachDay: 51.966949020423,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Preston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 568.3177793730557,
        lastWeekAverageDeathPerMillionEachDay: 8.546132020647454,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Putnam": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 921.1691762621788,
        lastWeekAverageDeathPerMillionEachDay: 7.5920536505124625,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Raleigh": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 340.78052371150886,
        lastWeekAverageDeathPerMillionEachDay: 11.683903670108874,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Randolph": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 139.39710751001917,
        lastWeekAverageDeathPerMillionEachDay: 4.978468125357828,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Ritchie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 628.0092108017584,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Roane": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 365.28345996493283,
        lastWeekAverageDeathPerMillionEachDay: 10.436670284712365,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Summers": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 954.4261512765452,
        lastWeekAverageDeathPerMillionEachDay: 11.362216086625537,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Taylor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 658.879904162923,
        lastWeekAverageDeathPerMillionEachDay: 17.113763744491504,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Tucker": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 146.2202076326948,
        lastWeekAverageDeathPerMillionEachDay: 20.88860109038497,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Tyler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 232.8017692934466,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Upshur": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 165.45334215751157,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 456.82960255824577,
        lastWeekAverageDeathPerMillionEachDay: 3.625631766335284,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Wetzel": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1062.0643876535016,
        lastWeekAverageDeathPerMillionEachDay: 18.965435493812528,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Wirt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 171.79178835251676,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Wood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 443.0182715103331,
        lastWeekAverageDeathPerMillionEachDay: 13.683962054373222,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Wyoming": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 784.5444738648623,
        lastWeekAverageDeathPerMillionEachDay: 21.01458412138024,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 593.4718100890207,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Ashland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 899.6272972625627,
        lastWeekAverageDeathPerMillionEachDay: 9.17987038023023,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Barron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1171.4260454424896,
        lastWeekAverageDeathPerMillionEachDay: 9.472447806812584,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Bayfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1064.1127959563714,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Brown": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 627.4996030876006,
        lastWeekAverageDeathPerMillionEachDay: 3.240101220762137,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Buffalo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 460.44048806691745,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Burnett": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1038.0173867912285,
        lastWeekAverageDeathPerMillionEachDay: 9.268012382064539,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Calumet": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 638.862824172972,
        lastWeekAverageDeathPerMillionEachDay: 2.8520661793436246,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Chippewa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1020.7553589656346,
        lastWeekAverageDeathPerMillionEachDay: 8.8377087356332,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1437.8558693276589,
        lastWeekAverageDeathPerMillionEachDay: 8.216319253300908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Columbia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 486.6856705833275,
        lastWeekAverageDeathPerMillionEachDay: 19.864721248299084,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Crawford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 681.9168061496496,
        lastWeekAverageDeathPerMillionEachDay: 8.856062417527916,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Dane": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 259.7426352902441,
        lastWeekAverageDeathPerMillionEachDay: 6.532762456998091,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Dodge": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1354.7513063673312,
        lastWeekAverageDeathPerMillionEachDay: 40.65880271210478,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Door": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 505.99971085730806,
        lastWeekAverageDeathPerMillionEachDay: 10.326524711373633,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 672.0741599073002,
        lastWeekAverageDeathPerMillionEachDay: 9.932130441979805,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Dunn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 440.8393581378946,
        lastWeekAverageDeathPerMillionEachDay: 6.297705116255636,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Eau Claire": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 754.9261319113964,
        lastWeekAverageDeathPerMillionEachDay: 6.825733561585863,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Florence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2793.9464493597206,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Fond du Lac": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 667.2920514878679,
        lastWeekAverageDeathPerMillionEachDay: 4.1446711272538375,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Forest": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2554.4202576632606,
        lastWeekAverageDeathPerMillionEachDay: 15.865964333312178,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1574.6806897490233,
        lastWeekAverageDeathPerMillionEachDay: 19.440502342580533,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Green": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 270.5627705627706,
        lastWeekAverageDeathPerMillionEachDay: 3.8651824366110086,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Green Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 687.3579019721886,
        lastWeekAverageDeathPerMillionEachDay: 15.106767076311836,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Iowa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 253.39978038685695,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Iron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3516.792685071215,
        lastWeekAverageDeathPerMillionEachDay: 100.4797910020347,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 629.7534273119219,
        lastWeekAverageDeathPerMillionEachDay: 48.442571331686295,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 637.025327655157,
        lastWeekAverageDeathPerMillionEachDay: 10.111513137383444,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Juneau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 299.7714242889797,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Kenosha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1203.1068465036183,
        lastWeekAverageDeathPerMillionEachDay: 16.850235945428828,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Kewaunee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1125.57502202212,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - La Crosse": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 440.6182212581345,
        lastWeekAverageDeathPerMillionEachDay: 10.894406569569261,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Lafayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 360.03600360036006,
        lastWeekAverageDeathPerMillionEachDay: 8.572285800008572,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Langlade": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2084.527593934025,
        lastWeekAverageDeathPerMillionEachDay: 7.444741406907232,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1884.5359330264928,
        lastWeekAverageDeathPerMillionEachDay: 10.354593038607103,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Manitowoc": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 696.3700130411112,
        lastWeekAverageDeathPerMillionEachDay: 7.235013122505051,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Marathon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1201.2498894555313,
        lastWeekAverageDeathPerMillionEachDay: 1.052804460521938,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Marinette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 966.5427509293679,
        lastWeekAverageDeathPerMillionEachDay: 10.621348911311735,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Marquette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1284.191601386927,
        lastWeekAverageDeathPerMillionEachDay: 27.51839145829129,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Menominee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2194.9078138718173,
        lastWeekAverageDeathPerMillionEachDay: 62.71165182490906,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Milwaukee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 973.8550066298274,
        lastWeekAverageDeathPerMillionEachDay: 13.897108827352895,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 454.0246038094827,
        lastWeekAverageDeathPerMillionEachDay: 3.0886027470032835,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Oconto": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1080.9385710519377,
        lastWeekAverageDeathPerMillionEachDay: 15.065345938006102,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Oneida": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1404.6916701783957,
        lastWeekAverageDeathPerMillionEachDay: 4.013404771938274,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Outagamie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 809.0055086888258,
        lastWeekAverageDeathPerMillionEachDay: 7.603435232037836,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Ozaukee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 560.4061824010042,
        lastWeekAverageDeathPerMillionEachDay: 3.202321042291453,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Pepin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 686.1534239055853,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Pierce": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 701.688730878982,
        lastWeekAverageDeathPerMillionEachDay: 13.365499635790133,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 502.47813078135346,
        lastWeekAverageDeathPerMillionEachDay: 6.525690010147447,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Portage": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 678.2343299610015,
        lastWeekAverageDeathPerMillionEachDay: 4.037109106910723,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Price": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 299.60302599056257,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Racine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1253.1136818619434,
        lastWeekAverageDeathPerMillionEachDay: 13.826457581519701,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Richland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 753.5358219336888,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Rock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 685.627532842783,
        lastWeekAverageDeathPerMillionEachDay: 6.121674400381991,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Rusk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 775.8499083086472,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Sauk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 450.0170696129852,
        lastWeekAverageDeathPerMillionEachDay: 11.084164276181902,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Sawyer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 603.9376736320812,
        lastWeekAverageDeathPerMillionEachDay: 17.25536210377375,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Shawano": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1491.4790092667304,
        lastWeekAverageDeathPerMillionEachDay: 6.985850160499908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Sheboygan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 745.6216403676088,
        lastWeekAverageDeathPerMillionEachDay: 6.192870767172831,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - St. Croix": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 264.6465314763969,
        lastWeekAverageDeathPerMillionEachDay: 3.1505539461475816,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Taylor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 884.8252470137149,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Trempealeau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 944.3826098687981,
        lastWeekAverageDeathPerMillionEachDay: 14.454835865338746,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Vernon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 713.7758743754462,
        lastWeekAverageDeathPerMillionEachDay: 23.174541375826173,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Vilas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1036.2694300518135,
        lastWeekAverageDeathPerMillionEachDay: 12.872912174556689,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Walworth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 847.2291754919705,
        lastWeekAverageDeathPerMillionEachDay: 19.255208533908423,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Washburn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 699.7455470737913,
        lastWeekAverageDeathPerMillionEachDay: 27.262813522355504,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 713.0570298601822,
        lastWeekAverageDeathPerMillionEachDay: 5.250788143300311,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Waukesha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 801.5873408576985,
        lastWeekAverageDeathPerMillionEachDay: 8.128972151555143,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Waupaca": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2549.519513630124,
        lastWeekAverageDeathPerMillionEachDay: 16.81001877118763,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Waushara": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 613.6726261097247,
        lastWeekAverageDeathPerMillionEachDay: 5.844501201044997,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Winnebago": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 936.552903604856,
        lastWeekAverageDeathPerMillionEachDay: 9.972169337407518,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Wood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 602.7479828490801,
        lastWeekAverageDeathPerMillionEachDay: 7.827895881156885,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Albany": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 231.4814814814815,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Big Horn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1272.2646310432572,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Campbell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 474.74158952115846,
        lastWeekAverageDeathPerMillionEachDay: 6.1654751885864725,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Carbon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 810.8108108108107,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Converse": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 723.4843003906817,
        lastWeekAverageDeathPerMillionEachDay: 31.0064700167435,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Crook": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 791.1392405063292,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Fremont": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1375.4107129212196,
        lastWeekAverageDeathPerMillionEachDay: 10.915958039057298,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Goshen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 984.0284611308757,
        lastWeekAverageDeathPerMillionEachDay: 10.813499572866766,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Hot Springs": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 226.60321776569225,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 592.0663114268799,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Laramie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 472.36180904522604,
        lastWeekAverageDeathPerMillionEachDay: 8.614501076812633,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 504.28643469490663,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Natrona": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 926.6447945102557,
        lastWeekAverageDeathPerMillionEachDay: 21.46667477629936,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Niobrara": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 424.4482173174873,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Park": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 239.775296293759,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Platte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 953.1752651018707,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Sheridan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 492.0452681646711,
        lastWeekAverageDeathPerMillionEachDay: 9.37229082218421,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Sublette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 508.5952598921778,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Sweetwater": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 354.2498169709279,
        lastWeekAverageDeathPerMillionEachDay: 3.3738077806755036,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Teton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 85.23695874531198,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Uinta": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 346.08919212894295,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Washakie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1409.3529788597054,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Weston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 288.72527789808004,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
}

const obj2 = {
  "US - West Virginia - Ritchie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 628.0092108017584,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Roane": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 365.28345996493283,
        lastWeekAverageDeathPerMillionEachDay: 10.436670284712365,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Summers": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 954.4261512765452,
        lastWeekAverageDeathPerMillionEachDay: 11.362216086625537,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Taylor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 658.879904162923,
        lastWeekAverageDeathPerMillionEachDay: 17.113763744491504,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Tucker": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 146.2202076326948,
        lastWeekAverageDeathPerMillionEachDay: 20.88860109038497,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Tyler": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 232.8017692934466,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Upshur": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 165.45334215751157,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Wayne": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 456.82960255824577,
        lastWeekAverageDeathPerMillionEachDay: 3.625631766335284,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Wetzel": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1062.0643876535016,
        lastWeekAverageDeathPerMillionEachDay: 18.965435493812528,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Wirt": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 171.79178835251676,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Wood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 443.0182715103331,
        lastWeekAverageDeathPerMillionEachDay: 13.683962054373222,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - West Virginia - Wyoming": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 784.5444738648623,
        lastWeekAverageDeathPerMillionEachDay: 21.01458412138024,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Adams": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 593.4718100890207,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Ashland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 899.6272972625627,
        lastWeekAverageDeathPerMillionEachDay: 9.17987038023023,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Barron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1171.4260454424896,
        lastWeekAverageDeathPerMillionEachDay: 9.472447806812584,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Bayfield": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1064.1127959563714,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Brown": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 627.4996030876006,
        lastWeekAverageDeathPerMillionEachDay: 3.240101220762137,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Buffalo": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 460.44048806691745,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Burnett": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1038.0173867912285,
        lastWeekAverageDeathPerMillionEachDay: 9.268012382064539,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Calumet": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 638.862824172972,
        lastWeekAverageDeathPerMillionEachDay: 2.8520661793436246,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Chippewa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1020.7553589656346,
        lastWeekAverageDeathPerMillionEachDay: 8.8377087356332,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Clark": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1437.8558693276589,
        lastWeekAverageDeathPerMillionEachDay: 8.216319253300908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Columbia": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 486.6856705833275,
        lastWeekAverageDeathPerMillionEachDay: 19.864721248299084,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Crawford": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 681.9168061496496,
        lastWeekAverageDeathPerMillionEachDay: 8.856062417527916,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Dane": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 259.7426352902441,
        lastWeekAverageDeathPerMillionEachDay: 6.532762456998091,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Dodge": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1354.7513063673312,
        lastWeekAverageDeathPerMillionEachDay: 40.65880271210478,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Door": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 505.99971085730806,
        lastWeekAverageDeathPerMillionEachDay: 10.326524711373633,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Douglas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 672.0741599073002,
        lastWeekAverageDeathPerMillionEachDay: 9.932130441979805,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Dunn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 440.8393581378946,
        lastWeekAverageDeathPerMillionEachDay: 6.297705116255636,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Eau Claire": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 754.9261319113964,
        lastWeekAverageDeathPerMillionEachDay: 6.825733561585863,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Florence": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2793.9464493597206,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Fond du Lac": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 667.2920514878679,
        lastWeekAverageDeathPerMillionEachDay: 4.1446711272538375,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Forest": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2554.4202576632606,
        lastWeekAverageDeathPerMillionEachDay: 15.865964333312178,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Grant": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1574.6806897490233,
        lastWeekAverageDeathPerMillionEachDay: 19.440502342580533,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Green": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 270.5627705627706,
        lastWeekAverageDeathPerMillionEachDay: 3.8651824366110086,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Green Lake": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 687.3579019721886,
        lastWeekAverageDeathPerMillionEachDay: 15.106767076311836,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Iowa": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 253.39978038685695,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Iron": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 3516.792685071215,
        lastWeekAverageDeathPerMillionEachDay: 100.4797910020347,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Jackson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 629.7534273119219,
        lastWeekAverageDeathPerMillionEachDay: 48.442571331686295,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Jefferson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 637.025327655157,
        lastWeekAverageDeathPerMillionEachDay: 10.111513137383444,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Juneau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 299.7714242889797,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Kenosha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1203.1068465036183,
        lastWeekAverageDeathPerMillionEachDay: 16.850235945428828,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Kewaunee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1125.57502202212,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - La Crosse": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 440.6182212581345,
        lastWeekAverageDeathPerMillionEachDay: 10.894406569569261,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Lafayette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 360.03600360036006,
        lastWeekAverageDeathPerMillionEachDay: 8.572285800008572,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Langlade": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2084.527593934025,
        lastWeekAverageDeathPerMillionEachDay: 7.444741406907232,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1884.5359330264928,
        lastWeekAverageDeathPerMillionEachDay: 10.354593038607103,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Manitowoc": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 696.3700130411112,
        lastWeekAverageDeathPerMillionEachDay: 7.235013122505051,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Marathon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1201.2498894555313,
        lastWeekAverageDeathPerMillionEachDay: 1.052804460521938,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Marinette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 966.5427509293679,
        lastWeekAverageDeathPerMillionEachDay: 10.621348911311735,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Marquette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1284.191601386927,
        lastWeekAverageDeathPerMillionEachDay: 27.51839145829129,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Menominee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2194.9078138718173,
        lastWeekAverageDeathPerMillionEachDay: 62.71165182490906,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Milwaukee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 973.8550066298274,
        lastWeekAverageDeathPerMillionEachDay: 13.897108827352895,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Monroe": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 454.0246038094827,
        lastWeekAverageDeathPerMillionEachDay: 3.0886027470032835,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Oconto": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1080.9385710519377,
        lastWeekAverageDeathPerMillionEachDay: 15.065345938006102,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Oneida": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1404.6916701783957,
        lastWeekAverageDeathPerMillionEachDay: 4.013404771938274,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Outagamie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 809.0055086888258,
        lastWeekAverageDeathPerMillionEachDay: 7.603435232037836,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Ozaukee": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 560.4061824010042,
        lastWeekAverageDeathPerMillionEachDay: 3.202321042291453,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Pepin": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 686.1534239055853,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Pierce": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 701.688730878982,
        lastWeekAverageDeathPerMillionEachDay: 13.365499635790133,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Polk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 502.47813078135346,
        lastWeekAverageDeathPerMillionEachDay: 6.525690010147447,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Portage": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 678.2343299610015,
        lastWeekAverageDeathPerMillionEachDay: 4.037109106910723,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Price": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 299.60302599056257,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Racine": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1253.1136818619434,
        lastWeekAverageDeathPerMillionEachDay: 13.826457581519701,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Richland": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 753.5358219336888,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Rock": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 685.627532842783,
        lastWeekAverageDeathPerMillionEachDay: 6.121674400381991,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Rusk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 775.8499083086472,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Sauk": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 450.0170696129852,
        lastWeekAverageDeathPerMillionEachDay: 11.084164276181902,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Sawyer": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 603.9376736320812,
        lastWeekAverageDeathPerMillionEachDay: 17.25536210377375,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Shawano": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1491.4790092667304,
        lastWeekAverageDeathPerMillionEachDay: 6.985850160499908,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Sheboygan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 745.6216403676088,
        lastWeekAverageDeathPerMillionEachDay: 6.192870767172831,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - St. Croix": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 264.6465314763969,
        lastWeekAverageDeathPerMillionEachDay: 3.1505539461475816,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Taylor": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 884.8252470137149,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Trempealeau": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 944.3826098687981,
        lastWeekAverageDeathPerMillionEachDay: 14.454835865338746,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Vernon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 713.7758743754462,
        lastWeekAverageDeathPerMillionEachDay: 23.174541375826173,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Vilas": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1036.2694300518135,
        lastWeekAverageDeathPerMillionEachDay: 12.872912174556689,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Walworth": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 847.2291754919705,
        lastWeekAverageDeathPerMillionEachDay: 19.255208533908423,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Washburn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 699.7455470737913,
        lastWeekAverageDeathPerMillionEachDay: 27.262813522355504,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Washington": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 713.0570298601822,
        lastWeekAverageDeathPerMillionEachDay: 5.250788143300311,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Waukesha": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 801.5873408576985,
        lastWeekAverageDeathPerMillionEachDay: 8.128972151555143,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Waupaca": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 2549.519513630124,
        lastWeekAverageDeathPerMillionEachDay: 16.81001877118763,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Waushara": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 613.6726261097247,
        lastWeekAverageDeathPerMillionEachDay: 5.844501201044997,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Winnebago": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 936.552903604856,
        lastWeekAverageDeathPerMillionEachDay: 9.972169337407518,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wisconsin - Wood": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 602.7479828490801,
        lastWeekAverageDeathPerMillionEachDay: 7.827895881156885,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Albany": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 231.4814814814815,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Big Horn": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1272.2646310432572,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Campbell": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 474.74158952115846,
        lastWeekAverageDeathPerMillionEachDay: 6.1654751885864725,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Carbon": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 810.8108108108107,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Converse": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 723.4843003906817,
        lastWeekAverageDeathPerMillionEachDay: 31.0064700167435,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Crook": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 791.1392405063292,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Fremont": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1375.4107129212196,
        lastWeekAverageDeathPerMillionEachDay: 10.915958039057298,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Goshen": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 984.0284611308757,
        lastWeekAverageDeathPerMillionEachDay: 10.813499572866766,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Hot Springs": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 226.60321776569225,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Johnson": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 592.0663114268799,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Laramie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 472.36180904522604,
        lastWeekAverageDeathPerMillionEachDay: 8.614501076812633,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Lincoln": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 504.28643469490663,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Natrona": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 926.6447945102557,
        lastWeekAverageDeathPerMillionEachDay: 21.46667477629936,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Niobrara": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 424.4482173174873,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Park": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 239.775296293759,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Platte": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 953.1752651018707,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Sheridan": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 492.0452681646711,
        lastWeekAverageDeathPerMillionEachDay: 9.37229082218421,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Sublette": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 508.5952598921778,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Sweetwater": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 354.2498169709279,
        lastWeekAverageDeathPerMillionEachDay: 3.3738077806755036,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Teton": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 85.23695874531198,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Uinta": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 346.08919212894295,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Washakie": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 1409.3529788597054,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
  "US - Wyoming - Weston": {
    medianAge: 35,
    dataPoints: [
      {
        allDeathPerMillion: 288.72527789808004,
        lastWeekAverageDeathPerMillionEachDay: 0,
        updatedAt: "2020-12-21 05:27:58",
      },
    ],
  },
}

const setMedianAge = (age: {[province: string]: number}) => {
  for (const province in obj) {
    const secondPart = province.split('-')[1]
    let targetKey;
    for (const key in age) {
      if (secondPart.includes(key)) {
        targetKey = key
        break;
      }
    }
    // @ts-ignore
    obj[province].medianAge = age[key] || 38.2
  } 

  return {...obj}
}

console.log(JSON.stringify(setMedianAge({Maine: 44.9, 
Hampshire: 43, Vermont: 42.8, "West Virginia": 42.8,
Florida: 42.2, Puerto: 41.5, "Virgin Islands": 41,
Connecticut: 41, Pennsylvania: 40.8, Delaware: 40.7,
Rhode: 40.1, Jersey: 40, Montana: 39.9, Michigan: 39.8,
"South Carolina": 39.6, Wisconsin: 39.6, Massachusetts: 39.4,
Ohio: 39.4, Oregon:39.4, Alabama: 39.2, Hawaii: 39.2,
"New York": 39, Kentucky: 38.9, "North Carolina": 38.9, Maryland: 38.8,
Tennessee: 38.8, Missouri: 38.7, Virginia: 38.4, Illinois: 38.3,
Iowa: 38.2, Minnesota: 38.1, Nevada: 38.1, "New Mexico": 38.1,
Wyoming: 38, Arizona: 37.9, Indiana: 37.9, Mississippi: 37.7,
Washington: 37.7, Louisiana: 37.2, "South Dakota": 37.1, Colorado: 36.9,
Georgia: 36.9, Kansas: 36.9, California: 36.8, Oklahoma: 36.7,
Idaho: 36.6, Nebraska: 36.6, "North Dakota": 35.2, Texas: 34.8,
Alaska: 34.6, "District Of Columbia": 34, Mariana: 33.6,
Utah: 31, Guam: 29 })))

// console.log(JSON.stringify(setMedianAge({Maine: 44.9, 
// Hampshire: 43, Vermont: 42.8, "West Virginia": 42.8,
// Florida: 42.2, Puerto: 41.5, "Virgin Islands": 41,
// Connecticut: 41, Pennsylvania: 40.8, Delaware: 40.7,
// Rhode: 40.1, Jersey: 40, Montana: 39.9, Michigan: 39.8,
// "South Carolina": 39.6, Wisconsin: 39.6, Massachusetts: 39.4,
// Ohio: 39.4, Oregon:39.4, Alabama: 39.2, Hawaii: 39.2,
// "New York": 39, Kentucky: 38.9, "North Carolina": 38.9, Maryland: 38.8,
// Tennessee: 38.8, Missouri: 38.7, Virginia: 38.4, Illinois: 38.3,
// Iowa: 38.2, Minnesota: 38.1, Nevada: 38.1, "New Mexico": 38.1,
// Wyoming: 38, Arizona: 37.9, Indiana: 37.9, Mississippi: 37.7,
// Washington: 37.7, Louisiana: 37.2, "South Dakota": 37.1, Colorado: 36.9,
// Georgia: 36.9, Kansas: 36.9, California: 36.8, Oklahoma: 36.7,
// Idaho: 36.6, Nebraska: 36.6, "North Dakota": 35.2, Texas: 34.8,
// Alaska: 34.6, "District Of Columbia": 34, Mariana: 33.6,
// Utah: 31, Guam: 29 })))